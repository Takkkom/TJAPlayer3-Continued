...
namespace TJAPlayer3
{
    internal class CDTX : Activity
    {
        ...
        public class CWAV : IDisposable
        {
            ...
            public int n内部番号;
            public int n表記上の番号;
            public CSound[] rSound = new CSound[TJAPlayer3._MainConfig.nPoliphonicSounds];     // 4
            public string strコメント文 = "";
            public string strファイル名 = "";
        ...
        public void tWave再生位置自動補正(CWAV wc)
        {
            if (wc.rSound[0] != null && wc.rSound[0].n総演奏時間ms >= 5000)
            {
                for (int i = 0; i < nPolyphonicSounds; i++)
                {
                    ...
                    {
                        long nCurrentTime = CSound管理.rc演奏用タイマ.SystemTimeMs;
        ...
        public void tチップの再生(CChip pChip, long n再生開始システム時刻ms, int nLane)
        {
            if (pChip.n整数値_内部番号 >= 0)
            {
                ...
                if (this.listWAV.TryGetValue(pChip.n整数値_内部番号, out CWAV wc))
                {
                    ...
                    {
                        sound.db周波数倍率 = 1.0;
                        sound.db再生速度 = ((double)TJAPlayer3._MainConfig.n演奏速度) / 20.0;
            ...
            foreach (CWAV cwav in this.listWAV.Values)
            {
                for (int i = 0; i < nPolyphonicSounds; i++)
                {
                    if ((cwav.rSound[i] != null) && cwav.rSound[i].b再生中)
                    {
                        ...
                        cwav.n一時停止時刻[i] = CSound管理.rc演奏用タイマ.SystemTimeMs;
                    }
                }
            }
        }
            ...
            foreach (CWAV cwav in this.listWAV.Values)
            {
                for (int i = 0; i < nPolyphonicSounds; i++)
                {
                    if ((cwav.rSound[i] != null) && cwav.rSound[i].b一時停止中)
                    {
                        ...
                        cwav.n再生開始時刻[i] += CSound管理.rc演奏用タイマ.SystemTimeMs - cwav.n一時停止時刻[i];
                    }
                }
            }
        }
