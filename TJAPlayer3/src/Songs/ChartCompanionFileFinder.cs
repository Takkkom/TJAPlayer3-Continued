...
{
    internal static class ChartCompanionFileFinder
    {
            ...
            string expectedCompanionFileName)
        {
            ...
            bool TryFindViaDecodedFileName(string prefix, Encoding encoding, out string foundCompanionFileName)
            {
                ...
                {
                    Trace.TraceWarning(
                        $"{nameof(ChartCompanionFileFinder)} could not find expected file '{expectedCompanionPath}' and could not check the existence of a file via {prefix} '{encoding.EncodingName}'. Possible illegal file path when combining directory '{directory}' with encoded file name '{decodedCompanionFileName}'.");
                ...
                Trace.TraceInformation(
                    $"{nameof(ChartCompanionFileFinder)} could not find expected file '{expectedCompanionPath}' but found '{decodedCompanionFileName}' via {prefix} '{encoding.EncodingName}', Code Page {encoding.CodePage}, Windows Code Page {encoding.WindowsCodePage}.");
            ...
            Trace.TraceWarning(
                $"{nameof(ChartCompanionFileFinder)} could not find expected file '{expectedCompanionPath}' by any available means.");
            ...
            out string foundCompanionFileName)
        {
            ...
            {
                Trace.TraceInformation(
                    $"{nameof(ChartCompanionFileFinder)} could not find expected file '{expectedCompanionPath}' but found '{mainFileNameWithCompanionFileExtension}' by matching the '{mainFileName}' file name with the expected file extension.");
            ...
            out string foundCompanionFileName)
        {
            ...
            {
                Trace.TraceWarning(
                    $"{nameof(ChartCompanionFileFinder)} could not find expected file '{expectedCompanionPath}' and could not search for appropriate sibling files because this file has no extension.");
            }
            else
            {
                ...
                if (filesWithTheCompanionFileExtension.Length == 1)
                {
                    ...
                    Trace.TraceInformation(
                        $"{nameof(ChartCompanionFileFinder)} could not find expected file '{expectedCompanionPath}' but found '{foundCompanionFileName}' by searching for a single sibling file with the expected extension.");
