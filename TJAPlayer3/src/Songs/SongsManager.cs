...
using TJAPlayer3.SongInfoNodeComparers;
using FDK;
...
namespace TJAPlayer3
{
	...
	internal class SongsManager
	{
		...
		[NonSerialized]
		public List<ScoreInfo> listSongsDB;					// songs.dbから構築されるlist
		public List<SongInfoNode> list曲ルート;			// 起動時にフォルダ検索して構築されるlist
		...
		public SongsManager()
		{
			this.listSongsDB = new List<ScoreInfo>();
			this.list曲ルート = new List<SongInfoNode>();
			this.n検索された曲ノード数 = 0;
			this.n検索されたスコア数 = 0;
			...
			if( File.Exists( SongsDBファイル名 ) )
			{
				...
				try
				{
					...
					while( true )
					{
						try
						{
							ScoreInfo item = this.tSongsDBからスコアを１つ読み込む( br );
		...
		private void t曲を検索してリストを作成する( string str基点フォルダ, bool b子BOXへ再帰する, List<SongInfoNode> listノードリスト, SongInfoNode node親 )
		{
				...
				try
				{
                    foreach( FileInfo fileinfo in info.GetFiles() )
                    {
                        ...
                        if( ( strExt.Equals( ".tja" ) || strExt.Equals( ".dtx" ) ) )
                        {
                            ...
                            #region[ 新処理 ]
                            Chart dtx = new Chart( fileinfo.FullName, false, 1.0, 0, 1 );
                            SongInfoNode c曲リストノード = new SongInfoNode();
                            c曲リストノード.NowNodeType = SongInfoNode.NodeType.SCORE;
                            ...
                            for( int n = 0; n < (int)Difficulty.Total; n++ )
                            {
                                if( dtx.b譜面が存在する[ n ] )
                                {
                                    ...
                                    c曲リストノード.arスコア[ n ] = new ScoreInfo();
            ...
            else
			{
				foreach( FileInfo fileinfo in info.GetFiles() )
				{
                    ...
                    if( ( strExt.Equals( ".tja" ) || strExt.Equals( ".dtx" ) ) )
                    {
                        ...
                        #region[ 新処理 ]
                        Chart dtx = new Chart( str基点フォルダ + fileinfo.Name, false, 1.0, 0, 0 );
                        SongInfoNode c曲リストノード = new SongInfoNode();
                        c曲リストノード.NowNodeType = SongInfoNode.NodeType.SCORE;
			...
			foreach( DirectoryInfo infoDir in info.GetDirectories() )
			{
				...
				{
					SongInfoNode c曲リストノード = new SongInfoNode();
					c曲リストノード.NowNodeType = SongInfoNode.NodeType.BOX;
					...
					c曲リストノード.list子リスト = new List<SongInfoNode>();
					c曲リストノード.arスコア[ 0 ] = new ScoreInfo();
					...
					{
						BoxDef boxdef = new BoxDef( infoDir.FullName + @"\box.def" );
		...
		private void tスコアキャッシュを曲リストに反映する( List<SongInfoNode> ノードリスト )
		{
			using( List<SongInfoNode>.Enumerator enumerator = ノードリスト.GetEnumerator() )
			{
				while( enumerator.MoveNext() )
				{
					SlowOrSuspendSearchTask();		// #27060 中断要求があったら、解除要求が来るまで待機, #PREMOVIE再生中は検索負荷を落とす

					SongInfoNode node = enumerator.Current;
					if( node.NowNodeType == SongInfoNode.NodeType.BOX )
					{
						this.tスコアキャッシュを曲リストに反映する( node.list子リスト );
					}
					else if( ( node.NowNodeType == SongInfoNode.NodeType.SCORE ) || ( node.NowNodeType == SongInfoNode.NodeType.SCORE_MIDI ) )
					{
						Predicate<ScoreInfo> match = null;
...
//Trace.TraceInformation( "songs.db に存在しません。({0})", node.arスコア[ lv ].ファイル情報.ファイルの絶対パス );
		...
		private ScoreInfo tSongsDBからスコアを１つ読み込む( BinaryReader br )
		{
			ScoreInfo cスコア = new ScoreInfo();
			...
			cスコア.譜面情報.曲種別 = (Chart.E種別) br.ReadInt32();
		...
		private void tSongsDBになかった曲をファイルから読み込んで反映する( List<SongInfoNode> ノードリスト )
		{
			foreach( SongInfoNode c曲リストノード in ノードリスト )
			{
				...
				else if( ( c曲リストノード.NowNodeType == SongInfoNode.NodeType.SCORE )
					  || ( c曲リストノード.NowNodeType == SongInfoNode.NodeType.SCORE_MIDI ) )
				{
					for( int i = 0; i < (int)Difficulty.Total; i++ )
					{
						if( ( c曲リストノード.arスコア[ i ] != null ) && !c曲リストノード.arスコア[ i ].bSongDBにキャッシュがあった )
						{
							...
							if( File.Exists( path ) )
							{
								try
								{
									Chart cdtx = new Chart( c曲リストノード.arスコア[ i ].ファイル情報.ファイルの絶対パス, true, 0, 0, 0 );
                                    if( File.Exists( c曲リストノード.arスコア[ i ].ファイル情報.フォルダの絶対パス + "set.def" ) )
									    cdtx = new Chart( c曲リストノード.arスコア[ i ].ファイル情報.ファイルの絶対パス, true, 0, 0, 1 );
...
//Debug.WriteLine( "★" + this.nファイルから反映できたスコア数 + " " + c曲リストノード.arスコア[ i ].譜面情報.タイトル );
		...
		public void t曲リストへ後処理を適用する()
		{
			...
			foreach ( string boxdefSkinSubfolderFullName in l )
			{
				if ( Array.BinarySearch( systemSkinNames,
					SkinManager.GetSkinName( boxdefSkinSubfolderFullName ),
		...
		private void t曲リストへ後処理を適用する( List<SongInfoNode> ノードリスト )
		{
			...
			#region [ ノードをソートする ]
			//-----------------------------
            if( TJAPlayer3._MainConfig.nDefaultSongSort == 0 )
            {
			    t曲リストのソート1_絶対パス順( ノードリスト );
            }
            else if( TJAPlayer3._MainConfig.nDefaultSongSort == 1 )
            {
                t曲リストのソート9_ジャンル順( ノードリスト, E楽器パート.TAIKO, 1, 0 );
            }
            else if( TJAPlayer3._MainConfig.nDefaultSongSort == 2 )
            {
		...
		private void tSongsDBにノードを１つ出力する( BinaryWriter bw, SongInfoNode node )
		{
			for( int i = 0; i < (int)Difficulty.Total; i++ )
			{
		...
		private void tSongsDBにリストを１つ出力する( BinaryWriter bw, List<SongInfoNode> list )
		{
			foreach( SongInfoNode c曲リストノード in list )
			{
				if(    ( c曲リストノード.NowNodeType == SongInfoNode.NodeType.SCORE )
					|| ( c曲リストノード.NowNodeType == SongInfoNode.NodeType.SCORE_MIDI ) )
				{
	    ...
	    public static void t曲リストのソート1_絶対パス順( List<SongInfoNode> ノードリスト )
	    {
	    ...
	    public static void t曲リストのソート1_絶対パス順( List<SongInfoNode> ノードリスト, E楽器パート part, int order, params object[] p )
	    {
            var comparer = new ComparerChain<SongInfoNode>(
                new ComparerNodeType(),
                new ComparerAbsolutePath(order),
                new ComparerTitle(order));

	        ノードリスト.Sort( comparer );
	    }

	    public static void t曲リストのソート2_タイトル順( List<SongInfoNode> ノードリスト, E楽器パート part, int order, params object[] p )
	    {
	        var comparer = new ComparerChain<SongInfoNode>(
	            new ComparerNodeType(),
	            new ComparerTitle(order),
		...
		public static void t曲リストのソート3_演奏回数の多い順( List<SongInfoNode> ノードリスト, E楽器パート part, int order, params object[] p )
		{
			order = -order;
			int nL12345 = (int) p[ 0 ];
			if ( part != E楽器パート.UNKNOWN )
			{
				ノードリスト.Sort( delegate( SongInfoNode n1, SongInfoNode n2 )
				{
					#region [ 共通処理 ]
...
// Debug.WriteLine( nSumPlayCountN1 + ":" + c曲リストノード.strタイトル );
		...
		public static void t曲リストのソート4_LEVEL順( List<SongInfoNode> ノードリスト, E楽器パート part, int order, params object[] p )
		{
			order = -order;
			int nL12345 = (int)p[ 0 ];
			if ( part != E楽器パート.UNKNOWN )
...
// Debug.WriteLine( nSumPlayCountN1 + ":" + c曲リストノード.strタイトル );
		...
		public static void t曲リストのソート5_BestRank順( List<SongInfoNode> ノードリスト, E楽器パート part, int order, params object[] p )
		{
			order = -order;
			int nL12345 = (int) p[ 0 ];
			if ( part != E楽器パート.UNKNOWN )
...
// Debug.WriteLine( nSumPlayCountN1 + ":" + c曲リストノード.strタイトル );
		...
		public static void t曲リストのソート6_SkillPoint順( List<SongInfoNode> ノードリスト, E楽器パート part, int order, params object[] p )
		{
			order = -order;
			int nL12345 = (int) p[ 0 ];
			if ( part != E楽器パート.UNKNOWN )
...
// Debug.WriteLine( nSumPlayCountN1 + ":" + c曲リストノード.strタイトル );
		...
		public static void t曲リストのソート7_更新日時順( List<SongInfoNode> ノードリスト, E楽器パート part, int order, params object[] p )
		{
			int nL12345 = (int) p[ 0 ];
			if ( part != E楽器パート.UNKNOWN )
			{
...
// Debug.WriteLine( nSumPlayCountN1 + ":" + c曲リストノード.strタイトル );
		...
		public static void t曲リストのソート8_アーティスト名順( List<SongInfoNode> ノードリスト, E楽器パート part, int order, params object[] p )
		{
			int nL12345 = (int) p[ 0 ]; 
			ノードリスト.Sort( delegate( SongInfoNode n1, SongInfoNode n2 )
			{
...
Debug.WriteLine( s + ":" + c曲リストノード.strタイトル );
	    ...
	    public static void t曲リストのソート9_ジャンル順(List<SongInfoNode> ノードリスト, E楽器パート part, int order, params object[] p)
	    {
	        try
	        {
	            var acGenreComparer = order == 1
	                ? (IComparer<SongInfoNode>) new ComparerAC8_14()
	                : new ComparerAC15();

	            var comparer = new ComparerChain<SongInfoNode>(
	                new ComparerNodeType(),
	                acGenreComparer,
	                new ComparerAbsolutePath(1),
	                new ComparerTitle(1));
...
Debug.WriteLine( dBPM + ":" + c曲リストノード.strタイトル );
        ...
        //-----------------
        public void tScoreIniを読み込んで譜面情報を設定する( string strScoreIniファイルパス, ScoreInfo score )
		{
			...
			try
			{
				var ini = new ScoreIni( strScoreIniファイルパス );
				...
				for( int n楽器番号 = 0; n楽器番号 < 3; n楽器番号++ )
				{
					...
					else
					{
						...
						score.譜面情報.最大ランク[ n楽器番号 ] = (int) ScoreIni.ERANK.UNKNOWN;
					}
					//-----------------
					#endregion
