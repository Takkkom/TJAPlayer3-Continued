    ...
    internal class Chart : Activity
    {
        ...
        public class CAVI : IDisposable
        {
            ...
            public void OnDeviceCreated()
            {
                ...
                try
                {
                    this.avi = new FDKAvi(strAVIファイル名);
            ...
            {
                return string.Format("CAVI{0}: File:{1}, Comment:{2}", Chart.tZZ(this.n番号), this.strファイル名, this.strコメント文);
            }
        ...
        public class CAVIPAN
        {
            ...
            public override string ToString()
            {
                return string.Format("CAVIPAN{0}: AVI:{14}, 開始サイズ:{1}x{2}, 終了サイズ:{3}x{4}, 動画側開始位置:{5}x{6}, 動画側終了位置:{7}x{8}, 表示側開始位置:{9}x{10}, 表示側終了位置:{11}x{12}, 移動時間:{13}ct",
                    ...
                    this.n移動時間ct,
                    Chart.tZZ(this.nAVI番号));
            }
        }
        public class CDirectShow : IDisposable
        {
            public FDK.DirectShow dshow;
            private bool bDispose済み;
            public int n番号;
            public string strコメント文 = "";
            public string strファイル名 = "";
            ...
            public void OnDeviceCreated()
            {
                ...
                try
                {
                    this.dshow = new FDK.DirectShow(TJAPlayer3.stage選曲.r確定されたスコア.ファイル情報.フォルダの絶対パス + this.strファイル名, TJAPlayer3.app.WindowHandle, true);
        ...
        public class CBPM
        {
            ...
            public override string ToString()
            {
                ...
                {
                    builder.Append(string.Format("CBPM{0}(内部{1})", Chart.tZZ(this.n表記上の番号), this.n内部番号));
                }
                else
                {
                    builder.Append(string.Format("CBPM{0}", Chart.tZZ(this.n表記上の番号)));
                }
        ...
        public class CSCROLL
        {
            ...
            public override string ToString()
            {
                ...
                if (this.n内部番号 != this.n表記上の番号)
                {
                    builder.Append(string.Format("CSCROLL{0}(内部{1})", Chart.tZZ(this.n表記上の番号), this.n内部番号));
                }
                else
                {
                    builder.Append(string.Format("CSCROLL{0}", Chart.tZZ(this.n表記上の番号)));
                }
		...
		public class CJPOSSCROLL
        {
            ...
            public override string ToString()
            {
                ...
                if (this.n内部番号 != this.n表記上の番号)
                {
                    builder.Append(string.Format("CJPOSSCROLL{0}(内部{1})", Chart.tZZ(this.n表記上の番号), this.n内部番号));
                }
                else
                {
                    builder.Append(string.Format("CJPOSSCROLL{0}", Chart.tZZ(this.n表記上の番号)));
                }
        ...
        public class CDELAY
        {
            ...
            public override string ToString()
            {
                ...
                if (this.n内部番号 != this.n表記上の番号)
                {
                    builder.Append(string.Format("CDELAY{0}(内部{1})", Chart.tZZ(this.n表記上の番号), this.n内部番号));
                }
                else
                {
                    builder.Append(string.Format("CDELAY{0}", Chart.tZZ(this.n表記上の番号)));
                }
        ...
        public class CBRANCH
        {
            ...
            public override string ToString()
            {
                ...
                if (this.n内部番号 != this.n表記上の番号)
                {
                    builder.Append(string.Format("CBRANCH{0}(内部{1})", Chart.tZZ(this.n表記上の番号), this.n内部番号));
                }
                else
                {
                    builder.Append(string.Format("CBRANCH{0}", Chart.tZZ(this.n表記上の番号)));
                }
        ...
        public class CChip : IComparable<Chart.CChip>, ICloneable
        {
            public bool bHit;
            ...
            public Chart.CAVI rAVI;
            public Chart.CAVIPAN rAVIPan;
            public Chart.CDirectShow rDShow;
            ...
            public Counter RollDelay; // 18.9.22 AioiLight Add 連打時に赤くなるやつのタイマー
            public Counter RollInputTime; // 18.9.22 AioiLight Add  連打入力後、RollDelayが作動するまでのタイマー
            ...
            public int GetDuration()
            {
                ...
                if (this.bWAVを使うチャンネルである)       // WAV
                {
                    ...
                    {
                        nDuration = (wc.rSound[0] == null) ? 0 : wc.rSound[0].TotalPlayTimeMs;
                    }
                }
            ...
            public int CompareTo(Chart.CChip other)
            {
                // まずは位置で比較。
        ...
        public class CWAV : IDisposable
        {
            ...
            public int n位置;
            public long[] n一時停止時刻 = new long[TJAPlayer3._MainConfig.nPoliphonicSounds];    // 4
            public int SongVol = FDKSound.DefaultSongVol;
            public LoudnessMetadata? SongLoudnessMetadata = null;
            public int n現在再生中のサウンド番号;
            public long[] n再生開始時刻 = new long[TJAPlayer3._MainConfig.nPoliphonicSounds];    // 4
            public int n内部番号;
            public int n表記上の番号;
            public FDKSound[] rSound = new FDKSound[TJAPlayer3._MainConfig.nPoliphonicSounds];     // 4
            public string strコメント文 = "";
            public string strファイル名 = "";
            ...
            public override string ToString()
            {
                ...
                {
                    sb.Append(string.Format("CWAV{0}: ", Chart.tZZ(this.n表記上の番号)));
                }
                else
                {
                    sb.Append(string.Format("CWAV{0}(内部{1}): ", Chart.tZZ(this.n表記上の番号), this.n内部番号));
                }
                sb.Append(
            ...
            public void Dispose(bool bManagedリソースの解放も行う)
            {
                ...
                {
                    for (int i = 0; i < TJAPlayer3._MainConfig.nPoliphonicSounds; i++) // 4
                    {
                        if (this.rSound[i] != null)
                            TJAPlayer3._SoundManager.DisposeSound(this.rSound[i]);
                        this.rSound[i] = null;

                        if ((i == 0) && TJAPlayer3._MainConfig.bLog作成解放ログ出力)
        ...
        {
            public FDKTexture TitleTex;
            public FDKTexture SubTitleTex;
            public string Title;
...
//		public bool bBASS演奏で直前のBASSを消音する;
        ...
        public Chart()
        {
            ...
            this.SongVol = FDKSound.DefaultSongVol;
        ...
        public Chart(string str全入力文字列)
            : this()
        {
            this.Activate();
            this.t入力_全入力文字列から(str全入力文字列);
        }
        public Chart(string strファイル名, bool bヘッダのみ)
            : this()
        {
            this.Activate();
            this.t入力(strファイル名, bヘッダのみ);
        }
        public Chart(string str全入力文字列, double db再生速度, int nBGMAdjust)
            : this()
        {
            this.Activate();
            this.t入力_全入力文字列から(str全入力文字列, str全入力文字列, db再生速度, nBGMAdjust);
        }
        public Chart(string strファイル名, bool bヘッダのみ, double db再生速度, int nBGMAdjust)
            : this()
        {
            this.Activate();
            this.t入力(strファイル名, bヘッダのみ, db再生速度, nBGMAdjust, 0, 0, false);
        }
        public Chart(string strファイル名, bool bヘッダのみ, double db再生速度, int nBGMAdjust, int nReadVersion)
            : this()
        {
            this.Activate();
            this.t入力(strファイル名, bヘッダのみ, db再生速度, nBGMAdjust, nReadVersion, 0, false);
        }
        public Chart(string strファイル名, bool bヘッダのみ, double db再生速度, int nBGMAdjust, int nReadVersion, int nPlayerSide, bool bSession)
            : this()
        {
            this.Activate();
        ...
        {
            if (wc.rSound[0] != null && wc.rSound[0].TotalPlayTimeMs >= 5000)
            {
                for (int i = 0; i < nPolyphonicSounds; i++)
                {
                    if ((wc.rSound[i] != null) && (wc.rSound[i].b再生中))
                    {
                        ...
                        if (nCurrentTime > wc.n再生開始時刻[i])
                        {
                            ...
                            wc.rSound[i].SetPosition(nAbsTimeFromStartPlaying);  // WASAPI/ASIO用
                        }
                    }
                }
            }
            ...
            if (this.listWAV.TryGetValue(nWaveの内部番号, out CWAV cwav))
            {
                for (int i = 0; i < nPolyphonicSounds; i++)
                {
                    if (cwav.rSound[i] != null && cwav.rSound[i].b再生中)
                    {
                        if (bミキサーからも削除する)
                        {
                            cwav.rSound[i].StopSoundAndRemoveMixer();
                        }
                        else
                        {
        ...
        public void tWAVの読み込み(CWAV cwav)
        {
            ...
            try
            {
                ...
                if (TJAPlayer3._SoundManager.GetCurrentSoundDeviceType() != "DirectSound") // DShowでの再生の場合はミキシング負荷が高くないため、
                {
                    // チップのライフタイム管理を行わない
                ...
                for (int i = 0; i < nPoly; i++)
                {
                    try
                    {
                        cwav.rSound[i] = TJAPlayer3._SoundManager.CreateFDKSound(str, SoundGroup.SongPlayback);

                        if (!TJAPlayer3._MainConfig.bDynamicBassMixerManagement)
                        {
                            cwav.rSound[i].ADDBassSoundToTheMixer();
                        }

                        if (TJAPlayer3._MainConfig.bLog作成解放ログ出力)
                        {
        ...
        public void tチップの再生(CChip pChip, long n再生開始システム時刻ms, int nLane)
        {
            if (pChip.n整数値_内部番号 >= 0)
            {
                ...
                if (this.listWAV.TryGetValue(pChip.n整数値_内部番号, out CWAV wc))
                {
                    ...
                    FDKSound sound = wc.rSound[index];
                    if (sound != null)
                    {
                        sound.db周波数倍率 = 1.0;
                        sound.PlaySpped = ((double)TJAPlayer3._MainConfig.n演奏速度) / 20.0;
        ...
        private void t入力_V4(string strInput)
        {
            if (!String.IsNullOrEmpty(strInput)) //空なら通さない
            {
                ...
                #region[ 読み込ませるコースを決定 ]
                if (this.b譜面が存在する[TJAPlayer3.stage選曲.n確定された曲の難易度[nPlayerSide]] == false)
                {
                ...
                //指定したコースの譜面の命令を消去する。
                strSplitした譜面[n読み込むコース] = ChartStyleExtractor.tセッション譜面がある(
                    strSplitした譜面[n読み込むコース],
                    TJAPlayer3._MainConfig.nPlayerCount > 1 ? (this.nPlayerSide + 1) : 0,
                    this.strファイル名の絶対パス);
        ...
        private void t命令を挿入する(string InputText)
        {
            ...
            else if (command == "#NEXTSONG")
            {
                ...
                dansongs.Wave = new CWAV
                {
                    ...
                    strファイル名 = ChartCompanionFileFinder.FindFileName(this.strフォルダ名, strファイル名, dansongs.FileName),
                    strコメント文 = "TJA BGM"
                };
        ...
        {
            if (InputText.Equals("#HBSCROLL") && TJAPlayer3._MainConfig.bスクロールモードを上書き == false)
            {
                TJAPlayer3._MainConfig.eScrollMode = EScrollMode.HBSCROLL;
            }
            if (InputText.Equals("#BMSCROLL") && TJAPlayer3._MainConfig.bスクロールモードを上書き == false)
            {
            ...
            if (this.nScoreModeTmp == 99) //2017.01.28 DD SCOREMODEを入力していない場合のみConfigで設定したモードにする
            {
                this.nScoreModeTmp = TJAPlayer3._MainConfig.nScoreMode;
            }
            if (TJAPlayer3._MainConfig.nScoreMode == 3 && !this.b配点が指定されている[2, this.n参照中の難易度])
        ...
        private void t入力_行解析ヘッダ(string InputText)
        {
            ...
            else if (strCommandName.Equals("WAVE"))
            {
                if (strBGM_PATH != null)
                {
                    Trace.TraceWarning($"{nameof(Chart)} is ignoring an extra WAVE header in {this.strファイル名の絶対パス}");
                }
                else
                {
                    this.strBGM_PATH = ChartCompanionFileFinder.FindFileName(this.strフォルダ名, strファイル名, strCommandParam);
            ...
            {
                this.SongVol = Convert.ToInt32(strCommandParam).Clamp(FDKSound.MinimumSongVol, FDKSound.MaximumSongVol);
            ...
            else if (strCommandName.Equals("BGMOVIE"))
            {
                ...
                if (!string.IsNullOrEmpty(strCommandParam))
                {
                    this.strBGVIDEO_PATH =
                        ChartCompanionFileFinder.FindFileName(this.strフォルダ名, strファイル名, strCommandParam);
                }
        ...
        private void tSetSenotes_branch()
        {
            ...
            //ひとまずチップだけのリストを作成しておく。
            List<Chart.CChip> list音符のみのリスト;
            List<Chart.CChip> list普通譜面のみのリスト;
            List<Chart.CChip> list玄人譜面のみのリスト;
            List<Chart.CChip> list達人譜面のみのリスト;
            list音符のみのリスト = new List<CChip>();
            ...
            foreach (CChip pChip in listChip)
            {
                switch (pChip.nチャンネル番号)
                {
                    ...
                    case 0x01:
                        ...
                        {
                            double _db再生速度 = (TJAPlayer3.DTXVmode.Enabled) ? this.dbDTXVPlaySpeed : this.db再生速度;
                            duration = (wc.rSound[0] == null) ? 0 : (int)(wc.rSound[0].TotalPlayTimeMs / _db再生速度); // #23664 durationに再生速度が加味されておらず、低速再生でBGMが途切れる問題を修正 (発声時刻msは、DTX読み込み時に再生速度加味済)
                        }
        ...
        private bool t入力_行解析_AVI_VIDEO(string strコマンド, string strパラメータ, string strコメント)
        {
            ...
            //-----------------
            int zz = ConvertUtility.Convert2DigitBase36StringToNumber(strコマンド.Substring(0, 2));
            if (zz < 0 || zz >= 36 * 36)
            {
        ...
        private bool t入力_行解析_AVIPAN(string strコマンド, string strパラメータ, string strコメント)
        {
            ...
            avipan.nAVI番号 = ConvertUtility.Convert2DigitBase36StringToNumber(strParams[i]);
        ...
        private bool t入力_行解析_BPM_BPMzz(string strコマンド, string strパラメータ, string strコメント)
        {
            ...
            else
            {
                ...
                //-----------------
                zz = ConvertUtility.Convert2DigitBase36StringToNumber(strコマンド.Substring(0, 2));
                if (zz < 0 || zz >= 36 * 36)
                {
        ...
        private bool t入力_行解析_SIZE(string strコマンド, string strパラメータ, string strコメント)
        {
            ...
            //-----------------
            int nWAV番号 = ConvertUtility.Convert2DigitBase36StringToNumber(strコマンド.Substring(0, 2));
        ...
        private bool t入力_行解析_チップ配置(string strコマンド, string strパラメータ, string strコメント)
        {
            ...
            #region [ n小節番号 を取得する。]
            //-----------------
            int n小節番号 = ConvertUtility.Convert3DigitMeasureNumberToNumber(strコマンド.Substring(0, 3));
            if (n小節番号 < 0)
                return false;
            ...
            else
            {
                ...
                //-----------------
                nチャンネル番号 = ConvertUtility.Convert2DigitHexadecimalStringToNumber(strコマンド.Substring(3, 2));
            ...
            while (ce.MoveNext())
            {
                ...
                if (ConvertUtility.Base36Characters.IndexOf(ce.Current) < 0)  // オブジェクト記述は36進数文字であること。
                {
            ...
            for (int i = 0; i < (n文字数 / 2); i++)    // 2文字で1オブジェクト数値
            {
                ...
                {
                    // Ch.03 のみ 16進数2桁。
                    nオブジェクト数値 = ConvertUtility.Convert2DigitHexadecimalStringToNumber(strパラメータ.Substring(i * 2, 2));
                }
                else
                {
                    // その他のチャンネルは36進数2桁。
                    nオブジェクト数値 = ConvertUtility.Convert2DigitBase36StringToNumber(strパラメータ.Substring(i * 2, 2));
                }
        ...
        {
            this.dbNowTime += TJAPlayer3._MainConfig.MusicPreTimeMs;
            this.dbNowBMScollTime += TJAPlayer3._MainConfig.MusicPreTimeMs * this.dbNowBPM / 15000;
        }
        //-----------------
        #endregion
    }
