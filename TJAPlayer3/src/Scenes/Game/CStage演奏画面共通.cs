...
//		Debug.WriteLine( "★Queue: remove " + Path.GetFileName( stm.csound.strファイル名 ));
		...
		private E判定 e指定時刻からChipのJUDGEを返すImpl( long nTime, CDTX.CChip pChip )
		{
			if ( pChip != null )
			{
                ...
                {
				    if ( CSound管理.rc演奏用タイマ.NowTimeMs > pChip.n発声時刻ms && CSound管理.rc演奏用タイマ.NowTimeMs < pChip.nノーツ終了時刻ms )
				    {
					    return E判定.Perfect;
				    }
                }
                ...
                {
				    if ( CSound管理.rc演奏用タイマ.NowTimeMs >= pChip.n発声時刻ms - 17 && CSound管理.rc演奏用タイマ.NowTimeMs < pChip.nノーツ終了時刻ms )
				    {
					    return E判定.Perfect;
				    }
                }
...
//Trace.TraceInformation( "NTime={0}, nChannel={1:x2}", nTime, nChannel );
        ...
        {
            //if( dbProcess_time >= pChip.n発声時刻ms && dbProcess_time < pChip.nノーツ終了時刻ms )
            if ((int)CSound管理.rc演奏用タイマ.NowTimeMs >= pChip.n発声時刻ms && (int)CSound管理.rc演奏用タイマ.NowTimeMs <= pChip.nノーツ終了時刻ms)
            {
                if( pChip.nRollCount == 0 )
                {
				...
				case E楽器パート.TAIKO:
					{
                        ...
                        if ( pChip.nチャンネル番号 == 0x15 || pChip.nチャンネル番号 == 0x16 )
                        {
                            ...
                            if( bAutoPlay )
                            {
                                ...
                                {
                                    if( CSound管理.rc演奏用タイマ.NowTimeMs > ( pChip.n発声時刻ms + ( 1000.0 / 15.0 ) * pChip.nRollCount ) )
                                    {
                                        if( this.nHand[ nPlayer ] == 0 )
                                            this.nHand[ nPlayer ]++;
                                        else
                                        ...
                                        this.tRollProcess( pChip, CSound管理.rc演奏用タイマ.NowTimeMs, 1, 0, 0, nPlayer );
                                    }
                                }
                            }
                            else
                            {
                                this.eRollState = E連打State.roll;
                                this.tRollProcess( pChip, CSound管理.rc演奏用タイマ.NowTimeMs, 1, nNowInput, 0, nPlayer );
                            }
                        ...
                        else if( pChip.nチャンネル番号 == 0x17 )
                        {
                            ...
                            if( bAutoPlay )
                            {
                                if( pChip.nBalloon != 0 )
                                {
                                    if( CSound管理.rc演奏用タイマ.NowTimeMs > ( pChip.n発声時刻ms + ( ( pChip.nノーツ終了時刻ms - pChip.n発声時刻ms ) / pChip.nBalloon ) * pChip.nRollCount ) )
                                    {
                                        if( this.nHand[ nPlayer ] == 0 )
                                            this.nHand[ nPlayer ]++;
                                        else
                                        ...
                                        this.tBalloonProcess( pChip, CSound管理.rc演奏用タイマ.NowTimeMs, nPlayer );
                                    }
                                }
                            }
                            else
                        ...
                        {
                            if( pChip.nノーツ終了時刻ms <= CSound管理.rc演奏用タイマ.NowTimeMs )
                            {
                                this.b連打中[ nPlayer ] = false;
                                //this.actChara.b風船連打中 = false;
                                pChip.bHit = true;
            ...
            if ( eJudgeResult != E判定.Poor && eJudgeResult != E判定.Miss )
            {
                ...
                if ((int)actGauge.db現在のゲージ値[nPlayer] >= 100 && this.bIsAlreadyMaxed[nPlayer] == false)
                {
                    if(TJAPlayer3.Skin.Game_Chara_Ptn_SoulIn != 0 && nPlayer == 0 && actChara.CharaAction_Balloon_Delay.IsEndValueReached)
                    {
                        ...
                        this.actChara.ctキャラクターアクション_魂MAX = new Counter(0, TJAPlayer3.Skin.Game_Chara_Ptn_SoulIn - 1, (dbUnit / TJAPlayer3.Skin.Game_Chara_Ptn_SoulIn) * 2, CSound管理.rc演奏用タイマ);
                ...
                if ((int)actGauge.db現在のゲージ値[nPlayer] >= 80 && this.bIsAlreadyCleared[nPlayer] == false)
                {
                    ...
                    {
                        this.actChara.アクションタイマーリセット();
                        this.actChara.ctキャラクターアクション_ノルマ = new Counter(0, TJAPlayer3.Skin.Game_Chara_Ptn_ClearIn - 1, (dbUnit / TJAPlayer3.Skin.Game_Chara_Ptn_ClearIn) * 2, CSound管理.rc演奏用タイマ);
                                ...
                                if (!pChip.bGOGOTIME) //2018.03.11 kairera0467 チップに埋め込んだフラグから読み取る
                                {
                                    if (TJAPlayer3.Skin.Game_Chara_Ptn_10combo != 0 && nPlayer == 0 && !this.actChara.ctキャラクターアクション_ノルマ.IsProcessed_Double && actChara.CharaAction_Balloon_Delay.IsEndValueReached)
                                    {
                                        if (TJAPlayer3.stage演奏ドラム画面.actGauge.db現在のゲージ値[0] < 100)
                                        {
                                            ...
                                            // ジャンプ_ノーマル
                                            this.actChara.アクションタイマーリセット();
                                            this.actChara.ctキャラクターアクション_10コンボ = new Counter(0, TJAPlayer3.Skin.Game_Chara_Ptn_10combo - 1, (dbUnit / TJAPlayer3.Skin.Game_Chara_Ptn_10combo) * 2, CSound管理.rc演奏用タイマ);
...
#region [ Minus & Equals Sound Group Level ]
		...
		{
			if ( ( ( base.eフェーズID != CStage.Eフェーズ.演奏_STAGE_FAILED ) && ( base.eフェーズID != CStage.Eフェーズ.演奏_STAGE_FAILED_フェードアウト ) ) && ( !TJAPlayer3._MainConfig.bストイックモード && TJAPlayer3._MainConfig.bAVI有効 ) )
			{
		...
		{
			if ( ( ( base.eフェーズID == CStage.Eフェーズ.演奏_STAGE_FAILED ) || ( base.eフェーズID == CStage.Eフェーズ.演奏_STAGE_FAILED_フェードアウト ) ) && ( ( this.actStageFailed.Draw() != 0 ) && ( base.eフェーズID != CStage.Eフェーズ.演奏_STAGE_FAILED_フェードアウト ) ) )
			{
		...
		protected bool t進行描画_チップ( E楽器パート ePlayMode, int nPlayer )
		{
			...
			var n現在時刻ms = CSound管理.rc演奏用タイマ.NowTimeMs;
					...
					case 0x50:	// 小節線
						{
                            if ( !pChip.bHit && ( pChip.nバーからの距離dot.Taiko < 0 ) )
						    {
                                ...
                                if( this.actPlayInfo.NowMeasure[nPlayer] == 0 )
                                {
                                    for (int i = 0; i < 2; i++)
                                    {
                                        ctChipAnime[i] = new Counter(0, 3, 60.0 / TJAPlayer3.stage演奏ドラム画面.actPlayInfo.dbBPM * 1 / 4, CSound管理.rc演奏用タイマ);
                                    }
                                    ...
                                    this.ct制御タイマ = new Counter(0, 10, 500, CSound管理.rc演奏用タイマ);
                                    ...
                                    {
                                        this.actMob.ctMob = new Counter(1, 180, 60.0 / TJAPlayer3.stage演奏ドラム画面.actPlayInfo.dbBPM * TJAPlayer3.Skin.Game_Mob_Beat / 180, CSound管理.rc演奏用タイマ);
                                        this.actMob.ctMobPtn = new Counter(0, TJAPlayer3.Skin.Game_Mob_Ptn - 1, 60.0 / TJAPlayer3.stage演奏ドラム画面.actPlayInfo.dbBPM * TJAPlayer3.Skin.Game_Mob_Ptn_Beat / TJAPlayer3.Skin.Game_Mob_Ptn, CSound管理.rc演奏用タイマ);
                                    }
                                    else
                                    {
                            ...
                            if(nPlayer == 0)
                            {
                                if (TJAPlayer3.Skin.Game_Chara_Ptn_GoGoStart != 0 && nPlayer == 0 && actChara.CharaAction_Balloon_Delay.IsEndValueReached)
                                {
                                    if (TJAPlayer3.stage演奏ドラム画面.actGauge.db現在のゲージ値[0] < 100)
                                    {
                                        ...
                                        // ゴーゴースタート_ノーマル
                                        this.actChara.アクションタイマーリセット();
                                        this.actChara.ctキャラクターアクション_ゴーゴースタート = new Counter(0, TJAPlayer3.Skin.Game_Chara_Ptn_GoGoStart - 1, (dbUnit / TJAPlayer3.Skin.Game_Chara_Ptn_GoGoStart) * 2, CSound管理.rc演奏用タイマ);
                                ...
                                if (TJAPlayer3.Skin.Game_Chara_Ptn_GoGoStart_Max != 0 && nPlayer == 0 && actChara.CharaAction_Balloon_Delay.IsEndValueReached)
                                {
                                    if (TJAPlayer3.stage演奏ドラム画面.actGauge.db現在のゲージ値[0] >= 100)
                                    {
                                        ...
                                        // ゴーゴースタート_MAX
                                        this.actChara.アクションタイマーリセット();
                                        this.actChara.ctキャラクターアクション_ゴーゴースタートMAX = new Counter(0, TJAPlayer3.Skin.Game_Chara_Ptn_GoGoStart_Max - 1, (dbUnit / TJAPlayer3.Skin.Game_Chara_Ptn_GoGoStart_Max) * 2, CSound管理.rc演奏用タイマ);
...
#region[ 15-19: 連打 ]
        ...
        public void t分岐処理( int n分岐先, int n分岐開始位置のChip番号, int nPlayer )
        {
            ...
            CDTX.CChip chipNoHit = this.r指定時刻に一番近い未ヒットChip( CSound管理.rc演奏用タイマ.NowTimeMs + 1, 0xDF, 0, 10000, nPlayer ); //とりあえず検索開始を1ms後にして対応。
            if( chipNoHit == null )
                return;
        ...
        protected float GetNowPBMTime( CDTX tja, float play_time )
        {
            ...
            play_time = CSound管理.rc演奏用タイマ.NowTimeMs - tja.nOFFSET;
...
#region [ 演奏開始の発声時刻msを取得し、タイマに設定 ]
			...
			CSound管理.rc演奏用タイマ.Reset();	// これでPAUSE解除されるので、次のPAUSEチェックは不要
			//if ( !this.bPAUSE )
			//{
...
#region [ タイマを再開して、PAUSEから復帰する ]
		...
		protected abstract void t進行描画_チップ_ドラムス( MainConfig configIni, ref CDTX dTX, ref CDTX.CChip pChip );
		protected abstract void t進行描画_チップ本体_ドラムス( MainConfig configIni, ref CDTX dTX, ref CDTX.CChip pChip );
		protected abstract void t進行描画_チップ_Taiko( MainConfig configIni, ref CDTX dTX, ref CDTX.CChip pChip, int nPlayer );
		protected abstract void t進行描画_チップ_Taiko連打( MainConfig configIni, ref CDTX dTX, ref CDTX.CChip pChip, int nPlayer );

		protected abstract void t進行描画_チップ_フィルイン( MainConfig configIni, ref CDTX dTX, ref CDTX.CChip pChip );
		protected abstract void t進行描画_チップ_小節線( MainConfig configIni, ref CDTX dTX, ref CDTX.CChip pChip, int nPlayer );
