...
{
	internal class CAct演奏スコア共通 : Activity
	{
		// プロパティ
  ...
  //      protected CTexture txScore_1P;
        protected Counter ctTimer;
        public Counter[] ct点数アニメタイマ;

        public Counter[] ctボーナス加算タイマ;
        ...
        private float[,] n点数アニメ拡大率_座標 = new float[,]
{
		...
		public void Add( E楽器パート part, bool autoPlay, long delta, int player )
		{
			double rev = 1.0;
			switch ( part )
			{
            ...
            this.ctTimer = new Counter( 0, 400, 1, TJAPlayer3.Timer );
            ...
            for( int sc = 0; sc < 1; sc++ )
            {
                for( int i = 0; i < 256; i++ )
                {
                    if( this.stScore[ i ].b使用中 == false )
                    {
                        ...
                        this.stScore[ i ].ctTimer = new Counter( 0, 500, 1, TJAPlayer3.Timer );
            ...
            for( int sc = 0; sc < 1; sc++ )
            {
                for( int i = 0; i < 256; i++ )
                {
                    if( this.stScore[ i ].b使用中 == false )
                    {
                        ...
                        this.stScore[ i ].ctTimer = new Counter( 0, 400, 1, TJAPlayer3.Timer );
		...
		public override void Activate()
		{
            ...
            for( int sc = 0; sc < 256; sc++ )
            {
                ...
                this.stScore[ sc ].ctTimer = new Counter();
            ...
            this.ctTimer = new Counter();

            this.ct点数アニメタイマ = new Counter[4];
            for (int i = 0; i < 4; i++)
            {
                this.ct点数アニメタイマ[i] = new Counter();
            }
            this.ctボーナス加算タイマ = new Counter[4];
            for (int i = 0; i < 4; i++)
            {
                this.ctボーナス加算タイマ[i] = new Counter();
            }
            base.Activate();
		}
                        ...
                        switch( mode )
                        {
                            case 0:
                                if( TJAPlayer3.Tx.Taiko_Score[0] != null )
                                {
                                    ...
                                    TJAPlayer3.Tx.Taiko_Score[0].Scaling.Y = ScoreScale[this.ct点数アニメタイマ[player].NowValue];
                            ...
                            case 1:
                                if(TJAPlayer3.Tx.Taiko_Score[1] != null )
                                {
                                    ...
                                    TJAPlayer3.Tx.Taiko_Score[1].Scaling.Y = 1;
                            ...
                            case 2:
                                if(TJAPlayer3.Tx.Taiko_Score[2] != null )
                                {
                                    ...
                                    TJAPlayer3.Tx.Taiko_Score[2].Scaling.Y = 1;
