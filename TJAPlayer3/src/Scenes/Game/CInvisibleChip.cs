...
namespace TJAPlayer3
{
	public class CInvisibleChip : IDisposable
	{
		...
		public void StartSemiInvisible( E楽器パート eInst )
		{
			...
			if ( !b演奏チップが１つでもバーを通過した[ nInst ] )
			{
				...
				if ( this.eInvisibleMode[ nInst ] == EInvisible.SEMI )
				{
					...
					ccounter[ nInst ].NowValue = nDisplayTimeMs;
				}
			}
		}
		/// <summary>
		...
		internal EChipInvisibleState SetInvisibleStatus( ref Chart.CChip cc )
		{
			...
			switch ( eInvisibleMode[ nInst ] )
			{
				...
				case EInvisible.SEMI:
					...
					if ( ccounter[ nInst ].NowValue <= 0 || ccounter[ nInst ].NowValue > nDisplayTimeMs + nFadeoutTimeMs )
					...
					else if ( ccounter[ nInst ].NowValue < nDisplayTimeMs )								// 表示期間
					{
						cc.b可視 = true;
						cc.n透明度 = 255;
						retcode = EChipInvisibleState.SHOW;
					}
					else if ( ccounter[ nInst ].NowValue < nDisplayTimeMs + nFadeoutTimeMs )		// フェードアウト期間
					{
						cc.b可視 = true;
						cc.n透明度 = 255 - (int) ( Convert.ToDouble( ccounter[ nInst ].NowValue - nDisplayTimeMs ) / nFadeoutTimeMs * 255.0 );
		...
		private STDGBVALUE<Counter> ccounter;
