...
{
	internal class CAct演奏AVI : Activity
	{
		// コンストラクタ
		...
		public void Start( int nチャンネル番号, Chart.CAVI rAVI, Chart.CDirectShow dsBGV, int n開始サイズW, int n開始サイズH, int n終了サイズW, int n終了サイズH, int n画像側開始位置X, int n画像側開始位置Y, int n画像側終了位置X, int n画像側終了位置Y, int n表示側開始位置X, int n表示側開始位置Y, int n表示側終了位置X, int n表示側終了位置Y, int n総移動時間ms, int n移動開始時刻ms )
		{
            if ( ( nチャンネル番号 == 0x54 || nチャンネル番号 == 0x5A ) && TJAPlayer3._MainConfig.bAVI有効 )
            {
                ...
                {
                    this.framewidth = (uint)this.dsBGV.dshow.Width;
                    this.frameheight = (uint)this.dsBGV.dshow.Height;
                    float f拡大率x;
                    float f拡大率y;
                    ...
                    if (this.tx描画用 == null)
                    {
                        this.tx描画用 = new FDKTexture(TJAPlayer3.app.Device, (int)this.framewidth, (int)this.frameheight, TJAPlayer3.app.GraphicsDeviceManager.CurrentSettings.BackBufferFormat, Pool.Managed);
                    }
                    ...
                    if (fAVIアスペクト比 < 1.77f)
                    {
                        ...
                        this.tx描画用.Scaling.X = this.ratio1;
                        this.tx描画用.Scaling.Y = this.ratio1;
                    }
                    else
                    {
                        //ワイドクリップの処理
		...
		public unsafe int t進行描画( int x, int y )
		{
			if ( !base.NotActivated )
			{
                ...
                if( this.dsBGV != null )
                {
                   nフレーム幅 = this.dsBGV.dshow.Width;
                   nフレーム高さ = this.dsBGV.dshow.Height;
                }
                ...
                if( ( this.tx描画用 != null ))
                {
                    if ( ( this.bDShowクリップを再生している == true ) && this.dsBGV.dshow != null )
                    {
                        ...
                        if( TJAPlayer3._MainConfig.eClipDispType == EClipDispType.背景のみ || TJAPlayer3._MainConfig.eClipDispType == EClipDispType.両方 )
                        {
                            if( this.dsBGV.dshow.IsFlipY )
                                this.tx描画用.VerticalFlipDraw2D( TJAPlayer3.app.Device, x, y );
                            else
                                this.tx描画用.Draw2D( TJAPlayer3.app.Device, x, y );
                        }
                        #endregion
                    }
                }
				else if ( ( this.tx描画用 != null ) && ( this.n総移動時間ms != -1 ) )
				{
					if ( this.bフレームを作成した && ( this.pBmp != IntPtr.Zero ) )
					{
						...
						int num14 = rectangle3.Pitch / this.tx描画用.TextureSize.Width;
						...
						if ( pBITMAPINFOHEADER->biBitCount == 0x18 )
						{
							...
							{
								case 2:
									this.rAVI.avi.tBitmap24ToGraphicsStreamR5G6B5( pBITMAPINFOHEADER, data, this.tx描画用.TextureSize.Width, this.tx描画用.TextureSize.Height );
									break;

								case 4:
									this.rAVI.avi.tBitmap24ToGraphicsStreamX8R8G8B8( pBITMAPINFOHEADER, data, this.tx描画用.TextureSize.Width, this.tx描画用.TextureSize.Height );
									break;
							}
						}
                    ...
                    //とりあえず16:9以外は再生しない。
                    if( dbAVI比率 < 1.77 )
					    this.tx描画用.Draw2D( TJAPlayer3.app.Device, 0, 0 );
				}
        ...
        public void t窓表示()
        {
            ...
            {
                #region[ ワイドクリップ ]
                float fRet = this.dsBGV.dshow.Width / this.dsBGV.dshow.Height;
                ...
                if( this.tx窓描画用 != null && fRet == 1.0 )
                {
                    ...
                    this.tx窓描画用.Scaling.X = (float)( fRatio[ 0 ] / this.dsBGV.dshow.Width );
                    this.tx窓描画用.Scaling.Y = (float)( fRatio[ 1 ] / this.dsBGV.dshow.Height );
                    if( this.dsBGV.dshow.IsFlipY )
                        this.tx窓描画用.VerticalFlipDraw2D( TJAPlayer3.app.Device, (int)fRatio[ 2 ], (int)fRatio[ 3 ] );
                    else
                        this.tx窓描画用.Draw2D( TJAPlayer3.app.Device, (int)fRatio[ 2 ], (int)fRatio[ 3 ] );
                }
        ...
        public void tReset()
        {
            ...
            if( this.tx描画用 != null && this.tx窓描画用 != null )
            {
				...
				this.tx描画用 = new FDKTexture( TJAPlayer3.app.Device, 1280, 720, TJAPlayer3.app.GraphicsDeviceManager.CurrentSettings.BackBufferFormat, Pool.Managed );
				this.tx窓描画用 = new FDKTexture( TJAPlayer3.app.Device, 1280, 720, TJAPlayer3.app.GraphicsDeviceManager.CurrentSettings.BackBufferFormat, Pool.Managed );
            }
