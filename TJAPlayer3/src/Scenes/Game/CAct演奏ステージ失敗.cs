...
{
	internal class CAct演奏ステージ失敗 : Activity
	{
		// コンストラクタ
		...
		public void Start()
		{
			...
			this.ct進行 = new Counter( 0, 1000, 2, TJAPlayer3.Timer );
            if( TJAPlayer3._MainConfig.eGameMode != EGame.OFF )
            {
			    this.ct進行 = new Counter( 0, 4000, 2, TJAPlayer3.Timer );
            }
		}
    ...
    //            CDTXMania.tテクスチャの解放( ref this.tx数字 );
		...
		public override int Draw()
		{
            ...
            if (TJAPlayer3._MainConfig.eGameMode == EGame.完走叩ききりまショー || TJAPlayer3._MainConfig.eGameMode == EGame.完走叩ききりまショー激辛)
            {
                ...
                if (this.ct進行.NowValue > 1500)
                {
                    if (TJAPlayer3.Tx.Failed_Game != null)
                        TJAPlayer3.Tx.Failed_Game.Draw2D(TJAPlayer3.app.Device, 0, 0);
            ...
            {
                if (this.ct進行.NowValue < 100)
                {
                    int x = (int)(640.0 * Math.Cos((Math.PI / 2 * this.ct進行.NowValue) / 100.0));
                    if ((x != 640) && (TJAPlayer3.Tx.Failed_Stage != null))
                    {
                        TJAPlayer3.Tx.Failed_Stage.Draw2D(TJAPlayer3.app.Device, 0, 0, new Rectangle(x, 0, 640 - x, 720));
                        TJAPlayer3.Tx.Failed_Stage.Draw2D(TJAPlayer3.app.Device, 640 + x, 0, new Rectangle(640, 0, 640 - x, 720));
                    }
                }
                else
                {
                    ...
                    {
                        TJAPlayer3.Tx.Failed_Stage.Draw2D(TJAPlayer3.app.Device, 0, 0);
                    }
                    if (this.ct進行.NowValue <= 250)
                    {
                        int num2 = TJAPlayer3.Random.Next(5) - 2;
                        int y = TJAPlayer3.Random.Next(5) - 2;
                        if (TJAPlayer3.Tx.Failed_Stage != null)
                        {
                            TJAPlayer3.Tx.Failed_Stage.Draw2D(TJAPlayer3.app.Device, num2, y);
                        }
                    }
                    if (!this.b効果音再生済み)
                    {
			...
			if( !this.ct進行.IsEndValueReached )
			{
				return 0;
			}
			return 1;
			...
			foreach( char ch in str )
			{
				for( int i = 0; i < this.st文字位置.Length; i++ )
				{
					if( this.st文字位置[ i ].ch == ch )
					{
						...
						{
                            TJAPlayer3.Tx.Balloon_Number_Roll.Draw2D( TJAPlayer3.app.Device, x - ( 62 * str.Length / 2 ), y, rectangle );
						}
						break;
					}
				}
