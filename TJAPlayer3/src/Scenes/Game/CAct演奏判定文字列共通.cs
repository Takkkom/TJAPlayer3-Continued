...
{
	internal class CAct演奏判定文字列共通 : Activity
	{
		// プロパティ
		...
		public virtual void Start( int nLane, E判定 judge, int lag, CDTX.CChip pChip, int player )
		{
			...
			if( ( (E判定文字表示位置) TJAPlayer3._MainConfig.判定文字表示位置.Drums ) != E判定文字表示位置.表示OFF )
			{
                for( int i = 0; i < 1; i++ )
                {
                    for( int j = 0; j < 12; j++ )
                    {
                        ...
                        {
				            this.st状態[ j ].ct進行 = new Counter( 0, 300, 1, TJAPlayer3.Timer );
