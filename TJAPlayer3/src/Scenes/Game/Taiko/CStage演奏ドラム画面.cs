...
namespace TJAPlayer3
{
	internal class CStage演奏ドラム画面 : GameScene
	{
		...
		public override void Activate()
		{
            ...
            {
                base.ReSetScore(TJAPlayer3.DTX.nScoreInit[0, TJAPlayer3.stage選曲.n確定された曲の難易度[player]], TJAPlayer3.DTX.nScoreDiff[TJAPlayer3.stage選曲.n確定された曲の難易度[player]], player);
            }
			base.Activate();
            ...
            this.ct手つなぎ = new Counter( 0, 60, 20, TJAPlayer3.Timer );
            ...
            Discord.UpdatePresence(TJAPlayer3._MainConfig.SendDiscordPlayingInformation ? TJAPlayer3.DTX.TITLE + ".tja" : "",
                Properties.Discord.Stage_InGame + (TJAPlayer3._MainConfig.b太鼓パートAutoPlay == true ? " (" + Properties.Discord.Info_IsAuto + ")" : ""),
                0,
                Discord.GetUnixTime() + (long)TJAPlayer3.DTX.listChip[TJAPlayer3.DTX.listChip.Count - 1].n発声時刻ms / 1000,
                TJAPlayer3._MainConfig.SendDiscordPlayingInformation ? difficultyName.ToLower() : "",
                TJAPlayer3._MainConfig.SendDiscordPlayingInformation ? String.Format("COURSE:{0} ({1})", difficultyName, TJAPlayer3.stage選曲.n確定された曲の難易度) : "");
        }
    ...
    //            CDTXMania.tテクスチャの解放( ref this.txPlayerNumber);
		...
		public override int Draw()
		{
			...
			if( !base.NotActivated )
			{
				...
				if ( base.JustStartedUpdate )
				{
					...
					this.ctチップ模様アニメ.Bass = new Counter( 0, 0x17, 20, TJAPlayer3.Timer );
					this.ctチップ模様アニメ.Taiko = new Counter( 0, 1, 500, TJAPlayer3.Timer );
                ...
                else if (TJAPlayer3._MainConfig.bBGA有効)
                {
                    actBackground.Draw();
                }
                ...
                if (!bDoublePlay && TJAPlayer3._MainConfig.ShowDancer)
                {
                    actDancer.Draw();
                }

                if(!bDoublePlay && TJAPlayer3._MainConfig.ShowFooter)
                    this.actFooter.Draw();
                ...
                //this.t進行描画_DANGER();
                //this.t進行描画_判定ライン();
                if ( !TJAPlayer3._MainConfig.bNoInfo )
                    this.t進行描画_ネームプレート();
                if( TJAPlayer3._MainConfig.ShowChara )
                    this.actChara.Draw();

                if(TJAPlayer3._MainConfig.ShowMob)
                    this.actMob.Draw();

                if ( TJAPlayer3._MainConfig.eGameMode != EGame.OFF )
                    this.actGame.Draw();

				this.t進行描画_譜面スクロール速度();
				this.t進行描画_チップアニメ();

                if(TJAPlayer3._MainConfig.ShowRunner)
                    this.actRunner.Draw();
                ...
                if( ( TJAPlayer3._MainConfig.eClipDispType == EClipDispType.ウィンドウのみ || TJAPlayer3._MainConfig.eClipDispType == EClipDispType.両方 ) && TJAPlayer3._MainConfig.nPlayerCount == 1 )
                    this.actAVI.t窓表示();
		...
		private FDKTexture txレーンフレームGB;
        ...
        private FDKTexture tx判定数表示パネル;
        private FDKTexture tx判定数小文字;
        ...
        private FDKTexture txMovie; //2016.08.30 kairera0467 ウィンドウ表示
		...
		{
			TJAPlayer3._MainConfig.n譜面スクロール速度.Drums = Math.Min( TJAPlayer3._MainConfig.n譜面スクロール速度.Drums + 1, 1999 );
		}
		protected override void ドラムスクロール速度ダウン()
		{
			TJAPlayer3._MainConfig.n譜面スクロール速度.Drums = Math.Max( TJAPlayer3._MainConfig.n譜面スクロール速度.Drums - 1, 0 );
		}
        ...
        {
			if( TJAPlayer3._MainConfig.bGraph.Drums )
			{
                this.actGraph.Draw();
            }
        }
		...
		{
		    var nInputAdjustTimeMs = TJAPlayer3._MainConfig.nInputAdjustTimeMs;
																		...
																		//		  2012.1.5 yyagi: (int)Eパッド.MAX に変更。Eパッドの要素数への依存を無くすため。
			{
				...
				foreach( STInputEvent inputEvent in listInputEvent )
				{
					...
					switch( ( (Eパッド) nPad ) )
					{
                        ...
                        case Eパッド.LRed2P:
                            ...
                            //-----------------------------
                            {
                                ...
                                if( e判定 != E判定.Miss && ( chipNoHit.nチャンネル番号 == 0x13 || chipNoHit.nチャンネル番号 == 0x1A ) && !TJAPlayer3._MainConfig.b大音符判定 )
								{
                        ...
                        case Eパッド.LBlue2P:
                            ...
                            //-----------------------------
                            {
                                ...
                                if( e判定 != E判定.Miss && ( chipNoHit.nチャンネル番号 == 0x14 || chipNoHit.nチャンネル番号 == 0x1B ) && !TJAPlayer3._MainConfig.b大音符判定 )
								{
					...
					// BAD or TIGHT 時の処理。
					if( TJAPlayer3._MainConfig.bTight && !b連打中[nUsePlayer]) // 18/8/13 - 連打時にこれが発動すると困る!!! (AioiLight)
		...
		protected override void t背景テクスチャの生成()
		{
			...
			string DefaultBgFilename = @"Graphics\Game\Background\0\Background.png";
			string BgFilename = "";
		...
		protected override void t進行描画_チップ_Taiko( MainConfig configIni, ref Chart dTX, ref Chart.CChip pChip, int nPlayer )
        {
            ...
            if( pChip.b可視 )
            {
                if( !pChip.bHit || pChip.bShow )
				{
                    ...
                    if( TJAPlayer3._MainConfig.eSTEALTH == Eステルスモード.STEALTH )
                    {
                        pChip.bShow = false;
                    }
                    #endregion
                        ...
                        if( TJAPlayer3.Tx.Notes != null )
                        {
                            ...
                            if (TJAPlayer3.Skin.SkinValue.Game_Notes_Anime)
                            {
                                if (this.actCombo.n現在のコンボ数[nPlayer] >= 300 && ctChipAnimeLag[nPlayer].IsEndValueReached)
                                {
                                    ...
                                    if ((int)ctChipAnime[nPlayer].NowValue_Double == 1 || (int)ctChipAnime[nPlayer].NowValue_Double == 3)
                                    {
                                        num9 = 260;
                                    }
                                    else
                                ...
                                else if (this.actCombo.n現在のコンボ数[nPlayer] >= 300 && !ctChipAnimeLag[nPlayer].IsEndValueReached)
                                {
                                ...
                                else if (this.actCombo.n現在のコンボ数[nPlayer] >= 50 && ctChipAnimeLag[nPlayer].IsEndValueReached)
                                {
                                    //num9 = base.n現在の音符の顔番号 != 0 ? base.n現在の音符の顔番号 * 130 : 0;
                                    if ((int)ctChipAnime[nPlayer].NowValue_Double <= 1)
                                    {
                                        num9 = 130;
                                    }
                                    else
                            ...
                            int nHand = this.ct手つなぎ.NowValue < 30 ? this.ct手つなぎ.NowValue : 60 - this.ct手つなぎ.NowValue;


                            x = ( x ) - ( ( int ) ( ( 130.0 * pChip.dbチップサイズ倍率 ) / 2.0 ) );
                            TJAPlayer3.Tx.Notes.IsAddBlend = false;
                            TJAPlayer3.Tx.SENotes.IsAddBlend = false;
                            ...
                            switch ( pChip.nチャンネル番号 )
                            {
                                case 0x11:
                                    ...
                                    if( TJAPlayer3.Tx.Notes != null )
                                    {
                                        if( TJAPlayer3._MainConfig.eSTEALTH != Eステルスモード.DORON )
                                            TJAPlayer3.Tx.Notes.Draw2D( device, x, y, new Rectangle( 130, num9, 130, 130 ) );
                                        TJAPlayer3.Tx.SENotes.Draw2D( device, x - 2, y + nSenotesY, new Rectangle( 0, 30 * pChip.nSenote, 136, 30 ) );
                                ...
                                case 0x12:
                                    ...
                                    {
                                        if( TJAPlayer3._MainConfig.eSTEALTH != Eステルスモード.DORON )
                                            TJAPlayer3.Tx.Notes.Draw2D( device, x, y, new Rectangle( 260, num9, 130, 130) );
                                        TJAPlayer3.Tx.SENotes.Draw2D( device, x - 2, y + nSenotesY, new Rectangle( 0, 30 * pChip.nSenote, 136, 30 ) );
                                        //CDTXMania.act文字コンソール.tPrint( x + 60, y + 140, C文字コンソール.Eフォント種別.白, pChip.nSenote.ToString() );
                                    }
                                    nLane = 1;
                                ...
                                case 0x13:
                                    if( TJAPlayer3.Tx.Notes != null )
                                    {
                                        if( TJAPlayer3._MainConfig.eSTEALTH != Eステルスモード.DORON )
                                        {
                                            TJAPlayer3.Tx.Notes.Draw2D( device, x, y, new Rectangle( 390, num9, 130, 130 ) );
                                ...
                                case 0x14:
                                    ...
                                    {
                                        if( TJAPlayer3._MainConfig.eSTEALTH != Eステルスモード.DORON )
                                            TJAPlayer3.Tx.Notes.Draw2D( device, x, y, new Rectangle( 520, num9, 130, 130 ) );
                                        TJAPlayer3.Tx.SENotes.Draw2D( device, x - 2, y + nSenotesY, new Rectangle( 0, 30 * pChip.nSenote, 136, 30 ) );
                                        //CDTXMania.act文字コンソール.tPrint( x + 60, y + 140, C文字コンソール.Eフォント種別.白, pChip.nSenote.ToString() );
                                    }
                                    nLane = 1;
                                ...
                                case 0x1A:
                                    if( TJAPlayer3.Tx.Notes != null )
                                    {
                                        if( TJAPlayer3._MainConfig.eSTEALTH != Eステルスモード.DORON )
                                        {
                                            if( nPlayer == 0 )
                                            {
                                                TJAPlayer3.Tx.Notes_Arm.VerticalFlipDraw2D( device, x + 25, ( y + 74 ) + nHand );
                                                TJAPlayer3.Tx.Notes_Arm.VerticalFlipDraw2D( device, x + 60, ( y + 104 ) - nHand );
                                            }
                                            else if( nPlayer == 1 )
                                            {
                                                TJAPlayer3.Tx.Notes_Arm.Draw2D( device, x + 25, ( y - 44 ) + nHand );
                                                TJAPlayer3.Tx.Notes_Arm.Draw2D( device, x + 60, ( y - 14 ) - nHand );
                                            }
                                            TJAPlayer3.Tx.Notes.Draw2D( device, x, y, new Rectangle( 1690, num9, 130, 130 ) );
                                            //CDTXMania.Tx.Notes.t3D描画( device, mat, new Rectangle( 390, num9, 130, 130 ) );
                                        }
                                        TJAPlayer3.Tx.SENotes.Draw2D( device, x - 2, y + nSenotesY, new Rectangle( 0, 390, 136, 30 ) );
                                    }
                                    break;
                                ...
                                case 0x1B:
                                    if( TJAPlayer3.Tx.Notes != null )
                                    {
                                        if( TJAPlayer3._MainConfig.eSTEALTH != Eステルスモード.DORON )
                                        {
                                            ...
                                            TJAPlayer3.Tx.Notes.Draw2D( device, x, y, new Rectangle( 1820, num9, 130, 130 ) );
                                        }
                                        TJAPlayer3.Tx.SENotes.Draw2D( device, x - 2, y + nSenotesY, new Rectangle( 0, 420, 136, 30 ) );
                                    }
                                    nLane = 1;
                                    break;
            ...
            if( pChip.b可視 )
            {
                ...
                if( ( 1400 > x ) && pChip.bShow )
                {
                    if( TJAPlayer3.Tx.Notes != null )
                    {
                        ...
                        if (pChip.RollInputTime != null && pChip.RollInputTime.IsEndValueReached)
                        {
                            pChip.RollInputTime.Stop();
                            pChip.RollInputTime.NowValue = 0;
                            pChip.RollDelay = new Counter(0, 1, 1, TJAPlayer3.Timer);
                        }
                        
                        if (pChip.RollDelay != null && pChip.RollDelay.IsEndValueReached && pChip.RollEffectLevel > 0)
                        {
                            pChip.RollEffectLevel--;
                            pChip.RollDelay = new Counter(0, 1, 1, TJAPlayer3.Timer);
                            pChip.RollDelay.NowValue = 0;
                        }
                        ...
                        if ( pChip.nチャンネル番号 == 0x15 ) //連打(小)
                        {
                            ...
                            if ( TJAPlayer3._MainConfig.eSTEALTH != Eステルスモード.DORON )
                            {
                                ...
                                TJAPlayer3.Tx.Notes.Scaling.X = (index - 65.0f + f末端ノーツのテクスチャ位置調整 + 1) / 128.0f;
                                ...
                                TJAPlayer3.Tx.Notes.Draw2D(TJAPlayer3.app.Device, x, y, 0, new Rectangle(650, num9, 130, 130));
                                TJAPlayer3.Tx.Notes.color4 = normalColor;
                                #endregion
                            }
                            TJAPlayer3.Tx.SENotes.Scaling.X = index - 44;
                            TJAPlayer3.Tx.SENotes.Draw2D( TJAPlayer3.app.Device, x + 90, y + nSenotesY, new Rectangle( 60, 240, 1, 30 ) );
                            TJAPlayer3.Tx.SENotes.Scaling.X = 1.0f;
                            TJAPlayer3.Tx.SENotes.Draw2D( TJAPlayer3.app.Device, x + 30, y + nSenotesY, new Rectangle(0, 240, 60, 30));
                            TJAPlayer3.Tx.SENotes.Draw2D(TJAPlayer3.app.Device, x, y + nSenotesY, new Rectangle(0, 30 * pChip.nSenote, 136, 30));
                        }
                        if( pChip.nチャンネル番号 == 0x16 )
                        {
                            ...
                            if( TJAPlayer3._MainConfig.eSTEALTH != Eステルスモード.DORON )
                            {
                                ...
                                TJAPlayer3.Tx.Notes.Scaling.X = (index - 65 + f末端ノーツのテクスチャ位置調整 + 1) / 128f;
                                TJAPlayer3.Tx.Notes.Draw2D(TJAPlayer3.app.Device, x + 64, y, new Rectangle(1171, 0, 128, 130));
                                ...
                                TJAPlayer3.Tx.Notes.Draw2D(TJAPlayer3.app.Device, x, y, new Rectangle(1040, num9, 130, 130));
                                TJAPlayer3.Tx.Notes.color4 = normalColor;
                                #endregion
                            }
                            TJAPlayer3.Tx.SENotes.Scaling.X = index - 70;
                            TJAPlayer3.Tx.SENotes.Draw2D( TJAPlayer3.app.Device, x + 116, y + nSenotesY, new Rectangle( 60, 240, 1, 30 ) );
                            TJAPlayer3.Tx.SENotes.Scaling.X = 1.0f;
                            TJAPlayer3.Tx.SENotes.Draw2D(TJAPlayer3.app.Device, x + 56, y + nSenotesY, new Rectangle(0, 240, 60, 30));
                            TJAPlayer3.Tx.SENotes.Draw2D(TJAPlayer3.app.Device, x - 2, y + nSenotesY, new Rectangle(0, 30 * pChip.nSenote, 136, 30));
                        }
                        ...
                        if( pChip.nチャンネル番号 == 0x18 )
                        {
                            ...
                            if( pChip.n連打音符State != 7 )
                            {
                                ...
                                TJAPlayer3.Tx.SENotes.Draw2D(TJAPlayer3.app.Device, x + 56, y + nSenotesY, new Rectangle( 58, 270, 78, 30 ) );
                            }
		...
		protected override void t進行描画_チップ_小節線( MainConfig configIni, ref Chart dTX, ref Chart.CChip pChip, int nPlayer )
		{
			...
			if ( configIni.b演奏情報を表示する )
			{
                ...
                if ( x >= 310 )
                {
				    TJAPlayer3._ConsoleText.tPrint(x + 8, y - 26, ConsoleText.FontType.White, nowMeasure.ToString());
                }
			}
			if ( ( pChip.b可視 ) && (TJAPlayer3.Tx.Bar != null ) )
			{
                if( x >= 0 )
                {
                    ...
                    mat *= Matrix.RotationZ(ConvertUtility.DegreeToRadian(-(90.0f * (float)pChip.dbSCROLL_Y)));
                    ...
                    if( pChip.bBranch )
                    {
                        ...
                        TJAPlayer3.Tx.Bar_Branch.Draw3D( TJAPlayer3.app.Device, mat, new Rectangle( 0, 0, 3, 130 ) );
                    }
                    else
                    {
                        //this.tx小節線.t2D描画( CDTXMania.app.Device, x - 3, y, new Rectangle( 0, 0, 3, 130 ) );
                        TJAPlayer3.Tx.Bar.Draw3D( TJAPlayer3.app.Device, mat, new Rectangle( 0, 0, 3, 130 ) );
                    }
                }
			}
		}
        ...
        public void t全体制御メソッド()
        {
            ...
            for( int i = 0; i < TJAPlayer3._MainConfig.nPlayerCount; i++ )
            {
                if( this.chip現在処理中の連打チップ[ i ] != null )
                {
                    ...
                    {
                        if (actChara.CharaAction_Balloon_Breaking.IsProcessed && chip現在処理中の連打チップ[i].nPlayerSide == 0)
                        {
                        }
                    }
                }
            ...
            if( ( this.n分岐した回数[ 0 ] < TJAPlayer3.DTX.listBRANCH.Count ) && TJAPlayer3._MainConfig.bBranchGuide && !TJAPlayer3._MainConfig.b太鼓パートAutoPlay )
            {
                f現在の精度 = 0;
                ...
                if( n種類 == 0 )
                {
                    ...
                    TJAPlayer3._ConsoleText.tPrint( 0, 128, ConsoleText.FontType.White, f現在の精度.ToString() );
                }
                if( n種類 == 1 )
                {
                    ...
                    TJAPlayer3._ConsoleText.tPrint( 0, 128, ConsoleText.FontType.White, this.nBranch_roll.ToString() );
                }


                TJAPlayer3._ConsoleText.tPrint( 0, 160, ConsoleText.FontType.White, string.Format( "NEXT BRANCH:{0:##0}", n次回分岐までの小節数.ToString() ) );
                TJAPlayer3._ConsoleText.tPrint( 0, 362, ConsoleText.FontType.White, string.Format( "NEXT BRANCH INFO:{0:##0} , {1:##0}", TJAPlayer3.DTX.listBRANCH[ this.n分岐した回数[ 0 ] ].n条件数値A.ToString(), TJAPlayer3.DTX.listBRANCH[ this.n分岐した回数[ 0 ] ].n条件数値B.ToString() ) );
                TJAPlayer3._ConsoleText.tPrint( 0, 144, ConsoleText.FontType.White, strNext.ToString() );
            }
            else if( ( this.n分岐した回数[ 0 ] >= TJAPlayer3.DTX.listBRANCH.Count ) && TJAPlayer3._MainConfig.bBranchGuide && !TJAPlayer3._MainConfig.b太鼓パートAutoPlay  )
            {
                TJAPlayer3._ConsoleText.tPrint( 0, 144, ConsoleText.FontType.White, strNext.ToString() );
            }
            #endregion
        ...
        {
            if( TJAPlayer3._MainConfig.nPlayerCount == 1 ? ( TJAPlayer3._MainConfig.bJudgeCountDisplay && !TJAPlayer3._MainConfig.b太鼓パートAutoPlay ) : false )
            {
                //ボードの横幅は333px
                //数字フォントの小さいほうはリザルトのものと同じ。
                if( TJAPlayer3.Tx.Judge_Meter != null )
                    TJAPlayer3.Tx.Judge_Meter.Draw2D( TJAPlayer3.app.Device, 0, 360 );
			...
			foreach( char ch in str )
			{
				for( int i = 0; i < this.st小文字位置.Length; i++ )
				{
					...
					if( this.st小文字位置[ i ].ch == ch )
					{
						...
						{
                            TJAPlayer3.Tx.Result_Number.Draw2D( TJAPlayer3.app.Device, x, y, rectangle );
						}
						break;
					}
				}
