...
{
	internal class CAct演奏Drums連打キャラ : Activity
	{
		// コンストラクタ
		...
		public override void Activate()
		{
            ...
            for (int i = 0; i < 128; i++)
            {
                ...
                RollCharas[i].Counter = new Counter();
            }
		...
		public override int Draw()
		{
			if( !base.NotActivated )
			{
                ...
                for (int i = 0; i < 128; i++)
                {
                    if(RollCharas[i].IsUsing)
                    {
                        RollCharas[i].OldValue = RollCharas[i].Counter.NowValue;
                        RollCharas[i].Counter.Tick();
                        if(RollCharas[i].Counter.IsEndValueReached)
                        {
                            RollCharas[i].Counter.Stop();
                            RollCharas[i].IsUsing = false;
                        }
                        for (int l = RollCharas[i].OldValue; l < RollCharas[i].Counter.NowValue; l++)
                        {
                            RollCharas[i].X += RollCharas[i].XAdd;
                            RollCharas[i].Y += RollCharas[i].YAdd;
                        }
                        TJAPlayer3.Tx.Effects_Roll[RollCharas[i].Type]?.Draw2D(TJAPlayer3.app.Device, RollCharas[i].X, RollCharas[i].Y);
                        // 画面外にいたら描画をやめさせる
                        if(RollCharas[i].X < 0 - TJAPlayer3.Tx.Effects_Roll[RollCharas[i].Type].TextureSize.Width || RollCharas[i].X > 1280)
                        {
                            RollCharas[i].Counter.Stop();
                            RollCharas[i].IsUsing = false;
                        }
                        if (RollCharas[i].Y < 0 - TJAPlayer3.Tx.Effects_Roll[RollCharas[i].Type].TextureSize.Height || RollCharas[i].Y > 720)
                        {
                            RollCharas[i].Counter.Stop();
                            RollCharas[i].IsUsing = false;
                        }
