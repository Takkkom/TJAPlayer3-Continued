	...
	internal class CAct演奏Drums判定文字列 : CAct演奏判定文字列共通
	{
		...
		public override int t進行描画( C演奏判定ライン座標共通 演奏判定ライン座標 )
		{
			if( !base.NotActivated )
			{
				for( int i = 0; i < 12; i++ )
				{
					if( !base.st状態[ i ].ct進行.IsStoped )
					{
						base.st状態[ i ].ct進行.Tick();
						if( base.st状態[ i ].ct進行.IsEndValueReached )
						{
							base.st状態[ i ].ct進行.Stop();
                            base.st状態[ i ].b使用中 = false;
						}
						int num2 = base.st状態[ i ].ct進行.NowValue;
				...
				for( int j = 0; j < 12; j++ )
				{
					if( !base.st状態[ j ].ct進行.IsStoped )
					{
						int baseX = 370;
                        //int baseY = 135;
                        int baseY = TJAPlayer3.Skin.nScrollFieldY[base.st状態[j].nPlayer] - 53;
						int x = TJAPlayer3.Skin.nScrollFieldX[ 0 ] - TJAPlayer3.Tx.Judge.TextureSize.Width / 2;
						int y = ( baseY + base.st状態[ j ].n相対Y座標 );
						if( TJAPlayer3.Tx.Judge != null )
						{
                            TJAPlayer3.Tx.Judge.Draw2D( TJAPlayer3.app.Device, x, y, base.st判定文字列[ (int) base.st状態[ j ].judge ].rc );
						}
					}
				}
			}
