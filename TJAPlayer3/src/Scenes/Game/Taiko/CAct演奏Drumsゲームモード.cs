...
{
    internal class CAct演奏Drumsゲームモード : Activity
    {
        /// <summary>
        ...
        public struct ST叩ききりまショー
        {
            ...
            public int n延長アニメ速度;
            public Counter ct残り時間;
            public Counter ct加算時間表示;
            public Counter ct加算審査中;
            public Counter ct針アニメ;
        }
        ...
        public void t叩ききりまショー_初期化()
        {
            ...
            this.st叩ききりまショー.ct残り時間 = new Counter( 0, 25000, 1, TJAPlayer3.Timer );
            this.st叩ききりまショー.ct加算時間表示 = new Counter();
            this.st叩ききりまショー.ct加算審査中 = new Counter();
            ...
            this.st叩ききりまショー.ct針アニメ = new Counter( 0, 1000, 1, TJAPlayer3.Timer );
        ...
        public void t叩ききりまショー_判定項目と難易度を決める()
        {
            ...
            else if( TJAPlayer3._MainConfig.eGameMode == EGame.完走叩ききりまショー激辛 )
            {
                #region[ 激辛 ]
                //激ムズの査定
                // 最大ズレ > 精度 > コンボ > 最小ズレ > ミス
        ...
        {
            if( TJAPlayer3._MainConfig.eGameMode == EGame.完走叩ききりまショー || TJAPlayer3._MainConfig.eGameMode == EGame.完走叩ききりまショー激辛 )
            {
                ...
                if( this.st叩ききりまショー.bタイマー使用中 )
                {
				    if( !this.st叩ききりまショー.ct残り時間.IsStoped || this.st叩ききりまショー.b加算アニメ中 == true )
				    {
					    ...
					    if( !TJAPlayer3.stage演奏ドラム画面.r検索範囲内にチップがあるか調べる( CSound管理.rc演奏用タイマ.NowTimeMs, 0, 5000, 0 ) || this.st叩ききりまショー.b加算アニメ中 == true )
					    {
                            this.st叩ききりまショー.bタイマー使用中 = false;
                ...
                {
                    if ((this.st叩ききりまショー.b最初のチップが叩かれた == true && ( TJAPlayer3.stage演奏ドラム画面.r検索範囲内にチップがあるか調べる(CSound管理.rc演奏用タイマ.NowTimeMs, 0, 2000, 0 ) ) ) )
                    {
                        this.st叩ききりまショー.bタイマー使用中 = true;
                        int nCount = this.st叩ききりまショー.ct残り時間.NowValue;
                        this.st叩ききりまショー.ct残り時間 = new Counter( 0, 25000, 1, TJAPlayer3.Timer );
                        this.st叩ききりまショー.ct針アニメ = new Counter( 0, 1000, 1, TJAPlayer3.Timer );
                        this.st叩ききりまショー.ct残り時間.NowValue = nCount;
                    }

                }


                if( ( this.st叩ききりまショー.ct残り時間.NowValue >= 20000 ) && this.st叩ききりまショー.ct残り時間.NowValue != 25000 )
                    this.t叩ききりまショー_評価をして残り時間を延長する();

                if( TJAPlayer3.Tx.Tile_Black != null )
                {
                    if( this.st叩ききりまショー.ct残り時間.NowValue >= 22000 && this.st叩ききりまショー.ct残り時間.NowValue < 23000 )
                        TJAPlayer3.Tx.Tile_Black.Opacity = 64;
                    else if( this.st叩ききりまショー.ct残り時間.NowValue >= 23000 && this.st叩ききりまショー.ct残り時間.NowValue < 24000 )
                        TJAPlayer3.Tx.Tile_Black.Opacity = 128;
                    else if( this.st叩ききりまショー.ct残り時間.NowValue >= 24000 )
                    ...
                    for (int i = 0; i <= (SampleFramework.GameWindowSize.Width / 64); i++)
                    {
                        ...
                        {
                            TJAPlayer3.Tx.Tile_Black.Draw2D(TJAPlayer3.app.Device, i * 64, j * 64);
                        }
                    }
                }
                ...
                if(TJAPlayer3.Tx.Taiko_Combo != null )
                {
                    if (TJAPlayer3.Tx.GameMode_Timer_Frame != null)
                        TJAPlayer3.Tx.GameMode_Timer_Frame.Draw2D( TJAPlayer3.app.Device, 230, 84 );
                    this.st叩ききりまショー.ct針アニメ.TickLoop();

                    int nCenterX = 230;
                    int nCerterY = 84;
                    float fRotate = -ConvertUtility.DegreeToRadian( 360.0f * ( this.st叩ききりまショー.ct針アニメ.NowValue / 1000.0f ) );
                    if( this.st叩ききりまショー.b加算アニメ中 == true )
                        fRotate = ConvertUtility.DegreeToRadian( 360.0f * ( this.st叩ききりまショー.ct針アニメ.NowValue / (float)this.st叩ききりまショー.n延長アニメ速度 ) );
                ...
                if( !this.st叩ききりまショー.ct加算審査中.IsStoped )
                {
				    if( !this.st叩ききりまショー.ct加算審査中.IsStoped )
				    {
					    ...
					    if( this.st叩ききりまショー.ct加算審査中.IsEndValueReached )
					    {
                ...
                if( !this.st叩ききりまショー.ct加算時間表示.IsStoped )
                {
				    if( !this.st叩ききりまショー.ct加算時間表示.IsStoped )
				    {
					    ...
					    if( this.st叩ききりまショー.ct加算時間表示.IsEndValueReached )
					    {
        ...
        private void t叩ききりまショー_評価をして残り時間を延長する()
        {
            ...
            if( this.n最後に時間延長した時刻 + 11000 <= CSound管理.rc演奏用タイマ.NowTimeMs )
            {
                //1項目につき5秒
                //-精度
                ...
                this.n最後に時間延長した時刻 = (int)CSound管理.rc演奏用タイマ.NowTimeMs;
                if( n延長する時間 < 0 )
                    n延長する時間 = 0;
                ...
                this.st叩ききりまショー.ct残り時間.NowValue -= (int)n延長する時間;
            }
            else if( this.st叩ききりまショー.ct残り時間.NowValue >= 24000 )
            {
        ...
        {
            this.st叩ききりまショー.ct加算審査中 = new Counter( 0, 2000, 1, TJAPlayer3.Timer );
            this.st叩ききりまショー.b加算アニメ中 = true;
        }
        private void t加算時間描画_Start()
        {
            this.st叩ききりまショー.ct加算時間表示 = new Counter( 0, 1, 1000, TJAPlayer3.Timer );
        }
				...
				for( int i = 0; i < this.st小文字位置.Length; i++ )
				{
                    if (this.st小文字位置[i].ch == ch)
                    {
						...
						if(TJAPlayer3.Tx.Taiko_Combo[0]  != null )
						{
                            ...
                            TJAPlayer3.Tx.Taiko_Combo[0].Scaling.Y = 1f;
                            TJAPlayer3.Tx.Taiko_Combo[0].Scaling.X = 1f;
                ...
                for( int i = 0; i < cFont.Length; i++ )
                {
                    if( cFont[ i ] == ch )
                    {
                        ...
                        if (TJAPlayer3.Tx.Taiko_Score[0] != null )
                        {
                            ...
                            TJAPlayer3.Tx.Taiko_Score[0].Draw2D( TJAPlayer3.app.Device, x, y, rectangle );
                        }
                    }
                }
                x += 20;
