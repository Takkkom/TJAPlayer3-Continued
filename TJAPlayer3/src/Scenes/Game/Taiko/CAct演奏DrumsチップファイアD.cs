...
{
	internal class CAct演奏DrumsチップファイアD : Activity
	{
		// コンストラクタ
                ...
                for (int i = 0; i < 9; i++)
                {
                    for (int j = 0; j < 45; j++)
                    {
                        if (!this.st大音符花火[j].b使用中)
                        {
                            ...
                            this.st大音符花火[j].ct進行 = new Counter(0, 40, 18, TJAPlayer3.Timer); // カウンタ
        ...
        public virtual void Start( int nLane, E判定 judge, int player )
		{
            for (int j = 0; j < 3 * 4; j++)
            {
			    ...
			    //for( int n = 0; n < 1; n++ )
			    {
	    			...
	    			this.st状態[ j ].ct進行 = new Counter( 0, 6, 25, TJAPlayer3.Timer );
                    ...
                    switch( nLane )
                    {
                        ...
                        case 0x1A:
                        case 0x1B:
                            this.st状態_大[ j ].ct進行 = new Counter( 0, 9, 20, TJAPlayer3.Timer );
		...
		public override void Activate()
		{
            for( int i = 0; i < 3 * 4; i++ )
			{
                ...
                this.st状態_大[ i ].ct進行 = new Counter();
			}
			for( int i = 0; i < 256; i++ )
			{
				this.st紙吹雪[ i ] = new ST紙吹雪();
				this.st紙吹雪[ i ].b使用中 = false;
				this.st紙吹雪[ i ].ct進行 = new Counter();
			}
            base.Activate();
		}
			...
			if( !base.NotActivated )
			{
                    ...
                    if( this.st状態[ i ].b使用中 )
                    {
				        if( !this.st状態[ i ].ct進行.IsStoped )
				        {
					        ...
					        if( TJAPlayer3.Tx.Effects_Hit_Explosion != null && !TJAPlayer3.IsPerformingCalibration )
					        {
                                ...
                                int nX = ( TJAPlayer3.Skin.nScrollFieldX[ this.st状態[ i ].nPlayer ] ) - ( (TJAPlayer3.Tx.Effects_Hit_Explosion.ImageSize.Width / 7 ) / 2 );
                                int nY = ( TJAPlayer3.Skin.nJudgePointY[ this.st状態[ i ].nPlayer ] ) - ( (TJAPlayer3.Tx.Effects_Hit_Explosion.ImageSize.Height / 4 ) / 2 );
                                ...
                                switch( st状態[ i ].judge )
                                {
                                    ...
                                    case E判定.Auto:
                                        if (!this.st状態_大[i].ct進行.IsStoped && TJAPlayer3.Tx.Effects_Hit_Explosion_Big != null && this.st状態_大[i].nIsBig == 1)  
                                                TJAPlayer3.Tx.Effects_Hit_Explosion.Draw2D(TJAPlayer3.app.Device, nX, nY, new Rectangle(this.st状態[i].ct進行.NowValue * 260, n + 520, 260, 260));
                                        else
                                            TJAPlayer3.Tx.Effects_Hit_Explosion.Draw2D(TJAPlayer3.app.Device, nX, nY, new Rectangle(this.st状態[i].ct進行.NowValue * 260, n, 260, 260));
                                        break;                                    
                                    case E判定.Good:
                                        if (!this.st状態_大[i].ct進行.IsStoped && TJAPlayer3.Tx.Effects_Hit_Explosion_Big != null && this.st状態_大[i].nIsBig == 1)
                                            TJAPlayer3.Tx.Effects_Hit_Explosion.Draw2D( TJAPlayer3.app.Device, nX, nY, new Rectangle( this.st状態[ i ].ct進行.NowValue * 260, n + 780, 260, 260 ) );
                                        else
                                            TJAPlayer3.Tx.Effects_Hit_Explosion.Draw2D(TJAPlayer3.app.Device, nX, nY, new Rectangle(this.st状態[i].ct進行.NowValue * 260, n + 260, 260, 260));
                                        break;
                                    case E判定.Miss:
                                    case E判定.Bad:
                                        break;
                ...
                for( int i = 0; i < 3 * 4; i++ )
			    {
				    if( !this.st状態_大[ i ].ct進行.IsStoped )
				    {
					    ...
					    if( this.st状態_大[ i ].ct進行.IsEndValueReached )
					    {
					    ...
					    if(TJAPlayer3.Tx.Effects_Hit_Explosion_Big != null && this.st状態_大[ i ].nIsBig == 1 )
					    {
                            ...
                            switch( st状態_大[ i ].judge )
                            {
                                ...
                                case E判定.Auto:
                                    ...
                                    {
                                        float fX = 415 - ((TJAPlayer3.Tx.Effects_Hit_Explosion_Big.ImageSize.Width * TJAPlayer3.Tx.Effects_Hit_Explosion_Big.Scaling.X ) / 2.0f);
                                        float fY = TJAPlayer3.Skin.nJudgePointY[ this.st状態_大[ i ].nPlayer ] - ((TJAPlayer3.Tx.Effects_Hit_Explosion_Big.ImageSize.Height * TJAPlayer3.Tx.Effects_Hit_Explosion_Big.Scaling.Y ) / 2.0f);
                                        //float fY = 257 - ((this.txアタックエフェクトUpper_big.sz画像サイズ.Height * this.txアタックエフェクトUpper_big.vc拡大縮小倍率.Y ) / 2.0f);

                                        ////7
                                        float f倍率 = 0.5f + ( (this.st状態_大[ i ].ct進行.NowValue * 0.5f) / 10.0f);
                                        ...
                                        TJAPlayer3.Tx.Effects_Hit_Explosion_Big.Draw3D( TJAPlayer3.app.Device, mat );
                                    }
                                    break;
                ...
                for (int i = 0; i < 45; i++)
                {
                    ...
                    {
                        this.st大音符花火[i].n前回のValue = this.st大音符花火[i].ct進行.NowValue;
                        this.st大音符花火[i].ct進行.Tick();
                        if (this.st大音符花火[i].ct進行.IsEndValueReached)
                        {
                ...
                for (int i = 0; i < 256; i++)
                {
                    ...
                    {
                        this.st紙吹雪[i].n前回のValue = this.st紙吹雪[i].ct進行.NowValue;
                        this.st紙吹雪[i].ct進行.Tick();
                        if (this.st紙吹雪[i].ct進行.IsEndValueReached)
                        {
                        ...
                        for (int n = this.st紙吹雪[i].n前回のValue; n < this.st紙吹雪[i].ct進行.NowValue; n++)
                        {
                        ...
                        float x = (float)(this.st紙吹雪[i].f半径 * Math.Cos((Math.PI / 2 * this.st紙吹雪[i].ct進行.NowValue) / 100.0)) * 2.3f;
