...
namespace TJAPlayer3
{
	...
	/// </summary>
	internal abstract class GameScene : BaseScene
	{
		// プロパティ
		...
		public void t演奏結果を格納する_ドラム( out ScoreIni.C演奏記録 Drums )
		{
			Drums = new ScoreIni.C演奏記録();

			//if (  )
			{
				Drums.nスコア = (long) this.actScore.Get( E楽器パート.DRUMS, 0 );
				Drums.dbゲーム型スキル値 = ScoreIni.tゲーム型スキルを計算して返す( TJAPlayer3.DTX.LEVEL.Drums, TJAPlayer3.DTX.n可視チップ数.Drums, this.nヒット数_Auto含まない.Drums.Perfect, this.actCombo.n現在のコンボ数.P1最高値, E楽器パート.DRUMS, TJAPlayer3._MainConfig.AutoPlay[0] );
				Drums.db演奏型スキル値 = ScoreIni.t演奏型スキルを計算して返す( TJAPlayer3.DTX.n可視チップ数.Drums, this.nヒット数_Auto含まない.Drums.Perfect, this.nヒット数_Auto含まない.Drums.Great, this.nヒット数_Auto含まない.Drums.Good, this.nヒット数_Auto含まない.Drums.Poor, this.nヒット数_Auto含まない.Drums.Miss, E楽器パート.DRUMS, TJAPlayer3._MainConfig.AutoPlay[0]);
				Drums.nPerfect数 = TJAPlayer3._MainConfig.b太鼓パートAutoPlay ? this.nヒット数_Auto含む.Drums.Perfect : this.nヒット数_Auto含まない.Drums.Perfect;
				Drums.nGreat数 = TJAPlayer3._MainConfig.b太鼓パートAutoPlay ? this.nヒット数_Auto含む.Drums.Great : this.nヒット数_Auto含まない.Drums.Great;
				Drums.nGood数 = TJAPlayer3._MainConfig.b太鼓パートAutoPlay ? this.nヒット数_Auto含む.Drums.Good : this.nヒット数_Auto含まない.Drums.Good;
				Drums.nPoor数 = TJAPlayer3._MainConfig.b太鼓パートAutoPlay ? this.nヒット数_Auto含む.Drums.Poor : this.nヒット数_Auto含まない.Drums.Poor;
				Drums.nMiss数 = TJAPlayer3._MainConfig.b太鼓パートAutoPlay ? this.nヒット数_Auto含む.Drums.Miss : this.nヒット数_Auto含まない.Drums.Miss;
				...
				Drums.bTight = TJAPlayer3._MainConfig.bTight;
				for ( int i = 0; i < 3; i++ )
				{
					Drums.bSudden[ i ] = TJAPlayer3._MainConfig.bSudden[ i ];
					Drums.bHidden[ i ] = TJAPlayer3._MainConfig.bHidden[ i ];
					Drums.eInvisible[ i ] = TJAPlayer3._MainConfig.eInvisible[ i ];
					Drums.bReverse[ i ] = TJAPlayer3._MainConfig.bReverse[ i ];
					Drums.eRandom[ i ] = TJAPlayer3._MainConfig.eRandom[ i ];
					Drums.bLight[ i ] = TJAPlayer3._MainConfig.bLight[ i ];
					Drums.bLeft[ i ] = TJAPlayer3._MainConfig.bLeft[ i ];
					Drums.f譜面スクロール速度[ i ] = ( (float) ( TJAPlayer3._MainConfig.n譜面スクロール速度[ i ] + 1 ) ) * 0.5f;
				}
				Drums.eDark = TJAPlayer3._MainConfig.eDark;
				Drums.n演奏速度分子 = TJAPlayer3._MainConfig.n演奏速度;
				Drums.n演奏速度分母 = 20;
				Drums.bSTAGEFAILED有効 = TJAPlayer3._MainConfig.bSTAGEFAILED有効;
				Drums.eダメージレベル = TJAPlayer3._MainConfig.eダメージレベル;
				...
				Drums.Hash = ScoreIni.t演奏セクションのMD5を求めて返す( Drums );
		...
		{
            listChip = new List<Chart.CChip>[ 4 ];
            for( int i = 0; i < TJAPlayer3._MainConfig.nPlayerCount; i++ )
            {
                ...
                int n整数値管理 = 0;
                foreach( Chart.CChip chip in listChip[ i ] )
                {
            ...
            this.ct制御タイマ = new Counter();
            ctChipAnime = new Counter[2];
            ctChipAnimeLag = new Counter[2];
            for (int i = 0; i < 2; i++)
            {
                ctChipAnime[i] = new Counter();
                ctChipAnimeLag[i] = new Counter();
			...
			cInvisibleChip = new CInvisibleChip( TJAPlayer3._MainConfig.nDisplayTimesMs, TJAPlayer3._MainConfig.nFadeoutTimeMs );
			...
			for ( int k = 0; k < 4; k++ )
			{
				...
				//}
				this.queWailing[ k ] = new Queue<Chart.CChip>();
				this.r現在の歓声Chip[ k ] = null;
				cInvisibleChip.eInvisibleMode[ k ] = TJAPlayer3._MainConfig.eInvisible[ k ];
				if ( TJAPlayer3.DTXVmode.Enabled )
				{
					TJAPlayer3._MainConfig.n譜面スクロール速度[ k ] = TJAPlayer3._MainConfig.nViewerScrollSpeed[ k ];
				}

				//this.nJudgeLinePosY_delta[ k ] = CDTXMania.ConfigIni.nJudgeLinePosOffset[ k ];		// #31602 2013.6.23 yyagi

				this.演奏判定ライン座標.n判定位置[ k ] = TJAPlayer3._MainConfig.e判定位置[ k ];
				this.演奏判定ライン座標.nJudgeLinePosY_delta[ k ] = TJAPlayer3._MainConfig.nJudgeLinePosOffset[ k ];
				this.bReverse[ k ]             = TJAPlayer3._MainConfig.bReverse[ k ];					//
            ...
            this.bDoublePlay = TJAPlayer3._MainConfig.nPlayerCount >= 2 ? true : false;
...
//			this.nRisky = CDTXMania.ConfigIni.nRisky;											// #23559 2011.7.28 yyagi
			actGauge.Init( TJAPlayer3._MainConfig.nRisky );									// #23559 2011.7.28 yyagi
			this.nPolyphonicSounds = TJAPlayer3._MainConfig.nPoliphonicSounds;
			e判定表示優先度 = TJAPlayer3._MainConfig.e判定表示優先度;

			TJAPlayer3.Skin.tRemoveMixerAll();	// 効果音のストリームをミキサーから解除しておく

			queueMixerSound = new Queue<stmixer>( 64 );
			bIsDirectSound = ( TJAPlayer3._SoundManager.GetCurrentSoundDeviceType() == "DirectSound" );
			bUseOSTimer = TJAPlayer3._MainConfig.bUseOSTimer;
			this.bPAUSE = false;
			if ( TJAPlayer3.DTXVmode.Enabled )
			{
				db再生速度 = TJAPlayer3.DTX.dbDTXVPlaySpeed;
				TJAPlayer3._MainConfig.n演奏速度 = (int) (TJAPlayer3.DTX.dbDTXVPlaySpeed * 20 + 0.5 );
			}
			else
			{
				db再生速度 = ( (double) TJAPlayer3._MainConfig.n演奏速度 ) / 20.0;
			}
			bValidScore = ( TJAPlayer3.DTXVmode.Enabled ) ? false : true;

			#region [ 演奏開始前にmixer登録しておくべきサウンド(開幕してすぐに鳴らすことになるチップ音)を登録しておく ]
			foreach ( Chart.CChip pChip in listChip[0] )
			{
...
//						Trace.TraceInformation( "first [DA] BAR=" + pChip.n発声位置 / 384 + " ch=" + pChip.nチャンネル番号.ToString( "x2" ) + ", wav=" + pChip.n整数値 + ", time=" + pChip.n発声時刻ms );
						if ( listWAV.TryGetValue( pChip.n整数値_内部番号, out Chart.CWAV wc ) )
						{
							for ( int i = 0; i < nPolyphonicSounds; i++ )
							{
								if ( wc.rSound[ i ] != null )
								{
									TJAPlayer3._SoundManager.AddMixer( wc.rSound[ i ], db再生速度, pChip.b演奏終了後も再生が続くチップである );
...
//			GCSettings.LatencyMode = this.gclatencymode;
			...
			{
			    var oldInputAdjustTimeMs = TJAPlayer3._MainConfig.nInputAdjustTimeMs;
			    ...
			    TJAPlayer3._MainConfig.nInputAdjustTimeMs = newInputAdjustTimeMs;
			}
		...
		{
			public Chart.CChip HH;
			public Chart.CChip SD;
			public Chart.CChip BD;
			public Chart.CChip HT;
			public Chart.CChip LT;
			public Chart.CChip FT;
			public Chart.CChip CY;
			public Chart.CChip HHO;
			public Chart.CChip RD;
			public Chart.CChip LC;
            public Chart.CChip LP;
            public Chart.CChip LBD;
			public Chart.CChip this[ int index ]
			{
				get
				{
					switch ( index )
        ...
        protected StartFade actFI;
		protected BlackFade actFO;
		protected ResultFade actFOClear;
		...
		protected STDGBVALUE<Counter> ctチップ模様アニメ;
        public Counter[] ctChipAnime;
        public Counter[] ctChipAnimeLag;
		...
		protected int[] n最後に再生した実WAV番号 = new int[5];	// #26388 2011.11.8 yyagi: change "n最後に再生した実WAV番号.GUITAR" and "n最後に再生した実WAV番号.BASS"
																//							into "n最後に再生した実WAV番号";
...
//		protected STDGBVALUE<int> nJudgeLinePosY_delta;			// #31602 2013.6.23 yyagi 表示遅延対策として、判定ラインの表示位置をずらす機能を追加する
		...
		protected STDGBVALUE<Queue<Chart.CChip>> queWailing;
		protected STDGBVALUE<Chart.CChip> r現在の歓声Chip;
		protected FDKTexture txチップ;
		protected FDKTexture txヒットバー;

		protected FDKTexture tx背景;

//		protected int nRisky_InitialVar, nRiskyTime;		// #23559 2011.7.28 yyagi → CAct演奏ゲージ共通クラスに隠蔽
		protected int nPolyphonicSounds;
		protected List<Chart.CChip>[] listChip = new List<Chart.CChip>[4];
		protected Dictionary<int, Chart.CWAV> listWAV;
        ...
        protected Chart.CChip[] chip現在処理中の連打チップ = new Chart.CChip[ 4 ];
        ...
        protected FDKSound soundRed;
        protected FDKSound soundBlue;
        protected FDKSound soundAdlib;
...
//		protected GCLatencyMode gclatencymode;
		...
		public void AddMixer( FDKSound cs, bool _b演奏終了後も再生が続くチップである )
		{
...
//		Debug.WriteLine( "★Queue: remove " + Path.GetFileName( stm.csound.strファイル名 ));
			...
			if ( queueMixerSound.Count > 0 )
			{
				...
				if ( ts.Milliseconds > 7 )
				{
					for ( int i = 0; i < 2 && queueMixerSound.Count > 0; i++ )
					{
						...
						if ( stm.bIsAdd )
						{
							TJAPlayer3._SoundManager.AddMixer( stm.csound, db再生速度, stm.b演奏終了後も再生が続くチップである );
						}
						else
						{
							TJAPlayer3._SoundManager.RemoveMixer( stm.csound );
						}
					}
				}
			}
	    ...
	    internal E判定 e指定時刻からChipのJUDGEを返す(long nTime, Chart.CChip pChip)
	    {
		...
		private E判定 e指定時刻からChipのJUDGEを返すImpl( long nTime, Chart.CChip pChip )
		{
			if ( pChip != null )
			{
                ...
                {
				    if ( SoundManager.PlayTimer.NowTimeMs > pChip.n発声時刻ms && SoundManager.PlayTimer.NowTimeMs < pChip.nノーツ終了時刻ms )
				    {
					    return E判定.Perfect;
				    }
                }
                ...
                {
				    if ( SoundManager.PlayTimer.NowTimeMs >= pChip.n発声時刻ms - 17 && SoundManager.PlayTimer.NowTimeMs < pChip.nノーツ終了時刻ms )
				    {
					    return E判定.Perfect;
				    }
                }
				...
				{
                    if( TJAPlayer3._MainConfig.bJust )
                        return E判定.Poor;
					return E判定.Good;
				}
		...
		protected Chart.CChip r空うちChip( E楽器パート part, Eパッド pad )
		{
			return null;
		}
		protected Chart.CChip r指定時刻に一番近いChip_ヒット未済問わず不可視考慮( long nTime, int nChannel, int nInputAdjustTime, int nPlayer )
		{
			//sw2.Start();
//Trace.TraceInformation( "NTime={0}, nChannel={1:x2}", nTime, nChannel );
			...
			{
				Chart.CChip chip = listChip[ nPlayer ][ nIndex_NearestChip_Future ];
			...
			{
				Chart.CChip chip = listChip[ nPlayer ][ nIndex_NearestChip_Past ];
			...
			Chart.CChip nearestChip_Future = listChip[ nPlayer ][ nIndex_NearestChip_Future ];
			Chart.CChip nearestChip_Past   = listChip[ nPlayer ][ nIndex_NearestChip_Past ];
		...
		protected Chart.CChip r指定時刻に一番近い連打Chip_ヒット未済問わず不可視考慮( long nTime, int nChannel, int nInputAdjustTime, int nPlayer )
		{
			//sw2.Start();
//Trace.TraceInformation( "NTime={0}, nChannel={1:x2}", nTime, nChannel );
			nTime += nInputAdjustTime;						// #24239 2011.1.23 yyagi InputAdjust
            ...
            List<Chart.CChip> playerListChip = listChip[ nPlayer ];
			...
			for ( ; nIndex_NearestChip_Future < count; nIndex_NearestChip_Future++)
			{
				...
				{
                    Chart.CChip chip = playerListChip[ nIndex_NearestChip_Future ];
			...
			for ( ; nIndex_NearestChip_Past >= 0; nIndex_NearestChip_Past-- )
			{
				...
				{
                    Chart.CChip chip = playerListChip[ nIndex_NearestChip_Past ];
			...
			else if ( nIndex_NearestChip_Past < 0 )	// 検索対象が過去方向には見つからなかった(しかし未来方向には見つかった)場合
			{
				...
				return playerListChip[ nIndex_NearestChip_Future ];
													...
													// 検索対象が過去未来の双方に見つかったなら、より近い方を採用する
			Chart.CChip nearestChip_Future = playerListChip[ nIndex_NearestChip_Future ];
			Chart.CChip nearestChip_Past   = playerListChip[ nIndex_NearestChip_Past ];
			int nDiffTime_Future = Math.Abs( (int) ( nTime - nearestChip_Future.n発声時刻ms ) );
			int nDiffTime_Past   = Math.Abs( (int) ( nTime - nearestChip_Past.n発声時刻ms ) );
			if ( nDiffTime_Future >= nDiffTime_Past )
			{
		...
		protected void tサウンド再生( Chart.CChip pChip, int nPlayer )
		{
        ...
        protected bool tRollProcess( Chart.CChip pChip, double dbProcess_time, int num, int sort, int Input, int nPlayer )
        {
            if( dbProcess_time >= pChip.n発声時刻ms && dbProcess_time < pChip.nノーツ終了時刻ms )
            {
                ...
                if(this.actRoll.ct連打アニメ[nPlayer].IsEndValueNotReached)
                {
                    this.actRoll.ct連打アニメ[nPlayer] = new Counter(0, 9, 14, TJAPlayer3.Timer);
                    this.actRoll.ct連打アニメ[nPlayer].NowValue = 1;
                }
                else
                {
                    this.actRoll.ct連打アニメ[nPlayer] = new Counter(0, 9, 14, TJAPlayer3.Timer);
                ...
                if(pChip.RollEffectLevel >= 100)
                {
                    ...
                    pChip.RollInputTime = new Counter(0, 1500, 1, TJAPlayer3.Timer);
                    pChip.RollDelay?.Stop();
                } else
                {
                    pChip.RollInputTime = new Counter(0, 150, 1, TJAPlayer3.Timer);
                    pChip.RollDelay?.Stop(); 
                }
                ...
                if (pChip.bGOGOTIME && !TJAPlayer3._MainConfig.ShinuchiMode) //2018.03.11 kairera0467 チップに埋め込んだフラグから読み取る
                {
                    ...
                    {
                        if( pChip.nチャンネル番号 == 0x15 )
                            this.actScore.Add( E楽器パート.TAIKO, TJAPlayer3._MainConfig.AutoPlay[nPlayer], (long)( 300 * 1.2f ), nPlayer );
                        else
                            this.actScore.Add( E楽器パート.TAIKO, TJAPlayer3._MainConfig.AutoPlay[nPlayer], (long)( 360 * 1.2f ), nPlayer );
                    }
                    // 新配点
                    else
                    {
                        if( pChip.nチャンネル番号 == 0x15 )
                            this.actScore.Add( E楽器パート.TAIKO, TJAPlayer3._MainConfig.AutoPlay[nPlayer], (long)( 100 * 1.2f ), nPlayer );
                        else
                            this.actScore.Add( E楽器パート.TAIKO, TJAPlayer3._MainConfig.AutoPlay[nPlayer], (long)( 200 * 1.2f ), nPlayer );
                    }
                }
                else
                {
                    ...
                    {
                        if( pChip.nチャンネル番号 == 0x15 )
                            this.actScore.Add( E楽器パート.TAIKO, TJAPlayer3._MainConfig.AutoPlay[nPlayer], 300L, nPlayer );
                        else
                            this.actScore.Add( E楽器パート.TAIKO, TJAPlayer3._MainConfig.AutoPlay[nPlayer], 360L, nPlayer );
                    }
                    // 新配点
                    else
                    {
                        if( pChip.nチャンネル番号 == 0x15 )
                            this.actScore.Add( E楽器パート.TAIKO, TJAPlayer3._MainConfig.AutoPlay[nPlayer], 100L, nPlayer );
                        else
                            this.actScore.Add( E楽器パート.TAIKO, TJAPlayer3._MainConfig.AutoPlay[nPlayer], 200L, nPlayer );
                    }
                }
        ...
        protected bool tBalloonProcess( Chart.CChip pChip, double dbProcess_time, int player )
        {
            //if( dbProcess_time >= pChip.n発声時刻ms && dbProcess_time < pChip.nノーツ終了時刻ms )
            if ((int)SoundManager.PlayTimer.NowTimeMs >= pChip.n発声時刻ms && (int)SoundManager.PlayTimer.NowTimeMs <= pChip.nノーツ終了時刻ms)
            {
                if( pChip.nRollCount == 0 )
                {
                ...
                if(actChara.CharaAction_Balloon_Breaking != null && player == 0)
                {
                    ...
                    actChara.CharaAction_Balloon_Breaking = new Counter(0, TJAPlayer3.Skin.SkinValue.Game_Chara_Ptn_Balloon_Breaking - 1, TJAPlayer3.Skin.SkinValue.Game_Chara_Balloon_Timer, TJAPlayer3.Timer);
                   
                }
                if (this.actBalloon.ct風船アニメ[player].IsEndValueNotReached)
                {
                    this.actBalloon.ct風船アニメ[player] = new Counter(0, 9, 14, TJAPlayer3.Timer);
                    this.actBalloon.ct風船アニメ[player].NowValue = 1;
                }
                else
                {
                    this.actBalloon.ct風船アニメ[player] = new Counter(0, 9, 14, TJAPlayer3.Timer);
                ...
                if ( pChip.nBalloon == pChip.nRollCount )
                {
                    ...
                    if(pChip.bGOGOTIME && !TJAPlayer3._MainConfig.ShinuchiMode)
                    {
                        this.actScore.Add(E楽器パート.TAIKO, TJAPlayer3._MainConfig.AutoPlay[player], 6000L, player);
                    } else
                    {
                        this.actScore.Add(E楽器パート.TAIKO, TJAPlayer3._MainConfig.AutoPlay[player], 5000L, player);
                    }
                    pChip.bHit = true;
                    pChip.IsHitted = true;
                    ...
                    if (actChara.CharaAction_Balloon_Broke != null && player == 0)
                    {
                        actChara.アクションタイマーリセット();
                        actChara.bマイどんアクション中 = true;
                        actChara.CharaAction_Balloon_Broke = new Counter(0, TJAPlayer3.Skin.SkinValue.Game_Chara_Ptn_Balloon_Broke - 1, TJAPlayer3.Skin.SkinValue.Game_Chara_Balloon_Timer, TJAPlayer3.Timer);
                        if(actChara.CharaAction_Balloon_Delay != null )actChara.CharaAction_Balloon_Delay = new Counter(0, TJAPlayer3.Skin.SkinValue.Game_Chara_Balloon_Delay - 1, 1, TJAPlayer3.Timer);
                    }
                ...
                else
                {
                    if(pChip.bGOGOTIME && !TJAPlayer3._MainConfig.ShinuchiMode)
                    {
                        this.actScore.Add(E楽器パート.TAIKO, TJAPlayer3._MainConfig.AutoPlay[player], 360L, player);
                    } else
                    {
                        this.actScore.Add(E楽器パート.TAIKO, TJAPlayer3._MainConfig.AutoPlay[player], 300L, player);
                    }
                    //CDTXMania.Skin.soundRed.t再生する();
                    this.soundRed?.t再生を開始する();
                }
		...
		protected E判定 tチップのヒット処理( long nHitTime, Chart.CChip pChip )
		{
			return tチップのヒット処理( nHitTime, pChip, true );
		}
		protected abstract E判定 tチップのヒット処理( long nHitTime, Chart.CChip pChip, bool bCorrectLane );
		protected E判定 tチップのヒット処理( long nHitTime, Chart.CChip pChip, E楽器パート screenmode )		// E楽器パート screenmode
		{
			return tチップのヒット処理( nHitTime, pChip, screenmode, true, 1, 0 );
		}
		protected E判定 tチップのヒット処理( long nHitTime, Chart.CChip pChip, E楽器パート screenmode, bool bCorrectLane, int nNowInput )
		{
			return tチップのヒット処理( nHitTime, pChip, screenmode, bCorrectLane, nNowInput, 0 );
		}
		protected unsafe E判定 tチップのヒット処理( long nHitTime, Chart.CChip pChip, E楽器パート screenmode, bool bCorrectLane, int nNowInput, int nPlayer )
		{
            ...
            {
                case 0:
                    bAutoPlay = TJAPlayer3._MainConfig.b太鼓パートAutoPlay;
                    break;
                case 1:
                    bAutoPlay = TJAPlayer3._MainConfig.b太鼓パートAutoPlay2P;
                    break;
            }
				...
				case E楽器パート.TAIKO:
					{
                        ...
                        if ( pChip.nチャンネル番号 == 0x15 || pChip.nチャンネル番号 == 0x16 )
                        {
                            ...
                            if( bAutoPlay )
                            {
                                if( TJAPlayer3._MainConfig.bAuto先生の連打 )
                                {
                                    if( SoundManager.PlayTimer.NowTimeMs > ( pChip.n発声時刻ms + ( 1000.0 / 15.0 ) * pChip.nRollCount ) )
                                    {
                                        if( this.nHand[ nPlayer ] == 0 )
                                            this.nHand[ nPlayer ]++;
                                        else
                                        ...
                                        this.tRollProcess( pChip, SoundManager.PlayTimer.NowTimeMs, 1, 0, 0, nPlayer );
                                    }
                                }
                            }
                            else
                            {
                                this.eRollState = E連打State.roll;
                                this.tRollProcess( pChip, SoundManager.PlayTimer.NowTimeMs, 1, nNowInput, 0, nPlayer );
                            }
                        ...
                        else if( pChip.nチャンネル番号 == 0x17 )
                        {
                            ...
                            if( bAutoPlay )
                            {
                                if( pChip.nBalloon != 0 )
                                {
                                    if( SoundManager.PlayTimer.NowTimeMs > ( pChip.n発声時刻ms + ( ( pChip.nノーツ終了時刻ms - pChip.n発声時刻ms ) / pChip.nBalloon ) * pChip.nRollCount ) )
                                    {
                                        if( this.nHand[ nPlayer ] == 0 )
                                            this.nHand[ nPlayer ]++;
                                        else
                                        ...
                                        this.tBalloonProcess( pChip, SoundManager.PlayTimer.NowTimeMs, nPlayer );
                                    }
                                }
                            }
                            else
                        ...
                        {
                            if( pChip.nノーツ終了時刻ms <= SoundManager.PlayTimer.NowTimeMs )
                            {
                                this.b連打中[ nPlayer ] = false;
                                //this.actChara.b風船連打中 = false;
                                pChip.bHit = true;
                        ...
                        if( eJudgeResult != E判定.Auto && eJudgeResult != E判定.Miss )
                        {
                            ...
                            if( TJAPlayer3._MainConfig.b太鼓パートAutoPlay ? true : ( nNowInput == 2 || nNowInput == 3 ) )
                            {
            ...
            if ( eJudgeResult != E判定.Poor && eJudgeResult != E判定.Miss )
            {
                ...
                {
                    if(TJAPlayer3.Skin.SkinValue.Game_Chara_Ptn_SoulIn != 0 && nPlayer == 0 && actChara.CharaAction_Balloon_Delay.IsEndValueReached)
                    {
                        this.actChara.アクションタイマーリセット();
                        this.actChara.ctキャラクターアクション_魂MAX = new Counter(0, TJAPlayer3.Skin.SkinValue.Game_Chara_Ptn_SoulIn - 1, (dbUnit / TJAPlayer3.Skin.SkinValue.Game_Chara_Ptn_SoulIn) * 2, SoundManager.PlayTimer);
                        this.actChara.ctキャラクターアクション_魂MAX.Tick_Double();
                        this.actChara.ctキャラクターアクション_魂MAX.NowValue_Double = 0D;
                ...
                {
                    if(TJAPlayer3.Skin.SkinValue.Game_Chara_Ptn_ClearIn != 0 && nPlayer == 0 && actChara.CharaAction_Balloon_Delay.IsEndValueReached)
                    {
                        this.actChara.アクションタイマーリセット();
                        this.actChara.ctキャラクターアクション_ノルマ = new Counter(0, TJAPlayer3.Skin.SkinValue.Game_Chara_Ptn_ClearIn - 1, (dbUnit / TJAPlayer3.Skin.SkinValue.Game_Chara_Ptn_ClearIn) * 2, SoundManager.PlayTimer);
                        this.actChara.ctキャラクターアクション_ノルマ.Tick_Double();
                        this.actChara.ctキャラクターアクション_ノルマ.NowValue_Double = 0D;
                        this.actChara.bマイどんアクション中 = true;
                    }
                ...
                case E楽器パート.TAIKO:
                            ...
                            case E判定.Perfect:
                                {
                                    ...
                                    if (this.actCombo.ctコンボ加算[nPlayer].IsEndValueNotReached)
                                    {
                                        this.actCombo.ctコンボ加算[nPlayer].NowValue = 1;
                                    }
                                    else
                                    {
                                        this.actCombo.ctコンボ加算[nPlayer].NowValue = 0;
                                    }
                                }
                                break;
                            case E判定.Great:
                    ...
                    if( pChip.nチャンネル番号 < 0x15 || ( pChip.nチャンネル番号 >= 0x1A ) )
                    {
                        ...
                        for (int i = 0; i < 2; i++)
                        {
                            ...
                            {
                                ctChipAnimeLag[i] = new Counter(0, 664, 1, TJAPlayer3.Timer);
                            }
                        }
                        ...
                        if (this.actCombo.n現在のコンボ数[0] % 10 == 0 && this.actCombo.n現在のコンボ数[0] > 0)
                        {
                            ...
                            //{
                                ...
                                {
                                    if (TJAPlayer3.Skin.SkinValue.Game_Chara_Ptn_10combo != 0 && nPlayer == 0 && !this.actChara.ctキャラクターアクション_ノルマ.IsProcessed_Double && actChara.CharaAction_Balloon_Delay.IsEndValueReached)
                                    {
                                        if (TJAPlayer3.stage演奏ドラム画面.actGauge.db現在のゲージ値[0] < 100)
                                        {
                                            ...
                                            // ジャンプ_ノーマル
                                            this.actChara.アクションタイマーリセット();
                                            this.actChara.ctキャラクターアクション_10コンボ = new Counter(0, TJAPlayer3.Skin.SkinValue.Game_Chara_Ptn_10combo - 1, (dbUnit / TJAPlayer3.Skin.SkinValue.Game_Chara_Ptn_10combo) * 2, SoundManager.PlayTimer);
                                            this.actChara.ctキャラクターアクション_10コンボ.Tick_Double();
                                            this.actChara.ctキャラクターアクション_10コンボ.NowValue_Double = 0D;
                                            this.actChara.bマイどんアクション中 = true;
                                            //this.actChara.マイどん_アクション_10コンボ();
                                        }
                                    }
                                    if (TJAPlayer3.Skin.SkinValue.Game_Chara_Ptn_10combo_Max != 0 && nPlayer == 0 && !this.actChara.ctキャラクターアクション_魂MAX.IsProcessed_Double && actChara.CharaAction_Balloon_Delay.IsEndValueReached)
                                    {
                                        if (TJAPlayer3.stage演奏ドラム画面.actGauge.db現在のゲージ値[0] >= 100)
                                        {
                                            // 魂ゲージMAX
			...
			if ( ( ( pChip.e楽器パート != E楽器パート.UNKNOWN ) ) && ( eJudgeResult != E判定.Miss ) && ( eJudgeResult != E判定.Bad ) && ( eJudgeResult != E判定.Poor ) && ( pChip.nチャンネル番号 <= 0x14 || pChip.nチャンネル番号 == 0x1A || pChip.nチャンネル番号 == 0x1B ) )
			{
                ...
                if( TJAPlayer3._MainConfig.ShinuchiMode )  //2016.07.04 kairera0467 真打モード。
                {
                    ...
                    this.actScore.Add( E楽器パート.TAIKO, TJAPlayer3._MainConfig.AutoPlay[nPlayer], nAddScore, nPlayer );
                }
                else if( TJAPlayer3.DTX.nScoreModeTmp == 2 )
                {
                    ...
                    {
                        if(this.actScore.ctボーナス加算タイマ[nPlayer].IsProcessed)
                        {
                            this.actScore.ctボーナス加算タイマ[nPlayer].Stop();
                            this.actScore.BonusAdd(nPlayer);
                        }
                        this.actScore.ctボーナス加算タイマ[nPlayer].NowValue = 0;
                        this.actScore.ctボーナス加算タイマ[nPlayer] = new Counter(0, 2, 1000, TJAPlayer3.Timer);
		...
		protected Chart.CChip r指定時刻に一番近いChipを過去方向優先で検索する( long nTime, int nChannel, int nInputAdjustTime, int nPlayer )
		{
			//sw2.Start();
...
//			while ( nIndex_NearestChip_Past >= 0 )		// 過去方向への検索
			...
			Chart.CChip nearestChip;	// = null;	// 以下のifブロックのいずれかで必ずnearestChipには非nullが代入されるので、null初期化を削除
...
//					nTimeDiff = Math.Abs( (int) ( nTime - nearestChip.n発声時刻ms ) );
		...
		protected Chart.CChip r指定時刻に一番近い未ヒットChipを過去方向優先で検索する( long nTime, int nPlayer )
		{
			//sw2.Start();
			int nIndex_InitialPositionSearchingToPast;
			int nTimeDiff;
...
//					nTimeDiff = Math.Abs( (int) ( nTime - nearestChip.n発声時刻ms ) );
        ...
        protected Chart.CChip GetChipOfNearest(long nowTime, int player)
        {
            var nearestChip = new Chart.CChip();
            var count = listChip[player].Count;
            var chips = listChip[player];
            var startPosision = NowProcessingChip[player];
            Chart.CChip pastChip; // 判定されるべき過去ノート
            Chart.CChip futureChip; // 判定されるべき未来ノート
            ...
            if(player == 0)
            {
                TJAPlayer3._ConsoleText.tPrint(0, 0, ConsoleText.FontType.White, pastChip != null ? pastChip.ToString() : "null");
                TJAPlayer3._ConsoleText.tPrint(0, 20, ConsoleText.FontType.White, futureChip != null ? futureChip.ToString() : "null");
                TJAPlayer3._ConsoleText.tPrint(0, 40, ConsoleText.FontType.White, nearestChip != null ? nearestChip.ToString() : "null");
                TJAPlayer3._ConsoleText.tPrint(0, 60, ConsoleText.FontType.White, startPosision.ToString());
        ...
        /// <returns>最も判定枠に近いノーツ。</returns>
        protected Chart.CChip GetChipOfNearest(long nowTime, int player, bool don)
        {
            var nearestChip = new Chart.CChip();
            var count = listChip[player].Count;
            var chips = listChip[player];
        ...
        protected Chart.CChip r指定時刻に一番近い未ヒットChip( long nTime, int nChannel, int nInputAdjustTime, int n検索範囲時間ms, int nPlayer )
		{
			//sw2.Start();
...
//					nTimeDiff = Math.Abs( (int) ( nTime - nearestChip.n発声時刻ms ) );
		...
		public bool r検索範囲内にチップがあるか調べる( long nTime, int nInputAdjustTime, int n検索範囲時間ms, int nPlayer )
		{
			...
			{
				Chart.CChip chip = listChip[ nPlayer ][ i ];
				if ( !chip.bHit )
				{
		...
		protected void ChangeInputAdjustTimeInPlaying( IInputDevice keyboard, int plusminus )		// #23580 2011.1.16 yyagi UI for InputAdjustTime in playing screen.
		{
		    ...
		    var newInputAdjustTimeMs = (TJAPlayer3._MainConfig.nInputAdjustTimeMs + offset).Clamp(-99, 99);
			TJAPlayer3._MainConfig.nInputAdjustTimeMs = newInputAdjustTimeMs;
		}
		...
		protected void tキー入力()
		{
			...
			if ( keyboard.GetKeyPressed( (int) SlimDX.DirectInput.Key.F1 ) &&
				( keyboard.GetKeyKeepPressed( (int) SlimDX.DirectInput.Key.RightShift ) || keyboard.GetKeyKeepPressed( (int) SlimDX.DirectInput.Key.LeftShift ) ) )
			{	// shift+f1 (pause)
			...
			if ( ( !this.bPAUSE && ( base.eフェーズID != BaseScene.Eフェーズ.演奏_STAGE_FAILED ) ) && ( base.eフェーズID != BaseScene.Eフェーズ.演奏_STAGE_FAILED_フェードアウト ) )
			{
				this.t入力処理_ドラム();
				if ( keyboard.GetKeyPressed( (int) SlimDX.DirectInput.Key.UpArrow ) && ( keyboard.GetKeyKeepPressed( (int) SlimDX.DirectInput.Key.RightShift ) || keyboard.GetKeyKeepPressed( (int) SlimDX.DirectInput.Key.LeftShift ) ) )
				{	// shift (+ctrl) + UpArrow (BGMAdjust)
					TJAPlayer3.DTX.t各自動再生音チップの再生時刻を変更する( ( keyboard.GetKeyKeepPressed( (int) SlimDX.DirectInput.Key.LeftControl ) || keyboard.GetKeyKeepPressed( (int) SlimDX.DirectInput.Key.RightControl ) ) ? 1 : 10 );
					TJAPlayer3.DTX.tWave再生位置自動補正();
				}
				else if ( keyboard.GetKeyPressed( (int) SlimDX.DirectInput.Key.DownArrow ) && ( keyboard.GetKeyKeepPressed( (int) SlimDX.DirectInput.Key.RightShift ) || keyboard.GetKeyKeepPressed( (int) SlimDX.DirectInput.Key.LeftShift ) ) )
				{	// shift + DownArrow (BGMAdjust)
					TJAPlayer3.DTX.t各自動再生音チップの再生時刻を変更する( ( keyboard.GetKeyKeepPressed( (int) SlimDX.DirectInput.Key.LeftControl ) || keyboard.GetKeyKeepPressed( (int) SlimDX.DirectInput.Key.RightControl ) ) ? -1 : -10 );
					TJAPlayer3.DTX.tWave再生位置自動補正();
				}
				else if ( keyboard.GetKeyPressed( (int) SlimDX.DirectInput.Key.UpArrow ) )
				{	// UpArrow(scrollspeed up)
					ドラムスクロール速度アップ();
				}
				else if ( keyboard.GetKeyPressed( (int) SlimDX.DirectInput.Key.DownArrow ) )
				{	// DownArrow (scrollspeed down)
					ドラムスクロール速度ダウン();
				}
				else if ( keyboard.GetKeyPressed( (int) SlimDX.DirectInput.Key.Delete ) )
				{	// del (debug info)
					TJAPlayer3._MainConfig.b演奏情報を表示する = !TJAPlayer3._MainConfig.b演奏情報を表示する;
				}
				else if ( keyboard.GetKeyPressed( (int) SlimDX.DirectInput.Key.LeftArrow ) )		// #24243 2011.1.16 yyagi UI for InputAdjustTime in playing screen.
				{
					ChangeInputAdjustTimeInPlaying( keyboard, -1 );
				}
				else if ( keyboard.GetKeyPressed( (int) SlimDX.DirectInput.Key.RightArrow ) )		// #24243 2011.1.16 yyagi UI for InputAdjustTime in playing screen.
				{
					ChangeInputAdjustTimeInPlaying( keyboard, +1 );
				}
				else if ( ( base.eフェーズID == BaseScene.Eフェーズ.共通_通常状態 ) && ( keyboard.GetKeyPressed( (int) SlimDX.DirectInput.Key.Escape ) ) && !this.actPauseMenu.bIsActivePopupMenu )
				{	// escape (exit)
                    this.t演奏中止();
				}
				...
				if ( keyboard.GetKeyPressed( (int) SlimDX.DirectInput.Key.F4 ) )
				{
                    if( TJAPlayer3._MainConfig.bJudgeCountDisplay == false )
                        TJAPlayer3._MainConfig.bJudgeCountDisplay = true;
                    else
                        TJAPlayer3._MainConfig.bJudgeCountDisplay = false;
				}
				else if ( keyboard.GetKeyPressed( (int) SlimDX.DirectInput.Key.F5 ) )
				{
                    switch( TJAPlayer3._MainConfig.eClipDispType  )
                    {
                        case EClipDispType.OFF:
                            TJAPlayer3._MainConfig.eClipDispType = EClipDispType.背景のみ;
                            break;
                        case EClipDispType.背景のみ:
                            TJAPlayer3._MainConfig.eClipDispType = EClipDispType.ウィンドウのみ;
                            break;
                        case EClipDispType.ウィンドウのみ:
                            TJAPlayer3._MainConfig.eClipDispType = EClipDispType.両方;
                            break;
                        case EClipDispType.両方:
                            TJAPlayer3._MainConfig.eClipDispType = EClipDispType.OFF;
                            break;
                    }
				}
#if DEBUG
                if ( keyboard.GetKeyPressed( (int) SlimDX.DirectInput.Key.F6 ) )
                {
                    if( TJAPlayer3._MainConfig.b太鼓パートAutoPlay == false )
                        TJAPlayer3._MainConfig.b太鼓パートAutoPlay = true;
                    else
                        TJAPlayer3._MainConfig.b太鼓パートAutoPlay = false;
                }
                if ( keyboard.GetKeyPressed( (int) SlimDX.DirectInput.Key.F7 ) )
                {
                    if(TJAPlayer3._MainConfig.b太鼓パートAutoPlay2P == false )
                        TJAPlayer3._MainConfig.b太鼓パートAutoPlay2P = true;
                    else
                        TJAPlayer3._MainConfig.b太鼓パートAutoPlay2P = false;
                }
#endif
			}
            if( !this.actPauseMenu.bIsActivePopupMenu && this.bPAUSE && ( ( base.eフェーズID != BaseScene.Eフェーズ.演奏_STAGE_FAILED ) ) && ( base.eフェーズID != BaseScene.Eフェーズ.演奏_STAGE_FAILED_フェードアウト ) )
			{
				if ( keyboard.GetKeyPressed( (int) SlimDX.DirectInput.Key.UpArrow ) )
				{	// UpArrow(scrollspeed up)
					ドラムスクロール速度アップ();
				}
				else if ( keyboard.GetKeyPressed( (int) SlimDX.DirectInput.Key.DownArrow ) )
...
#region [ Minus & Equals Sound Group Level ]
		...
		{
			if ( ( ( base.eフェーズID != BaseScene.Eフェーズ.演奏_STAGE_FAILED ) && ( base.eフェーズID != BaseScene.Eフェーズ.演奏_STAGE_FAILED_フェードアウト ) ) && ( !TJAPlayer3._MainConfig.bストイックモード && TJAPlayer3._MainConfig.bAVI有効 ) )
			{
		...
		{
			if ( base.eフェーズID != BaseScene.Eフェーズ.演奏_STAGE_FAILED )
			{
				BaseScene.Eフェーズ eフェーズid1 = base.eフェーズID;
			}
		}
		protected void t進行描画_STAGEFAILED()
		{
			if ( ( ( base.eフェーズID == BaseScene.Eフェーズ.演奏_STAGE_FAILED ) || ( base.eフェーズID == BaseScene.Eフェーズ.演奏_STAGE_FAILED_フェードアウト ) ) && ( ( this.actStageFailed.Draw() != 0 ) && ( base.eフェーズID != BaseScene.Eフェーズ.演奏_STAGE_FAILED_フェードアウト ) ) )
			{
				this.eフェードアウト完了時の戻り値 = E演奏画面の戻り値.ステージ失敗;
				base.eフェーズID = BaseScene.Eフェーズ.演奏_STAGE_FAILED_フェードアウト;
		...
		{
			this.actChipFireGB.Draw();
		}
		protected abstract void t進行描画_パネル文字列();
		protected void t進行描画_パネル文字列( int x, int y )
		{
			if ( ( base.eフェーズID != BaseScene.Eフェーズ.演奏_STAGE_FAILED ) && ( base.eフェーズID != BaseScene.Eフェーズ.演奏_STAGE_FAILED_フェードアウト ) )
			{
		...
		protected void tパネル文字列の設定()
		{
		    ...
		    this.actPanel.SetPanelString( panelString, TJAPlayer3.stage選曲.str確定された曲のジャンル, TJAPlayer3.Skin.SkinValue.Game_StageText );
		}
		...
		protected bool t進行描画_チップ( E楽器パート ePlayMode, int nPlayer )
		{
			...
			var n現在時刻ms = SoundManager.PlayTimer.NowTimeMs;
			...
			MainConfig configIni = TJAPlayer3._MainConfig;
			...
			{
				Chart.CChip pChip = dTX.listChip[ nCurrentTopChip ];
...
#region [ 01: BGM ]
					case 0x01:	// BGM
						if ( !pChip.bHit && ( pChip.nバーからの距離dot.Drums < 0 ) )
						{
							...
							{
								dTX.tチップの再生( pChip, SoundManager.PlayTimer.n前回リセットした時のシステム時刻 + pChip.n発声時刻ms, (int) Eレーン.BGM );
							}
						}
						break;
#endregion
                                ...
                                if( chip現在処理中の連打チップ[ nPlayer ] != null )
                                {
                                    ...
                                    if (nPlayer == 0 && chip現在処理中の連打チップ[nPlayer].nBalloon > chip現在処理中の連打チップ[nPlayer].nRollCount && chip現在処理中の連打チップ[nPlayer].nRollCount > 0 && actChara.CharaAction_Balloon_Miss != null)
                                    {
                                        if (TJAPlayer3.Skin.SkinValue.Game_Chara_Ptn_Balloon_Miss > 0)
                                        {
                                            actChara.アクションタイマーリセット();
                                            actChara.bマイどんアクション中 = true;
                                            actChara.CharaAction_Balloon_Miss = new Counter(0, TJAPlayer3.Skin.SkinValue.Game_Chara_Ptn_Balloon_Miss - 1, TJAPlayer3.Skin.SkinValue.Game_Chara_Balloon_Timer, TJAPlayer3.Timer);
                                            if (actChara.CharaAction_Balloon_Delay != null) actChara.CharaAction_Balloon_Delay = new Counter(0, TJAPlayer3.Skin.SkinValue.Game_Chara_Balloon_Delay - 1, 1, TJAPlayer3.Timer);
                                        }
                                    }
					...
					case 0x50:	// 小節線
						{
                            if ( !pChip.bHit && ( pChip.nバーからの距離dot.Taiko < 0 ) )
						    {
                                ...
                                if( this.actPlayInfo.NowMeasure[nPlayer] == 0 )
                                {
                                    for (int i = 0; i < 2; i++)
                                    {
                                        ctChipAnime[i] = new Counter(0, 3, 60.0 / TJAPlayer3.stage演奏ドラム画面.actPlayInfo.dbBPM * 1 / 4, SoundManager.PlayTimer);
                                    }
                                    ...
                                    this.ct制御タイマ = new Counter(0, 10, 500, SoundManager.PlayTimer);
                                    ...
                                    if (TJAPlayer3.Skin.SkinValue.Game_Mob_Ptn != 0 && TJAPlayer3.Skin.SkinValue.Game_Mob_Beat > 0 ) //2018.6.15 Game_Mob_Beatが0のままCCounter生成をされて無限ループが発生しないよう対策
                                    {
                                        this.actMob.ctMob = new Counter(1, 180, 60.0 / TJAPlayer3.stage演奏ドラム画面.actPlayInfo.dbBPM * TJAPlayer3.Skin.SkinValue.Game_Mob_Beat / 180, SoundManager.PlayTimer);
                                        this.actMob.ctMobPtn = new Counter(0, TJAPlayer3.Skin.SkinValue.Game_Mob_Ptn - 1, 60.0 / TJAPlayer3.stage演奏ドラム画面.actPlayInfo.dbBPM * TJAPlayer3.Skin.SkinValue.Game_Mob_Ptn_Beat / TJAPlayer3.Skin.SkinValue.Game_Mob_Ptn, SoundManager.PlayTimer);
                                    }
                                    else
                                    {
                                        this.actMob.ctMob = new Counter();
                                        this.actMob.ctMobPtn = new Counter();
                                    }
...
#region[ 9B-9F: 太鼓 ]
                    ...
                    case 0x9C:
						...
						if ( !pChip.bHit && ( pChip.nバーからの距離dot.Drums < 0 ) )
						{
                            ...
                            if( pChip.nコース == this.n現在のコース[ nPlayer ] )
                            {
							    if ( dTX.listBPM.TryGetValue( pChip.n整数値_内部番号, out Chart.CBPM cBPM ) )
							    {
...
//#endif
                    ...
                    case 0x9E: //ゴーゴータイム
                        if( !pChip.bHit && ( pChip.nバーからの距離dot.Drums < 0 ) )
                        {
                            ...
                            if(nPlayer == 0)
                            {
                                if (TJAPlayer3.Skin.SkinValue.Game_Chara_Ptn_GoGoStart != 0 && nPlayer == 0 && actChara.CharaAction_Balloon_Delay.IsEndValueReached)
                                {
                                    if (TJAPlayer3.stage演奏ドラム画面.actGauge.db現在のゲージ値[0] < 100)
                                    {
                                        // 魂ゲージMAXではない
                                        // ゴーゴースタート_ノーマル
                                        this.actChara.アクションタイマーリセット();
                                        this.actChara.ctキャラクターアクション_ゴーゴースタート = new Counter(0, TJAPlayer3.Skin.SkinValue.Game_Chara_Ptn_GoGoStart - 1, (dbUnit / TJAPlayer3.Skin.SkinValue.Game_Chara_Ptn_GoGoStart) * 2, SoundManager.PlayTimer);
                                        this.actChara.ctキャラクターアクション_ゴーゴースタート.Tick_Double();
                                        this.actChara.ctキャラクターアクション_ゴーゴースタート.NowValue_Double = 0D;
                                        this.actChara.bマイどんアクション中 = true;
                                        //this.actChara.マイどん_アクション_10コンボ();
                                    }
                                }
                                if (TJAPlayer3.Skin.SkinValue.Game_Chara_Ptn_GoGoStart_Max != 0 && nPlayer == 0 && actChara.CharaAction_Balloon_Delay.IsEndValueReached)
                                {
                                    if (TJAPlayer3.stage演奏ドラム画面.actGauge.db現在のゲージ値[0] >= 100)
                                    {
                                        // 魂ゲージMAX
                                        // ゴーゴースタート_MAX
                                        this.actChara.アクションタイマーリセット();
                                        this.actChara.ctキャラクターアクション_ゴーゴースタートMAX = new Counter(0, TJAPlayer3.Skin.SkinValue.Game_Chara_Ptn_GoGoStart_Max - 1, (dbUnit / TJAPlayer3.Skin.SkinValue.Game_Chara_Ptn_GoGoStart_Max) * 2, SoundManager.PlayTimer);
                                        this.actChara.ctキャラクターアクション_ゴーゴースタートMAX.Tick_Double();
                                        this.actChara.ctキャラクターアクション_ゴーゴースタートMAX.NowValue_Double = 0D;
                                        this.actChara.bマイどんアクション中 = true;
                                    }
                                }
...
//Debug.WriteLine( "[DA(AddMixer)] BAR=" + pChip.n発声位置 / 384 + " ch=" + pChip.nチャンネル番号.ToString( "x2" ) + ", wav=" + pChip.n整数値.ToString( "x2" ) + ", time=" + pChip.n発声時刻ms );
							pChip.bHit = true;
							if ( listWAV.TryGetValue( pChip.n整数値_内部番号, out Chart.CWAV wc ) )	// 参照が遠いので後日最適化する
							{
								for ( int i = 0; i < nPolyphonicSounds; i++ )
								{
									if ( wc.rSound[ i ] != null )
							...
							if ( listWAV.TryGetValue( pChip.n整数値_内部番号, out Chart.CWAV wc ) )	// 参照が遠いので後日最適化する
							{
							    for ( int i = 0; i < nPolyphonicSounds; i++ )
							    {
									if ( wc.rSound[ i ] != null )
									{
										...
										if ( !wc.rSound[ i ].IsContinueToPlayAfterTheEnd )	// #32248 2013.10.16 yyagi
...
#region[ 15-19: 連打 ]
        ...
        public void tBranchJudge( Chart dTX, int n連打数, int n良, int n可, int n不可, int nPlayer )
        {
        ...
        public int nBranchJudge( Chart dTX, int n連打数, int n良, int n可, int n不可 )
        {
        ...
        public void t分岐処理( int n分岐先, int n分岐開始位置のChip番号, int nPlayer )
        {
            ...
            Chart.CChip chipNoHit = this.r指定時刻に一番近い未ヒットChip( SoundManager.PlayTimer.NowTimeMs + 1, 0xDF, 0, 10000, nPlayer ); //とりあえず検索開始を1ms後にして対応。
            if( chipNoHit == null )
                return;
        ...
        protected float GetNowPBMTime( Chart tja, float play_time )
        {
            float bpm_time = 0;
            int last_input = 0;
            float last_bpm_change_time;
            play_time = SoundManager.PlayTimer.NowTimeMs - tja.nOFFSET;
            ...
            for (int i = 1; ; i++)
            {
                ...
                {
                    Chart.CBPM cBPM = tja.listBPM[ last_input ];
		...
		public void t再読込()
		{
			...
			base.eフェーズID = BaseScene.Eフェーズ.演奏_再読込;
			this.bPAUSE = false;
		}
		...
		public void t停止()
		{
			...
			TJAPlayer3.Timer.Stop();		// 再生時刻カウンタ停止
        ...
        public void t数値の初期化( bool b演奏記録, bool b演奏状態 )
        {
            if( b演奏記録 )
            {
                ...
                this.actGauge.Init( TJAPlayer3._MainConfig.nRisky );
            }
            if( b演奏状態 )
            {
                for( int i = 0; i < TJAPlayer3._MainConfig.nPlayerCount; i++ )
...
#region [ 演奏済みフラグのついたChipをリセットする ]
			...
			{
				Chart.CChip pChip = dTX.listChip[ i ];
				if ( pChip.bHit )
				{
					Chart.CChip p = (Chart.CChip) pChip.Clone();
					p.bHit = false;
                    p.IsHitted = false;
...
#region [ 演奏開始の発声時刻msを取得し、タイマに設定 ]
			...
			//}
			SoundManager.PlayTimer.n現在時刻 = nStartTime;
#endregion

			List<FDKSound> pausedCSound = new List<FDKSound>();
			...
			for ( int i = this.n現在のトップChip; i >= 0; i-- )
			{
				...
				if ( ( pChip.n発声時刻ms + nDuration > 0 ) && ( pChip.n発声時刻ms <= nStartTime ) && ( nStartTime <= pChip.n発声時刻ms + nDuration ) )
				{
					if ( pChip.bWAVを使うチャンネルである && ( pChip.nチャンネル番号 >> 4 ) != 0xB )	// wav系チャンネル、且つ、空打ちチップではない
					{
						...
						if ( ( wc.bIsBGMSound && TJAPlayer3._MainConfig.bBGM音を発声する ) || ( !wc.bIsBGMSound ) )
						{
							TJAPlayer3.DTX.tチップの再生( pChip, SoundManager.PlayTimer.n前回リセットした時のシステム時刻 + pChip.n発声時刻ms, (int) Eレーン.BGM );
#region [ PAUSEする ]
							int j = wc.n現在再生中のサウンド番号;
							if ( wc.rSound[ j ] != null )
							{
							    wc.rSound[ j ].t再生を一時停止する();
							    wc.rSound[ j ].SetPosition( nStartTime - pChip.n発声時刻ms );
...
#region [ PAUSEしていたサウンドを一斉に再生再開する(ただしタイマを止めているので、ここではまだ再生開始しない) ]
			foreach ( FDKSound cs in pausedCSound )
			{
				cs.PlaySound();
			}
			pausedCSound.Clear();
...
#region [ タイマを再開して、PAUSEから復帰する ]
		...
		{
			TJAPlayer3._MainConfig.bAVI有効 = true;
			TJAPlayer3._MainConfig.bBGA有効 = true;
			for ( int i = 0; i < 3; i++ )
			{
				TJAPlayer3._MainConfig.bGraph[ i ] = false;
				TJAPlayer3._MainConfig.bHidden[ i ] = false;
				TJAPlayer3._MainConfig.bLeft[ i ] = false;
				TJAPlayer3._MainConfig.bLight[ i ] = false;
				TJAPlayer3._MainConfig.bReverse[ i ] = false;
				TJAPlayer3._MainConfig.bSudden[ i ] = false;
				TJAPlayer3._MainConfig.eInvisible[ i ] = EInvisible.OFF;
				TJAPlayer3._MainConfig.eRandom[ i ] = Eランダムモード.OFF;
				TJAPlayer3._MainConfig.n表示可能な最小コンボ数[ i ] = 65535;
				TJAPlayer3._MainConfig.判定文字表示位置[ i ] = E判定文字表示位置.表示OFF;
				// CDTXMania.ConfigIni.n譜面スクロール速度[ i ] = CDTXMania.ConfigIni.nViewerScrollSpeed[ i ];	// これだけはOn活性化()で行うこと。
																												// そうしないと、演奏開始直後にスクロール速度が変化して見苦しい。
			}

			TJAPlayer3._MainConfig.eDark = Eダークモード.OFF;

			TJAPlayer3._MainConfig.b演奏情報を表示する = TJAPlayer3._MainConfig.bViewerShowDebugStatus;
			TJAPlayer3._MainConfig.bScoreIniを出力する = false;
			TJAPlayer3._MainConfig.bSTAGEFAILED有効 = false;
			TJAPlayer3._MainConfig.bTight = false;
			TJAPlayer3._MainConfig.bストイックモード = false;

			TJAPlayer3._MainConfig.nRisky = 0;
		}

		protected abstract void t進行描画_チップ_ドラムス( MainConfig configIni, ref Chart dTX, ref Chart.CChip pChip );
		protected abstract void t進行描画_チップ本体_ドラムス( MainConfig configIni, ref Chart dTX, ref Chart.CChip pChip );
		protected abstract void t進行描画_チップ_Taiko( MainConfig configIni, ref Chart dTX, ref Chart.CChip pChip, int nPlayer );
		protected abstract void t進行描画_チップ_Taiko連打( MainConfig configIni, ref Chart dTX, ref Chart.CChip pChip, int nPlayer );

		protected abstract void t進行描画_チップ_フィルイン( MainConfig configIni, ref Chart dTX, ref Chart.CChip pChip );
		protected abstract void t進行描画_チップ_小節線( MainConfig configIni, ref Chart dTX, ref Chart.CChip pChip, int nPlayer );
		...
		protected bool t進行描画_フェードイン_アウト()
		{
			switch ( base.eフェーズID )
			{
				...
				case BaseScene.Eフェーズ.演奏_STAGE_FAILED_フェードアウト:
				...
				case BaseScene.Eフェーズ.演奏_STAGE_CLEAR_フェードアウト:
		...
		{
			if ( TJAPlayer3._MainConfig.eDark != Eダークモード.FULL )
			{
				int y = TJAPlayer3._MainConfig.bReverse.Drums ? 53 - 演奏判定ライン座標.nJudgeLinePosY_delta.Drums : 567 + 演奏判定ライン座標.nJudgeLinePosY_delta.Drums;
																// #31602 2013.6.23 yyagi 描画遅延対策として、判定ラインの表示位置をオフセット調整できるようにする
				if ( this.txヒットバー != null )
				{
				    this.txヒットバー.Draw2D( TJAPlayer3.app.Device, 295, y, new Rectangle( 0, 0, 558, 6 ) );
				}
			}
		}
		...
		{
			if ( ( (E判定文字表示位置) TJAPlayer3._MainConfig.判定文字表示位置.Drums ) != E判定文字表示位置.コンボ下 )	// 判定ライン上または横
			{
				this.actJudgeString.t進行描画( 演奏判定ライン座標 );
			}
		}
		...
		protected void t背景テクスチャの生成( string DefaultBgFilename, Rectangle bgrect, string bgfilename )
		{
            try
            {
                if( !String.IsNullOrEmpty( bgfilename ) )
                    this.tx背景 = TJAPlayer3.CreateFDKTexture( TJAPlayer3.stage選曲.r確定されたスコア.ファイル情報.フォルダの絶対パス + bgfilename );
                else
                    this.tx背景 = TJAPlayer3.CreateFDKTexture( SkinManager.Path( DefaultBgFilename ) );
            }
            catch (Exception e)
            {
