	...
	internal class CStage曲読み込み : CStage
	{
		...
		public override void Activate()
		{
			...
			try
			{
                ...
                {
                    this.pfTITLE = new CPrivateFastFont( new FontFamily( TJAPlayer3._MainConfig.FontName ), TJAPlayer3.Skin.SongLoading_Title_FontSize );
                    this.pfSUBTITLE = new CPrivateFastFont( new FontFamily( TJAPlayer3._MainConfig.FontName ), TJAPlayer3.Skin.SongLoading_SubTitle_FontSize);
                }
                else
                {
				...
				case CStage.Eフェーズ.NOWLOADING_DTXファイルを読み込む:
					{
                        ...
                        {
						    TJAPlayer3.DTX = new CDTX( str, false, ( (double) TJAPlayer3._MainConfig.n演奏速度 ) / 20.0, ini.stファイル.BGMAdjust, 0, 0, true );
                            if( TJAPlayer3._MainConfig.nPlayerCount == 2 )
						        TJAPlayer3.DTX_2P = new CDTX( str, false, ( (double) TJAPlayer3._MainConfig.n演奏速度 ) / 20.0, ini.stファイル.BGMAdjust, 0, 1, true );
                            if( File.Exists( TJAPlayer3.DTX.strフォルダ名 + @"\\set.def" ) )
                            {
						        TJAPlayer3.DTX = new CDTX( str, false, ( (double) TJAPlayer3._MainConfig.n演奏速度 ) / 20.0, ini.stファイル.BGMAdjust, 0, 1, true );
                                if( TJAPlayer3._MainConfig.nPlayerCount == 2 )
						            TJAPlayer3.DTX_2P = new CDTX( str, false, ( (double) TJAPlayer3._MainConfig.n演奏速度 ) / 20.0, ini.stファイル.BGMAdjust, 0, 1, true );
                            }
                            ...
                            if (TJAPlayer3.stage選曲.n確定された曲の難易度 == (int)Difficulty.Dan && TJAPlayer3.DTX.List_DanSongs != null)
                            {
                                ...
                                if (!string.IsNullOrEmpty(TJAPlayer3._MainConfig.FontName))
                                {
                                    pfTitle = new CPrivateFont(new FontFamily(TJAPlayer3._MainConfig.FontName), 30);
                                    pfSubTitle = new CPrivateFont(new FontFamily(TJAPlayer3._MainConfig.FontName), 22);
                                }
                                else
                                {
