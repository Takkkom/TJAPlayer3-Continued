...
namespace TJAPlayer3
{
	internal class CStageタイトル : CStage
	{
		...
		public CStageタイトル()
		{
			...
			base.ChildActivities.Add( this.actFIfromSetup = new CActFIFOWhite() );
			base.ChildActivities.Add( this.actFI = new CActFIFOWhite() );
			base.ChildActivities.Add( this.actFO = new CActFIFOWhite() );
		}
		...
		public override int Draw()
		{
			if( !base.NotActivated )
			{
					...
					&& TJAPlayer3.act現在入力を占有中のプラグイン == null )	// プラグインの入力占有がない
				{
					...
					this.ctキー反復用.Up.RepeatKey( TJAPlayer3.Input管理.Keyboard.bキーが押されている( (int)SlimDX.DirectInput.Key.UpArrow ), new Counter.KeyProcess( this.tカーソルを上へ移動する ) );
					...
					this.ctキー反復用.Down.RepeatKey( TJAPlayer3.Input管理.Keyboard.bキーが押されている( (int)SlimDX.DirectInput.Key.DownArrow ), new Counter.KeyProcess( this.tカーソルを下へ移動する ) );
					...
					if( ( TJAPlayer3.Pad.b押されたDGB( Eパッド.CY ) || TJAPlayer3.Pad.b押された( E楽器パート.DRUMS, Eパッド.RD ) ) || ( TJAPlayer3.Pad.b押された( E楽器パート.DRUMS, Eパッド.LC ) || ( TJAPlayer3._MainConfig.bEnterがキー割り当てのどこにも使用されていない && TJAPlayer3.Input管理.Keyboard.bキーが押された( (int)SlimDX.DirectInput.Key.Return ) ) ) )
					{
...
//						Trace.TraceInformation( "DTXMania Title: SPACE key registered. " + CDTXMania.ct.nシステム時刻 );
                ...
                if (TJAPlayer3.Tx.Title_Background != null )
                    TJAPlayer3.Tx.Title_Background.Draw2D( TJAPlayer3.app.Device, 0, 0 );
				...
				if( TJAPlayer3.Tx.Title_Menu != null )
				{
					...
					if( this.ctカーソルフラッシュ用.NowValue <= 100 )
					{
                        ...
                        TJAPlayer3.Tx.Title_Menu.Scaling.X = nMag;
                        TJAPlayer3.Tx.Title_Menu.Scaling.Y = nMag;
                        TJAPlayer3.Tx.Title_Menu.Opacity = (int) ( 255.0 * ( 1.0 - ( ( (double) this.ctカーソルフラッシュ用.NowValue ) / 100.0 ) ) );
						int x_magnified = x + ( (int) ( ( MENU_W * ( 1.0 - nMag ) ) / 2.0 ) );
						int y_magnified = y + ( (int) ( ( MENU_H * ( 1.0 - nMag ) ) / 2.0 ) );
                        TJAPlayer3.Tx.Title_Menu.Draw2D( TJAPlayer3.app.Device, x_magnified, y_magnified, new Rectangle( 0, MENU_H * 5, MENU_W, MENU_H ) );
					}
                    TJAPlayer3.Tx.Title_Menu.Scaling.X = 1f;
                    TJAPlayer3.Tx.Title_Menu.Scaling.Y = 1f;
                    TJAPlayer3.Tx.Title_Menu.Opacity = 0xff;
                    TJAPlayer3.Tx.Title_Menu.Draw2D( TJAPlayer3.app.Device, x, y, new Rectangle( 0, MENU_H * 4, MENU_W, MENU_H ) );
				}
				if( TJAPlayer3.Tx.Title_Menu != null )
				{
                    ...
                    TJAPlayer3.Tx.Title_Menu.Draw2D( TJAPlayer3.app.Device, MENU_X, MENU_Y, new Rectangle( 0, 0, MENU_W, MENU_H ) );
                    TJAPlayer3.Tx.Title_Menu.Draw2D( TJAPlayer3.app.Device, MENU_X, MENU_Y + MENU_H, new Rectangle( 0, MENU_H * 2, MENU_W, MENU_H * 2 ) );
				}
