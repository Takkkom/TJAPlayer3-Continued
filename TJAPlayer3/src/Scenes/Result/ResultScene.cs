...
{
	internal class ResultScene : BaseScene
	{
		// プロパティ
		...
		public int n総合ランク値;
		public Chart.CChip[] r空うちドラムチップ;
		public STDGBVALUE<ScoreIni.C演奏記録> st演奏記録;
		...
		public ResultScene()
		{
			this.st演奏記録.Drums = new ScoreIni.C演奏記録();
			this.st演奏記録.Guitar = new ScoreIni.C演奏記録();
			this.st演奏記録.Bass = new ScoreIni.C演奏記録();
			this.st演奏記録.Taiko = new ScoreIni.C演奏記録();
			this.r空うちドラムチップ = new Chart.CChip[ 10 ];
			this.n総合ランク値 = -1;
			this.nチャンネル0Atoレーン07 = new int[] { 1, 2, 3, 4, 5, 7, 6, 1, 7, 0 };
			base.eステージID = BaseScene.Eステージ.結果;
			base.eフェーズID = BaseScene.Eフェーズ.共通_通常状態;
			base.NotActivated = true;
			base.ChildActivities.Add( this.actResultImage = new CActResultImage() );
			base.ChildActivities.Add( this.actParameterPanel = new CActResultParameterPanel() );
			base.ChildActivities.Add( this.actRank = new CActResultRank() );
			base.ChildActivities.Add( this.actSongBar = new CActResultSongBar() );
			base.ChildActivities.Add( this.actOption = new OptionPanel() );
			base.ChildActivities.Add( this.actFI = new ResultFade() );
		...
		public override void Activate()
		{
			...
			try
			{
				...
				for( int i = 0; i < 3; i++ )
				{
					...
					{
						ScoreIni.C演奏記録 part = this.st演奏記録[ i ];
						bool bIsAutoPlay = true;
						switch( i )
						{
							case 0:
                                bIsAutoPlay = TJAPlayer3._MainConfig.b太鼓パートAutoPlay;
								break;
						...
						this.bオート[ i ] = bIsAutoPlay;	// #23596 10.11.16 add ikanick そのパートがオートなら1
															//        10.11.17 change (int to bool) ikanick
						this.nランク値[ i ] = ScoreIni.tランク値を計算して返す( part );
					}
				}
				this.n総合ランク値 = ScoreIni.t総合ランク値を計算して返す( this.st演奏記録.Drums, this.st演奏記録.Guitar, this.st演奏記録.Bass );
				//---------------------
				#endregion
			...
			//			this.b新記録ランク[ i ] = true;
					...
					// 新記録スコアチェック
					if( ( this.st演奏記録[ i ].nスコア > ini.stセクション[ i * 2 ].nスコア ) && !TJAPlayer3._MainConfig.b太鼓パートAutoPlay )
					{
						this.b新記録スコア[ i ] = true;
					...
					ScoreIni.t更新条件を取得する( out b更新が必要か否か[ 0 ], out b更新が必要か否か[ 1 ], out b更新が必要か否か[ 2 ] );
                ...
                if( TJAPlayer3._MainConfig.bScoreIniを出力する )
				    ini.t書き出し( str );
				//---------------------
				#endregion
				...
				{
					ScoreInfo cスコア = TJAPlayer3.stage選曲.r確定されたスコア;
					bool[] b更新が必要か否か = new bool[ 3 ];
					ScoreIni.t更新条件を取得する( out b更新が必要か否か[ 0 ], out b更新が必要か否か[ 1 ], out b更新が必要か否か[ 2 ] );
					for( int m = 0; m < 3; m++ )
					{
                ...
                // Discord Presenseの更新
                Discord.UpdatePresence(TJAPlayer3.DTX.TITLE + ".tja", Properties.Discord.Stage_Result + (TJAPlayer3._MainConfig.b太鼓パートAutoPlay == true ? " (" + Properties.Discord.Info_IsAuto + ")" : ""), TJAPlayer3.StartupTime);
		...
		public override void Deactivate()
		{
			...
			{
				TJAPlayer3._SoundManager.DisposeSound( this.rResultSound );
		...
		public override int Draw()
		{
			if( !base.NotActivated )
			{
				...
				{
                    TJAPlayer3.Tx.Result_Background.Draw2D( TJAPlayer3.app.Device, 0, 0 );
				}
				if( this.ct登場用.IsProcessed && ( TJAPlayer3.Tx.Result_Header != null ) )
				{
					double num2 = ( (double) this.ct登場用.NowValue ) / 100.0;
					double num3 = Math.Sin( Math.PI / 2 * num2 );
					num = ( (int) ( TJAPlayer3.Tx.Result_Header.ImageSize.Height * num3 ) ) - TJAPlayer3.Tx.Result_Header.ImageSize.Height;
				}
				else
				{
					num = 0;
				...
				{
                    TJAPlayer3.Tx.Result_Header.Draw2D( TJAPlayer3.app.Device, 0, 0 );
				}
                if ( this.actResultImage.Draw() == 0 )
				{
					this.bアニメが完了 = false;
				}
                ...
                if ( base.eフェーズID == BaseScene.Eフェーズ.共通_フェードイン )
				{
				...
				else if( ( base.eフェーズID == BaseScene.Eフェーズ.共通_フェードアウト ) )			//&& ( this.actFO.On進行描画() != 0 ) )
				{
				...
				if ( this.bアニメが完了 == true && this.bIsCheckedWhetherResultScreenShouldSaveOrNot == false	// #24609 2011.3.14 yyagi; to save result screen in case BestRank or HiSkill.
					&& TJAPlayer3._MainConfig.bScoreIniを出力する
					&& TJAPlayer3._MainConfig.bIsAutoResultCapture)												// #25399 2011.6.9 yyagi
				{
...
//							this.actFO.tフェードアウト開始();
		...
		private ResultFade actFI;
		private BlackFade actFO;
		private OptionPanel actOption;
			...
			if ( bIsAutoSave )
			{
				...
				for ( int i = 0; i < 3; i++ )
				{
					if ( this.b新記録ランク[ i ] == true || this.b新記録スキル[ i ] == true )
					{
						...
						string strRank = ( (ScoreIni.ERANK) ( this.nランク値[ i ] ) ).ToString();
