	...
	internal class CStageコンフィグ : CStage
	{
		...
		public override void Activate()
		{
			...
			try
			{
                ...
                {
                    this.ftフォント = new Font(TJAPlayer3._MainConfig.FontName, 18.0f, FontStyle.Bold, GraphicsUnit.Pixel);
                }
                else
                {
		...
		public override void Deactivate()
		{
			...
			try
			{
				TJAPlayer3._MainConfig.Write( TJAPlayer3.strEXEのあるフォルダ + "Config.ini" );	// CONFIGだけ
		...
		public override int Draw()
		{
			...
			for ( int i = 0; i < txMenuItemLeft.GetLength( 0 ); i++ )
			{
				...
				int num4 = txMenuItemLeft[ i, flag ].ImageSize.Width;
                txMenuItemLeft[i, flag].Draw2D(TJAPlayer3.app.Device, 282 - (num4 / 2) + TJAPlayer3.Skin.Config_ItemText_Correction_X, menuY + TJAPlayer3.Skin.Config_ItemText_Correction_Y ); //55
				//txMenuItem.Dispose();
				menuY += stepY;
			}
			//---------------------
			...
			if( this.tx説明文パネル != null )
				this.tx説明文パネル.Draw2D( TJAPlayer3.app.Device, 67, 382 );
			//---------------------
			#endregion
			#region [ アイテム ]
			//---------------------
			...
			if ( !TJAPlayer3.EnumSongs.IsEnumerating || TJAPlayer3.actEnumSongs.bコマンドでの曲データ取得 != true )
			{
				...
				else if ( ( TJAPlayer3.Pad.b押されたDGB( Eパッド.CY ) || TJAPlayer3.Pad.b押された( E楽器パート.DRUMS, Eパッド.RD ) ) || ( TJAPlayer3.Pad.b押された( E楽器パート.DRUMS, Eパッド.LC ) || ( TJAPlayer3._MainConfig.bEnterがキー割り当てのどこにも使用されていない && TJAPlayer3.Input管理.Keyboard.bキーが押された( (int) SlimDX.DirectInput.Key.Return ) ) ) )
				{
		...
		private void t説明文パネルに現在選択されているメニューの説明を描画する()
		{
			try
			{
				...
				this.tx説明文パネル = new FDKTexture( TJAPlayer3.app.Device, image, TJAPlayer3.TextureFormat );
				image.Dispose();
			}
