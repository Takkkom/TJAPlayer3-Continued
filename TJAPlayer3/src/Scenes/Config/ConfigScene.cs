...
{
	internal class ConfigScene : BaseScene
	{
		// プロパティ

		public DFPFontManager actFont { get; private set; }
		...
		{
			DFPFontManager font;
			base.eステージID = BaseScene.Eステージ.コンフィグ;
			base.eフェーズID = BaseScene.Eフェーズ.共通_通常状態;
			this.actFont = font = new DFPFontManager();
			base.ChildActivities.Add( font );
			base.ChildActivities.Add( this.actFIFO = new WhiteFade() );
		...
		{																						//
			this.actKeyAssign.t開始( part, pad, this.actList.ib現在の選択項目.Name );		//
		...
		public override void Deactivate()
		{
			...
			try
			{
				TJAPlayer3._MainConfig.Write( TJAPlayer3.DirectoryWithThisEXE + "Config.ini" );	// CONFIGだけ
		...
		public override int Draw()
		{
			...
			for ( int i = 0; i < txMenuItemLeft.GetLength( 0 ); i++ )
			{
                ...
                txMenuItemLeft[i, flag].Draw2D(TJAPlayer3.app.Device, 282 - (num4 / 2) + TJAPlayer3.Skin.SkinValue.Config_ItemText_Correction_X, menuY + TJAPlayer3.Skin.SkinValue.Config_ItemText_Correction_Y ); //55
				//txMenuItem.Dispose();
				menuY += stepY;
			}
			//---------------------
		...
		private void t説明文パネルに現在選択されている項目の説明を描画する()
		{
			try
			{
                ...
                BaseItem item = this.actList.ib現在の選択項目;
				if( ( item.Description != null ) && ( item.Description.Length > 0 ) )
				{
                    graphics.DrawString( item.Description, this.ftフォント, Brushes.White, new RectangleF( 8f, 0, 630, 430 ) );
				}
				graphics.Dispose();
				if( this.tx説明文パネル != null )
				{
