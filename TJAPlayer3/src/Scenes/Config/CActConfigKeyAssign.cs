...
{
	internal class CActConfigKeyAssign : Activity
	{
		// プロパティ
		...
		public void t開始( EKeyConfigPart part, EKeyConfigPad pad, string strパッド名 )
		{
			if( part != EKeyConfigPart.UNKNOWN )
			{
				...
				{
					this.structReset用KeyAssign[ i ].入力デバイス = TJAPlayer3._MainConfig.KeyAssign[ (int) part ][ (int) pad ][ i ].入力デバイス;
					this.structReset用KeyAssign[ i ].ID = TJAPlayer3._MainConfig.KeyAssign[ (int) part ][ (int) pad ][ i ].ID;
					this.structReset用KeyAssign[ i ].コード = TJAPlayer3._MainConfig.KeyAssign[ (int) part ][ (int) pad ][ i ].コード;
				}
			}
		}
			...
			if( !this.bキー入力待ち )
			{
				...
				switch( this.n現在の選択行 )
				{
					case 0x10:
						for( int i = 0; i < 0x10; i++ )
						{
							TJAPlayer3._MainConfig.KeyAssign[ (int) this.part ][ (int) this.pad ][ i ].入力デバイス = this.structReset用KeyAssign[ i ].入力デバイス;
							TJAPlayer3._MainConfig.KeyAssign[ (int) this.part ][ (int) this.pad ][ i ].ID = this.structReset用KeyAssign[ i ].ID;
							TJAPlayer3._MainConfig.KeyAssign[ (int) this.part ][ (int) this.pad ][ i ].コード = this.structReset用KeyAssign[ i ].コード;
						}
						return;
		...
		public override void Activate()
		{
			...
			this.n現在の選択行 = 0;
			this.bキー入力待ち = false;
			this.structReset用KeyAssign = new MainConfig.CKeyAssign.STKEYASSIGN[ 0x10 ];
			base.Activate();
		}
			...
			if( !base.NotActivated )
			{
				if( this.bキー入力待ち )
				{
					if( TJAPlayer3.Input管理.Keyboard.GetKeyPressed( (int)SlimDX.DirectInput.Key.Escape ) )
					{
						...
						this.bキー入力待ち = false;
						TJAPlayer3.Input管理.Polling( TJAPlayer3.app.bApplicationActive, false );
					}
				...
				else if( ( TJAPlayer3.Input管理.Keyboard.GetKeyPressed( (int)SlimDX.DirectInput.Key.Delete ) && ( this.n現在の選択行 >= 0 ) ) && ( this.n現在の選択行 <= 15 ) )
				{
					TJAPlayer3.Skin.sound決定音.t再生する();
					TJAPlayer3._MainConfig.KeyAssign[ (int) this.part ][ (int) this.pad ][ this.n現在の選択行 ].入力デバイス = E入力デバイス.不明;
					TJAPlayer3._MainConfig.KeyAssign[ (int) this.part ][ (int) this.pad ][ this.n現在の選択行 ].ID = 0;
					TJAPlayer3._MainConfig.KeyAssign[ (int) this.part ][ (int) this.pad ][ this.n現在の選択行 ].コード = 0;
				}
				if(TJAPlayer3.Tx.Menu_Highlight != null )
				{
                    ...
                    TJAPlayer3.Tx.Menu_Highlight.Draw2D( TJAPlayer3.app.Device, num2, num3, new Rectangle( 0, 0, 0x10, 0x20 ) );
					num2 += 0x10;
					Rectangle rectangle = new Rectangle( 8, 0, 0x10, 0x20 );
					for( int j = 0; j < 14; j++ )
					{
                        TJAPlayer3.Tx.Menu_Highlight.Draw2D( TJAPlayer3.app.Device, num2, num3, rectangle );
						num2 += 0x10;
					}
                    TJAPlayer3.Tx.Menu_Highlight.Draw2D( TJAPlayer3.app.Device, num2, num3, new Rectangle( 0x10, 0, 0x10, 0x20 ) );
				}
				int num5 = 20;
				int x = 0x134;
				int y = 0x40;
				...
				y += num5;
				MainConfig.CKeyAssign.STKEYASSIGN[] stkeyassignArray = TJAPlayer3._MainConfig.KeyAssign[ (int) this.part ][ (int) this.pad ];
				for( int i = 0; i < 0x10; i++ )
				{
		...
		private MainConfig.CKeyAssign.STKEYASSIGN[] structReset用KeyAssign;
		...
		private bool tキーチェックとアサイン_Joypad()
		{
			...
			{
				if( device.NowInputDeviceType == InputTypes.Joystick )
				{
					for( int i = 0; i < 6 + 0x80 + 8; i++ )		// +6 for Axis, +8 for HAT
					{
						if( device.GetKeyPressed( i ) )
						{
							TJAPlayer3.Skin.sound決定音.t再生する();
							TJAPlayer3._MainConfig.t指定した入力が既にアサイン済みである場合はそれを全削除する( E入力デバイス.ジョイパッド, device.ID, i );
							TJAPlayer3._MainConfig.KeyAssign[ (int) this.part ][ (int) this.pad ][ this.n現在の選択行 ].入力デバイス = E入力デバイス.ジョイパッド;
							TJAPlayer3._MainConfig.KeyAssign[ (int) this.part ][ (int) this.pad ][ this.n現在の選択行 ].ID = device.ID;
							TJAPlayer3._MainConfig.KeyAssign[ (int) this.part ][ (int) this.pad ][ this.n現在の選択行 ].コード = i;
							return true;
						}
					}
				}
			...
			for( int i = 0; i < 0x100; i++ )
			{
				if( i != (int)SlimDX.DirectInput.Key.Escape &&
					...
					i != (int)SlimDX.DirectInput.Key.Delete &&
					 TJAPlayer3.Input管理.Keyboard.GetKeyPressed( i ) )
				{
					TJAPlayer3.Skin.sound決定音.t再生する();
					TJAPlayer3._MainConfig.t指定した入力が既にアサイン済みである場合はそれを全削除する( E入力デバイス.キーボード, 0, i );
					TJAPlayer3._MainConfig.KeyAssign[ (int) this.part ][ (int) this.pad ][ this.n現在の選択行 ].入力デバイス = E入力デバイス.キーボード;
					TJAPlayer3._MainConfig.KeyAssign[ (int) this.part ][ (int) this.pad ][ this.n現在の選択行 ].ID = 0;
					TJAPlayer3._MainConfig.KeyAssign[ (int) this.part ][ (int) this.pad ][ this.n現在の選択行 ].コード = i;
					return true;
				}
		...
		private bool tキーチェックとアサイン_MidiIn()
		{
			foreach( IInputDevice device in TJAPlayer3.Input管理.InputDevices )
			{
				if( device.NowInputDeviceType == InputTypes.MidiIn )
				{
					for( int i = 0; i < 0x100; i++ )
					{
						if( device.GetKeyPressed( i ) )
						{
							TJAPlayer3.Skin.sound決定音.t再生する();
							TJAPlayer3._MainConfig.t指定した入力が既にアサイン済みである場合はそれを全削除する( E入力デバイス.MIDI入力, device.ID, i );
							TJAPlayer3._MainConfig.KeyAssign[ (int) this.part ][ (int) this.pad ][ this.n現在の選択行 ].入力デバイス = E入力デバイス.MIDI入力;
							TJAPlayer3._MainConfig.KeyAssign[ (int) this.part ][ (int) this.pad ][ this.n現在の選択行 ].ID = device.ID;
							TJAPlayer3._MainConfig.KeyAssign[ (int) this.part ][ (int) this.pad ][ this.n現在の選択行 ].コード = i;
							return true;
						}
		...
		private bool tキーチェックとアサイン_Mouse()
		{
			for( int i = 0; i < 8; i++ )
			{
				if( TJAPlayer3.Input管理.Mouse.GetKeyPressed( i ) )
				{
					TJAPlayer3._MainConfig.t指定した入力が既にアサイン済みである場合はそれを全削除する( E入力デバイス.マウス, 0, i );
					TJAPlayer3._MainConfig.KeyAssign[ (int) this.part ][ (int) this.pad ][ this.n現在の選択行 ].入力デバイス = E入力デバイス.マウス;
					TJAPlayer3._MainConfig.KeyAssign[ (int) this.part ][ (int) this.pad ][ this.n現在の選択行 ].ID = 0;
					TJAPlayer3._MainConfig.KeyAssign[ (int) this.part ][ (int) this.pad ][ this.n現在の選択行 ].コード = i;
				}
			}
