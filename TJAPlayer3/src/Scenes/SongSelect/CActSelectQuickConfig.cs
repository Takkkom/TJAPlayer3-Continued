		...
		public override void tEnter押下Main( int nSortOrder )
		{
            switch ( n現在の選択行 )
            {
				case (int) EOrder.ScrollSpeed:
					TJAPlayer3._MainConfig.n譜面スクロール速度[ nCurrentTarget ] = (int) GetObj現在値( (int) EOrder.ScrollSpeed );
					break;

				case (int) EOrder.PlaySpeed:
					TJAPlayer3._MainConfig.n演奏速度 = (int) GetObj現在値( (int) EOrder.PlaySpeed );
					break;
				case (int) EOrder.Random:
                    TJAPlayer3._MainConfig.eRandom.Taiko = (Eランダムモード)GetIndex( (int)EOrder.Random );
					break;
				case (int) EOrder.Stealth:
                    TJAPlayer3._MainConfig.eSTEALTH = (Eステルスモード)GetIndex( (int)EOrder.Stealth );
					break;
				case (int) EOrder.GameMode:
					...
					TJAPlayer3._MainConfig.eGameMode = game;
					break;
                case (int)EOrder.ShinuchiMode:
                    TJAPlayer3._MainConfig.ShinuchiMode = !TJAPlayer3._MainConfig.ShinuchiMode;
                    break;
