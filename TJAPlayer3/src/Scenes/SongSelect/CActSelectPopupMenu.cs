...
{
	internal class CActSelectPopupMenu : Activity
	{
	...
	//		this.n現在の選択行 = 0;
		...
		public int t進行描画()
		{
			if ( !base.NotActivated && this.bIsActivePopupMenu )
			{
				if ( this.bキー入力待ち )
				{
				...
				{
                    TJAPlayer3.Tx.Menu_Title.Draw2D( TJAPlayer3.app.Device, 160, 40 );
				}
				#endregion
				#region [ ソートメニュータイトル描画 ]
				int x = 240, y = 44;
				stqMenuTitle.txName.Draw2D( TJAPlayer3.app.Device, x, y );
				#endregion
				#region [ カーソル描画 ]
				if ( TJAPlayer3.Tx.Menu_Highlight != null )
				{
                    ...
                    TJAPlayer3.Tx.Menu_Highlight.Draw2D( TJAPlayer3.app.Device, curX, curY, new Rectangle( 0, 0, 16, 32 ) );
					curX += 0x10;
					Rectangle rectangle = new Rectangle( 8, 0, 0x10, 0x20 );
					for ( int j = 0; j < 16; j++ )
					{
                        TJAPlayer3.Tx.Menu_Highlight.Draw2D( TJAPlayer3.app.Device, curX, curY, rectangle );
						curX += 16;
					}
                    TJAPlayer3.Tx.Menu_Highlight.Draw2D( TJAPlayer3.app.Device, curX, curY, new Rectangle( 0x10, 0, 16, 32 ) );
				}
				#endregion
				#region [ ソート候補文字列描画 ]
				for ( int i = 0; i < lciMenuItems.Length; i++ )
				{
					...
					if ( lciMenuItems[ i ].txName != null )
					{
						...
						lciMenuItems[ i ].txName.Draw2D( TJAPlayer3.app.Device, 180, 77 + i * 32 );
					}
