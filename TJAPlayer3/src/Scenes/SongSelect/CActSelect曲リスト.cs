...
//			this.On非活性化();
		...
		public override int Draw()
		{
			...
			if( !this.b登場アニメ全部完了 )
			{
                if( this.n現在のスクロールカウンタ == 0 )
                {
                                ...
                                if (TJAPlayer3.Tx.SongSelect_Frame_Score != null)
                                {
                                    ...
                                    else
                                    {
                                        if (TJAPlayer3.stage選曲.r現在選択中のスコア.譜面情報.nレベル[TJAPlayer3.stage選曲.n現在選択中の曲の難易度] >= 0)
                                        {
                                            // 譜面がありますね
                                            TJAPlayer3.Tx.SongSelect_Frame_Score.color4 = new Color4(1f, 1f, 1f);
                                            TJAPlayer3.Tx.SongSelect_Frame_Score.t2D下中央基準描画(TJAPlayer3.app.Device, 494 + 120, TJAPlayer3.Skin.SongSelect_Overall_Y + 463, new Rectangle(0, 360 + (360 * (TJAPlayer3.stage選曲.n現在選択中の曲の難易度 - (int)Difficulty.Tower)), TJAPlayer3.Tx.SongSelect_Frame_Score.TextureSize.Width, 360));
                                        }
                                        else
                                        {
                                            // ないですね
                    ...
                    if (TJAPlayer3.Tx.SongSelect_Branch_Text != null && TJAPlayer3.stage選曲.r現在選択中のスコア.譜面情報.b譜面分岐[ TJAPlayer3.stage選曲.n現在選択中の曲の難易度 ] )
                        TJAPlayer3.Tx.SongSelect_Branch_Text.Draw2D( TJAPlayer3.app.Device, 483, TJAPlayer3.Skin.SongSelect_Overall_Y+21 );
				...
				for( int i = 0; i < 13; i++ )	// パネルは全13枚。
				{
					if( this.ct登場アニメ用[ i ].NowValue >= 0 )
					{
						double db割合0to1 = ( (double) this.ct登場アニメ用[ i ].NowValue ) / 100.0;
						...
						if( i == 5 )
						{
                            ...
                            if( this.ttk選択している曲のサブタイトル != null )
                            {
                                ...
                                int nサブタイY = (int)(TJAPlayer3.Skin.SongSelect_Overall_Y + 440 - (tx選択している曲のサブタイトル.ImageSize.Height * tx選択している曲のサブタイトル.Scaling.Y ));
							    tx選択している曲のサブタイトル.Draw2D( TJAPlayer3.app.Device, 707, nサブタイY );
                                if( this.ttk選択している曲の曲名 != null )
                                {
                                    ResolveTitleTexture(this.ttk選択している曲の曲名).Draw2D( TJAPlayer3.app.Device, 750, TJAPlayer3.Skin.SongSelect_Overall_Y+23);
                                }
                            }
                            else
                            {
						...
						else
						{
                            ...
                            if( this.stバー情報[ nパネル番号 ].b分岐[ TJAPlayer3.stage選曲.n現在選択中の曲の難易度 ] == true && i != 5 )
                                TJAPlayer3.Tx.SongSelect_Branch.Draw2D( TJAPlayer3.app.Device, this.ptバーの座標[ nパネル番号 ].X + 66, TJAPlayer3.Skin.SongSelect_Overall_Y-5);
                            ...
                            if( this.stバー情報[ nパネル番号 ].ttkタイトル != null )
                                ResolveTitleTexture(this.stバー情報[ nパネル番号 ].ttkタイトル).Draw2D( TJAPlayer3.app.Device, this.ptバーの座標[ i ].X + 28, TJAPlayer3.Skin.SongSelect_Overall_Y+23);
							#endregion
						}
					}
				}
				...
				for( int i = 0; i < 13; i++ )	// パネルは全13枚。
				{
		...
		
					{
                        ...
                        if (this.stバー情報[nパネル番号].b分岐[TJAPlayer3.stage選曲.n現在選択中の曲の難易度] == true && n見た目の行番号 != 5)
                            TJAPlayer3.Tx.SongSelect_Branch.Draw2D(TJAPlayer3.app.Device, xAnime + 66, TJAPlayer3.Skin.SongSelect_Overall_Y - 5);
                        //-----------------
                        #endregion
                        ...
                        if (n現在のスクロールカウンタ != 0)
                            ResolveTitleTexture(this.stバー情報[ nパネル番号 ].ttkタイトル).Draw2D( TJAPlayer3.app.Device, xAnime + 28, TJAPlayer3.Skin.SongSelect_Overall_Y+23);
                        else if (n見た目の行番号 != 5)
                            ResolveTitleTexture(this.stバー情報[nパネル番号].ttkタイトル).Draw2D(TJAPlayer3.app.Device, xAnime + 28, TJAPlayer3.Skin.SongSelect_Overall_Y + 23);
            ...
            if( this.e曲のバー種別を返す( this.r現在選択中の曲 ) == Eバー種別.Score && this.nStrジャンルtoNum( this.r現在選択中の曲.strジャンル ) != 8 )
            {
                if( TJAPlayer3.Tx.SongSelect_GenreText != null )
                    TJAPlayer3.Tx.SongSelect_GenreText.Draw2D( TJAPlayer3.app.Device, 496, TJAPlayer3.Skin.SongSelect_Overall_Y-64, new Rectangle( 0, 60 * this.nStrジャンルtoNum( this.r現在選択中の曲.strジャンル ), 288, 60 ) );
            }
  ...
  //      private CTexture txバー中央;
			...
			switch( strジャンル )
            {
                case "J-POP":
	    			...
	    			if( TJAPlayer3.Tx.SongSelect_Bar_Genre[1] != null )
                        TJAPlayer3.Tx.SongSelect_Bar_Genre[1].Draw2D(TJAPlayer3.app.Device, x, y );
	    			//-----------------
		    		#endregion
                    break;
                case "アニメ":
	    			...
	    			if(TJAPlayer3.Tx.SongSelect_Bar_Genre[2] != null )
                        TJAPlayer3.Tx.SongSelect_Bar_Genre[2].Draw2D( TJAPlayer3.app.Device, x, y );
	    			//-----------------
		    		#endregion
                    break;
                case "ゲームミュージック":
	    			...
	    			if(TJAPlayer3.Tx.SongSelect_Bar_Genre[3] != null )
                        TJAPlayer3.Tx.SongSelect_Bar_Genre[3].Draw2D( TJAPlayer3.app.Device, x, y );
	    			//-----------------
		    		#endregion
                    break;
                case "ナムコオリジナル":
	    			...
	    			if(TJAPlayer3.Tx.SongSelect_Bar_Genre[4] != null )
                        TJAPlayer3.Tx.SongSelect_Bar_Genre[4].Draw2D( TJAPlayer3.app.Device, x, y );
	    			//-----------------
		    		#endregion
                    break;
                case "クラシック":
	    			...
	    			if(TJAPlayer3.Tx.SongSelect_Bar_Genre[5] != null )
                        TJAPlayer3.Tx.SongSelect_Bar_Genre[5].Draw2D( TJAPlayer3.app.Device, x, y );
	    			//-----------------
		    		#endregion
                    break;
                case "バラエティ":
	    			...
	    			if(TJAPlayer3.Tx.SongSelect_Bar_Genre[6] != null )
                        TJAPlayer3.Tx.SongSelect_Bar_Genre[6].Draw2D( TJAPlayer3.app.Device, x, y );
	    			//-----------------
		    		#endregion
                    break;
                case "どうよう":
	    			...
	    			if(TJAPlayer3.Tx.SongSelect_Bar_Genre[7] != null )
                        TJAPlayer3.Tx.SongSelect_Bar_Genre[7].Draw2D( TJAPlayer3.app.Device, x, y );
	    			//-----------------
		    		#endregion
                    break;
                case "ボーカロイド":
                case "VOCALOID":
	    			...
	    			if(TJAPlayer3.Tx.SongSelect_Bar_Genre[8] != null )
                        TJAPlayer3.Tx.SongSelect_Bar_Genre[8].Draw2D( TJAPlayer3.app.Device, x, y );
	    			//-----------------
		    		#endregion
                    break;
                case "難易度ソート":
	    			...
	    			if( this.tx曲バー_難易度[ this.n現在選択中の曲の現在の難易度レベル ] != null )
		    			this.tx曲バー_難易度[ this.n現在選択中の曲の現在の難易度レベル ].Draw2D( TJAPlayer3.app.Device, x, y );
	    			//-----------------
		    		#endregion
                    break;
                default:
	    			...
	    			if(TJAPlayer3.Tx.SongSelect_Bar_Genre[0] != null )
                        TJAPlayer3.Tx.SongSelect_Bar_Genre[0].Draw2D( TJAPlayer3.app.Device, x, y );
	    			//-----------------
		    		#endregion
                    break;
			}
	    ...
	    private static FDKTexture GenerateTitleTexture(TitleTextureKey titleTextureKey)
	    {
	        ...
	        {
	            FDKTexture tx文字テクスチャ = TJAPlayer3.tテクスチャの生成(bmp, false);
