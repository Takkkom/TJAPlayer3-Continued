					...
					&& TJAPlayer3.act現在入力を占有中のプラグイン == null )
				{
					...
					if ( !this.actSortSongs.bIsActivePopupMenu && !this.actQuickConfig.bIsActivePopupMenu /*&&  !this.act難易度選択画面.bIsDifficltSelect */ )
					{
						...
						if ( this.act曲リスト.r現在選択中の曲 != null )
						{
                            ...
                            #endregion
                            #region [ Up ]
                            this.ctキー反復用.Up.RepeatKey( TJAPlayer3.Input管理.Keyboard.bキーが押されている( (int) SlimDX.DirectInput.Key.LeftArrow ), new Counter.KeyProcess( this.tカーソルを上へ移動する ) );
							...
							#endregion
							#region [ Down ]
							this.ctキー反復用.Down.RepeatKey( TJAPlayer3.Input管理.Keyboard.bキーが押されている( (int) SlimDX.DirectInput.Key.RightArrow ), new Counter.KeyProcess( this.tカーソルを下へ移動する ) );
