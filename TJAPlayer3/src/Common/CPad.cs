...
namespace TJAPlayer3
{
    public class CPad
	{
		...
		internal CPad( MainConfig configIni, InputManager mgrInput )
		{
				...
				if( ( device.list入力イベント != null ) && ( device.list入力イベント.Count != 0 ) )
				{
					foreach( STInputEvent event2 in device.list入力イベント )
					{
						for( int i = 0; i < stkeyassignArray.Length; i++ )
						{
							...
							{
								case E入力デバイス.キーボード:
									if( ( device.e入力デバイス種別 == InputTypes.Keyboard ) && ( event2.nKey == stkeyassignArray[ i ].コード ) )
									{
										list.Add( event2 );
								...
								case E入力デバイス.MIDI入力:
									if( ( ( device.e入力デバイス種別 == InputTypes.MidiIn ) && ( device.ID == stkeyassignArray[ i ].ID ) ) && ( event2.nKey == stkeyassignArray[ i ].コード ) )
									{
										list.Add( event2 );
								...
								case E入力デバイス.ジョイパッド:
									if( ( ( device.e入力デバイス種別 == InputTypes.Joystick ) && ( device.ID == stkeyassignArray[ i ].ID ) ) && ( event2.nKey == stkeyassignArray[ i ].コード ) )
									{
										list.Add( event2 );
								...
								case E入力デバイス.マウス:
									if( ( device.e入力デバイス種別 == InputTypes.Mouse ) && ( event2.nKey == stkeyassignArray[ i ].コード ) )
									{
										list.Add( event2 );
