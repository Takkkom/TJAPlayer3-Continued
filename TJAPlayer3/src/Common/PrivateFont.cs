...
namespace TJAPlayer3
{
    ...
    public class PrivateFont : IDisposable
	{
		#region [ コンストラクタ ]
		public PrivateFont( FontFamily fontfamily, int pt, FontStyle style )
		{
			Initialize( null, fontfamily, pt, style );
		}
		public PrivateFont( FontFamily fontfamily, int pt )
		{
			Initialize( null, fontfamily, pt, FontStyle.Regular );
		}
		public PrivateFont( string fontpath, int pt, FontStyle style )
		{
			Initialize( fontpath, null, pt, style );
		}
		public PrivateFont( string fontpath, int pt )
		{
		...
		{
			this._PrivateFontCollection = null;
			...
			else
			{
				try
				{
					this._PrivateFontCollection = new System.Drawing.Text.PrivateFontCollection();	//PrivateFontCollectionオブジェクトを作成する
					this._PrivateFontCollection.AddFontFile(fontpath);								//PrivateFontCollectionにフォントを追加する
		...
		protected Bitmap DrawPrivateFont( string drawstr, DrawMode drawmode, Color fontColor, Color edgeColor, Color gradationTopColor, Color gradationBottomColor )
		{
            ...
            int nEdgePt = (bEdge) ? (10 * _pt / TJAPlayer3.Skin.SkinValue.Font_Edge_Ratio) : 0; //SkinConfigにて設定可能に(rhimm)
			...
			// レイアウト枠
			Rectangle r = new Rectangle( 0, 0, stringSize.Width + nEdgePt * 2 + (TJAPlayer3.Skin.SkinValue.Text_Correction_X * stringSize.Width / 100), stringSize.Height + nEdgePt * 2 + (TJAPlayer3.Skin.SkinValue.Text_Correction_Y * stringSize.Height / 100));
			...
			OriginPoint = new Point( nEdgePt * 2, nEdgePt * 2 );
		...
		protected Bitmap DrawPrivateFont_V( string drawstr, Color fontColor, Color edgeColor, bool bVertical )
		{
            ...
            for( int i = 0; i < strName.Length; i++ )
            {
                ...
                Graphics cal = Graphics.FromImage( bmpDummy );
                Rectangle accurateSize = this.MeasureStringPrecisely( cal, strName[ i ], this._Font, strSize, sFormat );
                int marginSize = strSize.Height - accurateSize.Height;

                Rectangle rect = new Rectangle( 0, -marginSize + 2, 46, ( strSize.Height + 16 ));

                if( strName[ i ] == "ー" || strName[ i ] == "-" || strName[ i ] == "～" || strName[ i ] == "<" || strName[ i ] == ">" || strName[ i ] == "(" || strName[ i ] == ")" || strName[ i ] == "「" || strName[ i ] == "」" || strName[ i ] == "[" || strName[ i ] == "]" )
                {
                    nHeight += ( accurateSize.Width ) + 4;
                }
                else if( strName[ i ] == "_" ){ nHeight += ( accurateSize.Height ) + 6;  }
                else if( strName[ i ] == " " )
                { nHeight += ( 12 ); }
                else { nHeight += ( accurateSize.Height ) + 10; }
            ...
            for (int i = 0; i < strName.Length; i++)
            {
                ...
                Rectangle accurateSize = this.MeasureStringPrecisely(gCal, strName[i], this._Font, strSize, sFormat);
                int marginSize = strSize.Height - accurateSize.Height;

                //Bitmap bmpV = new Bitmap( 36, ( strSize.Height + 12 ) - 6 );

                Bitmap bmpV = new Bitmap((accurateSize.Width + 12) + nAdded, (accurateSize.Height) + 12);
                ...
                if (strName[i].In(TJAPlayer3.Skin.SkinValue.SongSelect_CorrectionX_Chara))
                {
                    edgeOffsetX = TJAPlayer3.Skin.SkinValue.SongSelect_CorrectionX_Chara_Value;
                }
                else
                {
                    edgeOffsetX = 0;
                }
                if (strName[i].In(TJAPlayer3.Skin.SkinValue.SongSelect_CorrectionY_Chara))
                {
                    edgeOffsetY = TJAPlayer3.Skin.SkinValue.SongSelect_CorrectionY_Chara_Value;
                }
                else
                {
                    edgeOffsetY = 0;
                ...
                Rectangle rect = new Rectangle(-3 - nAdded + (edgeOffsetX * _pt / 100), -accurateSize.Y - 2 + (edgeOffsetY * _pt / 100), (strSize.Width + 12), (strSize.Height + 12));
                ...
                int nEdgePt = (10 * _pt / TJAPlayer3.Skin.SkinValue.Font_Edge_Ratio_Vertical); // SkinConfigにて設定可能に(rhimm)
                ...
                else if (strName[i].In(TJAPlayer3.Skin.SkinValue.SongSelect_Rotate_Chara))
                {
                    bmpV.RotateFlip(RotateFlipType.Rotate90FlipNone);
                }
                ...
                Gcambus.DrawImage( bmpV, (bmpCambus.Width / 2) - (bmpV.Width / 2) + offset, nNowPos + offsetY );
        ...
        public void Dispose()
        {
            if (!this.IsDiposed)
            {
                ...
                this._PrivateFontCollection?.Dispose();
                this._PrivateFontCollection = null;
