...
namespace FDK
{
	public class Win32Manager
	{
		...
		public struct WINDOWPLACEMENT
		{
			public int length;
			public int flags;
			public Win32Manager.EShowWindow showCmd;
			public Point ptMinPosition;
			public Point ptMaxPosition;
			public Win32Manager.RECT rcNormalPosition;
			public static int Length
			{
				get
				{
					return Marshal.SizeOf( typeof( Win32Manager.WINDOWPLACEMENT ) );
				}
			}
		}
		[StructLayout( LayoutKind.Sequential )]
		...
		public static bool IAppInIdleState
		{
			get
			{
		...
		public static class KeyToggleFeature
		{
			public static void Disable()
			{
				if( ( stored.dwFlags & 1L ) == 0L )
				{
					Win32Manager.TOGGLEKEYS structure = new Win32Manager.TOGGLEKEYS();
					...
					Win32Manager.SystemParametersInfo( 0x35, (uint) cb, ptr, 0 );
			...
			#region [ private ]
			//-----------------
			static KeyToggleFeature()
			{
				int cb = Marshal.SizeOf( stored );
				IntPtr ptr = Marshal.AllocCoTaskMem( cb );
				Marshal.StructureToPtr( stored, ptr, false );
				Win32Manager.SystemParametersInfo( 0x34, (uint) cb, ptr, 0 );
				stored = (Win32Manager.TOGGLEKEYS) Marshal.PtrToStructure( ptr, typeof( Win32Manager.TOGGLEKEYS ) );
				Marshal.FreeCoTaskMem( ptr );
			}

			private const uint SPI_GETTOGGLEKEYS = 0x34;
			private const uint SPI_SETTOGGLEKEYS = 0x35;
			private static Win32Manager.TOGGLEKEYS stored = new Win32Manager.TOGGLEKEYS();
		...
		public static class KeyFilterFeature
		{
			public static void Disable()
			{
				if( ( stored.dwFlags & 1L ) == 0L )
				{
					Win32Manager.FILTERKEYS structure = new Win32Manager.FILTERKEYS();
					structure.dwFlags = stored.dwFlags;
					structure.cbSize = stored.cbSize;
					structure.dwFlags &= -5;
					structure.dwFlags &= -9;
					...
					IntPtr ptr = Marshal.AllocCoTaskMem( cb );
					Marshal.StructureToPtr( structure, ptr, false );
					Win32Manager.SystemParametersInfo( 0x3b, (uint) cb, ptr, 0 );
					Marshal.FreeCoTaskMem( ptr );
				}
			}
			public static void Restore()
			...
			static KeyFilterFeature()
			{
				...
				IntPtr ptr = Marshal.AllocCoTaskMem( cb );
				Marshal.StructureToPtr( stored, ptr, false );
				Win32Manager.SystemParametersInfo( 50, (uint) cb, ptr, 0 );
				stored = (Win32Manager.FILTERKEYS) Marshal.PtrToStructure( ptr, typeof( Win32Manager.FILTERKEYS ) );
				Marshal.FreeCoTaskMem( ptr );
			}
			...
			private static Win32Manager.FILTERKEYS stored = new Win32Manager.FILTERKEYS();
			//-----------------
			#endregion
		}
		public static class StickyKeysFeature
		{
			public static void Disable()
			{
				if( ( stored.dwFlags & 1L ) == 0L )
				{
					Win32Manager.STICKYKEYS structure = new Win32Manager.STICKYKEYS();
					structure.dwFlags = stored.dwFlags;
					structure.cbSize = stored.cbSize;
					structure.dwFlags &= -5;
					structure.dwFlags &= -9;
			...
			static StickyKeysFeature()
			{
				...
				IntPtr ptr = Marshal.AllocCoTaskMem( cb );
				Marshal.StructureToPtr( stored, ptr, false );
				Win32Manager.SystemParametersInfo( 0x3a, (uint) cb, ptr, 0 );
				stored = (Win32Manager.STICKYKEYS) Marshal.PtrToStructure( ptr, typeof( Win32Manager.STICKYKEYS ) );
				Marshal.FreeCoTaskMem( ptr );
			}
			...
			private static Win32Manager.STICKYKEYS stored = new Win32Manager.STICKYKEYS();
			//-----------------
			#endregion
		}
