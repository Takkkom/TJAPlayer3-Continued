...
namespace FDK
{
	...
	/// </summary>
	public class FDKTextureAf : FDKTexture, IDisposable
	{
		...
		public FDKTextureAf( Device device, string path, Format format, bool transparentBlack )
		...
		/// <exception cref="TextureCreateFailedException">テクスチャの作成に失敗しました。</exception>
		public FDKTextureAf( Device device, string path, Format format, bool transparentBlack, Pool pool )
		{
		...
		public new void MakeTexture( Device device, string path, Format format, bool transparentBlack, Pool pool )
		{
			...
			if ( !bサイズは２の累乗でなければならない && b条件付きでサイズは２の累乗でなくてもOK )
			{
				...
				base.MakeTexture( device, path, format, transparentBlack, pool );
				return;
			}
...
//Debug.WriteLine( Path.GetFileName( strファイル名 ) + ": texture最適化結果: width=" + w + ", height=" + h + ", 折りたたみ回数=" + foldtimes );
			...
			bmpOrg.Dispose();
			bmpOrg = null;
			base.MakeTexture( device, bmpNew, format, transparentBlack, pool );
			bmpNew.Dispose();
			bmpNew = null;
			#endregion
			...
			this.ImageSize = new Size( orgWidth, orgHeight );
		}
...
//Debug.WriteLine( "powちょうどではないので、溢れあり。まずは1回折りたたむ。" );
		...
		public new void Draw2D( Device device, int x, int y )
		{
#if TEST_FOLDTEXTURE
			...
			for ( int n = 0; n <= _foldtimes; n++ )
			{
				...
				if ( b横長のテクスチャである )
				{
					...
					r = new Rectangle( 0, currentHeight, this.FullImage.Width, _orgHeight );
					base.Draw2D( device, x + n * this.FullImage.Width, y, 1f, r );
				}
				else
				{
					int currentWidth = n * _orgWidth;
					r = new Rectangle( currentWidth, 0, _orgWidth, this.FullImage.Height );
					base.Draw2D( device, x, y + n * this.FullImage.Height, 1f, r );
				}
			}
#endif
		}
		public new void Draw2D( Device device, int x, int y, Rectangle rc )
		{
			Rectangle r;
			if ( b横長のテクスチャである )
			{
				int beginFold = rc.X / this.FullImage.Width;
				int endFold = ( rc.X + rc.Width ) / FullImage.Width;
				for ( int i = beginFold; i <= endFold; i++ )
				{
					if ( i > _foldtimes ) break;

					int newRcY = i * _orgHeight + rc.Y;
					int newRcX = ( i == beginFold ) ? ( rc.X % this.FullImage.Width ) : 0;
					int newRcWidth = ( newRcX + rc.Width > FullImage.Width ) ? FullImage.Width - newRcX : rc.Width;

					r = new Rectangle( newRcX, newRcY, newRcWidth, rc.Height );
					base.Draw2D( device, x, y, 1f, r );

					int deltaX = ( i == beginFold ) ? ( i + 1 ) * FullImage.Width - rc.X : FullImage.Width;
			...
			{
				int beginFold = rc.Y / this.FullImage.Height;
				int endFold = ( rc.Y + rc.Height ) / FullImage.Height;
				for ( int i = beginFold; i <= endFold; i++ )
				{
					if ( i > _foldtimes ) break;

					int newRcX = i * _orgWidth + rc.X;
					int newRcY = ( i == beginFold ) ? ( rc.Y % this.FullImage.Height ) : 0;
					int newRcHeight = ( newRcY + rc.Height > FullImage.Height ) ? FullImage.Height - newRcY : rc.Height;

					r = new Rectangle( newRcX, newRcY, rc.Width, newRcHeight );
					base.Draw2D( device, x, y, 1f, r );

					int deltaY = ( i == beginFold ) ? ( i + 1 ) * FullImage.Height - rc.Y : FullImage.Height;
		...
		public new void Draw2D( Device device, float x, float y )
		{
			Draw2D( device, (int) x, (int) y );
		}
		public void Draw2D( Device device, float x, float y, Rectangle rc )
		{
			Draw2D( device, (int) x, (int) y, rc );
		}
