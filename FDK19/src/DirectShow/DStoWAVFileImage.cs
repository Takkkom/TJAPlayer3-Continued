	...
	public class DStoWAVFileImage
	{
		...
		public static void t変換( string fileName, out byte[] wavFileImage )
		{
			...
			try
			{
				...
				{
					FDKCommon.ReleaseCOMObject( ref sampleGrabber );
				}
				//-----------------
				#endregion
				...
				DirectShow.tビデオレンダラをグラフから除去する( graphBuilder );		// オーディオレンダラをNullに変えるより前に実行すること。（CDirectShow.tオーディオレンダラをNullレンダラに変えてフォーマットを取得する() の中で一度再生するので、そのときにActiveウィンドウが表示されてしまうため。）
				...
				WaveFormat wfx;
				byte[] wfx拡張領域;
				DirectShow.tオーディオレンダラをNullレンダラに変えてフォーマットを取得する( graphBuilder, out wfx, out wfx拡張領域 );
			...
			{
				FDKCommon.ReleaseCOMObject( ref graphBuilder );
			}
		}
