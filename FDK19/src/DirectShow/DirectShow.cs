...
namespace FDK
{
	...
	/// </summary>
	public class DirectShow : IDisposable
	{
		// プロパティ

		public const uint WM_DSGRAPHNOTIFY = Win32Manager.WM_APP + 1;

		public enum GraphStateType { 完全停止中, 再生のみ停止中, 再生中, 完全停止へ遷移中, 再生のみ停止へ遷移中, 再生へ遷移中, 未定 }
		public GraphStateType NowGraphState
		{
			get
			{
				var status = GraphStateType.未定;
				...
				if( this.MediaCtrl != null )
				{
					...
					if( hr == Win32Manager.E_FAIL )
					{
						#region [ 失敗。]
						//-----------------
						status = GraphStateType.未定;
						//-----------------
						#endregion
					}
					else if( hr == Win32Manager.VFW_S_STATE_INTERMEDIATE )
					{
						#region [ 遷移中。]
						//-----------------
						switch( fs )
						{
							case FilterState.Running:
								status = GraphStateType.再生へ遷移中;
								break;

							case FilterState.Paused:
								status = GraphStateType.再生のみ停止へ遷移中;
								break;

							case FilterState.Stopped:
								status = GraphStateType.完全停止へ遷移中;
								break;
					...
					else
					{
						...
						{
							case FilterState.Running:
								status = GraphStateType.再生中;
								break;

							case FilterState.Paused:
								status = GraphStateType.再生のみ停止中;
								break;

							case FilterState.Stopped:
								status = GraphStateType.完全停止中;
								break;
		...
		public bool IsSoundOnly
		{
			get;
			protected set;
		}
		
		public long NowGraphPositionMs
		{
			get
			{
		...
		public int Volume
		{
			...
			set
			{
				...
				{
					volume = (int) ( ( 20.0 * Math.Log10( ( (double) value ) / 100.0 ) ) * 100.0 );
				}
		...
		public int Balance
		{
			set
			{
				...
				int balance = Math.Min( Math.Max( value, -100 ), +100 );
				int balance_double = 0;
				...
				{
					balance_double = (int) ( ( 20.0 * Math.Log10( ( (double) ( balance + 100 ) ) / 100.0 ) ) * 100.0 );
				}
				else
				{
					balance_double = (int) ( ( -20.0 * Math.Log10( ( (double) ( 100 - balance ) ) / 100.0 ) ) * 100.0 );
				}

				// デシベル位置でグラフの位置を変更。

				this.BasicAudio.put_Balance( balance_double );
			}
		}
		...
		public DirectShow( string fileName, IntPtr hWnd, bool withOutAudioRenderer )
		{
			// 初期化。
			...
			DirectShow.tインスタンスを登録する( this );


			// 並列処理準備。

			if( DirectShow.n並列度 == 0 )	// 算出がまだなら算出する。
				DirectShow.n並列度 = Environment.ProcessorCount;	// 並列度＝CPU数とする。

			unsafe
			{
				this.dgライン描画ARGB32 = new DGライン描画[ DirectShow.n並列度 ];
				this.dgライン描画XRGB32 = new DGライン描画[ DirectShow.n並列度 ];

				for( int i = 0; i < DirectShow.n並列度; i++ )
				{
			...
			try
			{
...
#else
                    DirectShow.SearchMMRenderers( this.graphBuilder, out videoRenderer, out videoInputPin, out audioRenderer, out audioInputPin );
#endif
					if( videoRenderer == null && audioRenderer != null )
						this.IsSoundOnly = true;
					else
					{
						FDKCommon.ReleaseCOMObject( ref videoInputPin );
						FDKCommon.ReleaseCOMObject( ref videoRenderer );
                        FDKCommon.ReleaseCOMObject( ref audioInputPin );
                        FDKCommon.ReleaseCOMObject( ref audioRenderer );
					}
				}
				...
				if( !this.IsSoundOnly )
				{
					...
					this.nスキャンライン幅byte = (int) this.nデータサイズbyte / this.Height;
				...
				if( withOutAudioRenderer )
				{
					WaveFormat dummy1;
					byte[] dummy2;
					DirectShow.tオーディオレンダラをNullレンダラに変えてフォーマットを取得する( this.graphBuilder, out dummy1, out dummy2 );
				}
				...
				this.MediaEventEx.SetNotifyWindow( hWnd, (int) WM_DSGRAPHNOTIFY, new IntPtr( this.InstanceID ) );
			}
#if !DEBUG
			catch( Exception e )
			{
				FDKCommon.LogDetailsOfExceptions( e );
				this.Dispose();
				throw;	// 例外発出。
			}
#endif
		...
		public GraphStateType t遷移完了まで待って状態を取得する()
		{
			var status = GraphStateType.未定;
			...
			return this.NowGraphState;
		}
		public unsafe void t現時点における最新のスナップイメージをTextureに転写する( FDKTexture texture )
		{
			int hr;
			...
			try
			{
				...
				{
					this.IsFlipY = false;		// こちらの方法では常に正常
					...
					var ptr = stackalloc UInt32*[ DirectShow.n並列度 ];	// stackalloc（GC対象外、メソッド終了時に自動開放）は、スタック変数相手にしか使えない。
					ptr[ 0 ] = (UInt32*) dr.Data.DataPointer.ToPointer();	//		↓
					for( int i = 1; i < DirectShow.n並列度; i++ )			// スタック変数で確保、初期化して…
						ptr[ i ] = ptr[ i - 1 ] + this.Width;				//		↓
					...
					else
					{
						...
						var ar = new IAsyncResult[ DirectShow.n並列度 ];
						for( int i = 0; i < DirectShow.n並列度; i++ )
						{
							ar[ i ] = ( bARGB32 ) ?
		...
		public static void tグラフを解析しデバッグ出力する( IGraphBuilder graphBuilder )
		{
			...
			DsError.ThrowExceptionForHR( hr );
			{
				...
				while( eFilters.Next( 1, filters, IntPtr.Zero ) == Win32Manager.S_OK )
				{
					...
					DsError.ThrowExceptionForHR( hr );
					{
						...
						if( filterInfo.pGraph != null )
							FDKCommon.ReleaseCOMObject( ref filterInfo.pGraph );
					}
					...
					{
						var pins = new IPin[ 1 ];
						while( ePins.Next( 1, pins, IntPtr.Zero ) == Win32Manager.S_OK )
						{
							PinInfo pinInfo;
									...
									DsError.ThrowExceptionForHR( hr );
									{
										...
										DsError.ThrowExceptionForHR( hr );
										{
											...
											if( connectFilterInfo.pGraph != null )
												FDKCommon.ReleaseCOMObject( ref connectFilterInfo.pGraph );
										}

										Debug.WriteLine( connectPinInfo.name );		// 接続先ピン名
										if( connectPinInfo.filter != null )
											FDKCommon.ReleaseCOMObject( ref connectPinInfo.filter );
										DsUtils.FreePinInfo( connectPinInfo );
									}
									FDKCommon.ReleaseCOMObject( ref connectPin );
								}
								if( pinInfo.filter != null )
									FDKCommon.ReleaseCOMObject( ref pinInfo.filter );
								DsUtils.FreePinInfo( pinInfo );
							}
							FDKCommon.ReleaseCOMObject( ref pins[ 0 ] );
						}
					}
					FDKCommon.ReleaseCOMObject( ref ePins );

					FDKCommon.ReleaseCOMObject( ref filters[ 0 ] );
				}
			}
			FDKCommon.ReleaseCOMObject( ref eFilters );
		...
		public static void tオーディオレンダラをNullレンダラに変えてフォーマットを取得する( IGraphBuilder graphBuilder, out WaveFormat wfx, out byte[] wfx拡張データ )
		{
			...
			try
			{
				...
				audioRenderer = DirectShow.tオーディオレンダラを探して返す( graphBuilder );
				...
				//-----------------
				{
					...
					hr = Win32Manager.S_FALSE;
					while( fs != FilterState.Stopped || hr != Win32Manager.S_OK )
				...
				try
				{
					...
					if( nはみ出しサイズbyte > 0 )
					{
						...
						unsafe
						{
							...
							Win32Manager.CopyMemory( dst, src + nWaveFormatEx本体サイズ, (uint) nはみ出しサイズbyte );
						}
						hGC.Free();
					}
					//-----------------
			...
			{
				FDKCommon.ReleaseCOMObject( ref nullRendererInputPin );
				FDKCommon.ReleaseCOMObject( ref nullRenderer );
				FDKCommon.ReleaseCOMObject( ref rendererConnectedOutputPin );
				FDKCommon.ReleaseCOMObject( ref rendererInputPin );
				FDKCommon.ReleaseCOMObject( ref audioRenderer );
			}
		}
		public static void ConnectNullRendererFromSampleGrabber(IGraphBuilder graphBuilder, IBaseFilter sampleGrabber)
		{
			...
			{
				// videoRenderer を探す。
				DirectShow.SearchMMRenderers(graphBuilder, out videoRenderer, out videoRendererInputPin, out audioRenderer, out audioRendererInputPin);
				...
				{
					FDKCommon.ReleaseCOMObject(ref connectedOutputPin);
			...
			{
				FDKCommon.ReleaseCOMObject(ref connectedOutputPin);
				FDKCommon.ReleaseCOMObject(ref videoRendererInputPin);
				FDKCommon.ReleaseCOMObject(ref videoRenderer);
				FDKCommon.ReleaseCOMObject(ref audioRenderer);
				FDKCommon.ReleaseCOMObject(ref audioRendererInputPin);
				FDKCommon.ReleaseCOMObject(ref nullRendererInputPin);
				FDKCommon.ReleaseCOMObject(ref grabberOutputPin);
				FDKCommon.ReleaseCOMObject(ref grabberOutputConnectedPin);
			}
		}
						...
						try
						{
							...
							while( ePins.Next( 1, pins, IntPtr.Zero ) == Win32Manager.S_OK )
							{
								try
								{
									...
									if( hr == Win32Manager.VFW_E_NOT_CONNECTED )
										continue;	// つながってない
									DsError.ThrowExceptionForHR( hr );
									//-----------------
									#endregion
		...
		public static void tビデオレンダラをグラフから除去する( IGraphBuilder graphBuilder )
		{
			...
			try
			{
				...
				DirectShow.tビデオレンダラとその入力ピンを探して返す( graphBuilder, out videoRenderer, out renderInputPin );
			...
			{
				FDKCommon.ReleaseCOMObject( ref connectedOutputPin );
				FDKCommon.ReleaseCOMObject( ref renderInputPin );
				FDKCommon.ReleaseCOMObject( ref videoRenderer );
			}
		}
				...
				while( ePins.Next( 1, pins, IntPtr.Zero ) == Win32Manager.S_OK )
				{
					...
					finally
					{
						if( pinfo.filter != null )
							FDKCommon.ReleaseCOMObject( ref pinfo.filter );
		...
		protected static Dictionary<int, DirectShow> dicインスタンス = new Dictionary<int, DirectShow>();	// <インスタンスID, そのIDを持つインスタンス>

		protected static void tインスタンスを登録する( DirectShow ds )
		{
			for( int i = 1; i < DirectShow.nインスタンスIDの最大数; i++ )
			{
				if( !DirectShow.dicインスタンス.ContainsKey( i ) )		// 空いている番号を使う。
				{
					ds.InstanceID = i;
					DirectShow.dicインスタンス.Add( i, ds );
					break;
				}
			}
		}
		...
		{
			if( DirectShow.dicインスタンス.ContainsKey( nインスタンスID ) )
				DirectShow.dicインスタンス.Remove( nインスタンスID );
		}
		//-----------------
		#endregion
		...
		protected virtual void Dispose( bool bManagedリソースも解放する )
		{
			if( bManagedリソースも解放する )
			{
				...
				DirectShow.tインスタンスを解放する( this.InstanceID );
			}
			...
			FDKCommon.ReleaseCOMObject( ref this.nullRenderer );
			FDKCommon.ReleaseCOMObject( ref this.memoryRenderer );
			FDKCommon.ReleaseCOMObject( ref this.memoryRendererObject );
			FDKCommon.ReleaseCOMObject( ref this.graphBuilder );
			//-----------------
			#endregion

			FDKCommon.RunGarbageCollector();
		}
        ~DirectShow()
		{
		...
		private unsafe void tライン描画XRGB32( int n )
		{
			...
			for( int y = n; y < this.Height; y += DirectShow.n並列度 )
			{
				byte* ptrPixel = ptrSnap + ( ( ( this.Height - y ) - 1 ) * this.nスキャンライン幅byte );

				// アルファ無視なので一括コピー。CopyMemory() は自前でループ展開するよりも速い。
				Win32Manager.CopyMemory( (void*) ptrTexture, (void*) ptrPixel, (uint) ( this.Width * 4 ) );

				ptrTexture += this.Width * DirectShow.n並列度;
			}
		}
		private unsafe void tライン描画ARGB32( int n )
		{
			...
			L031: *pt++ = nAlpha | *ptrPixel++;
				if( ( pt - ptrTexture ) < this.Width ) goto L000;
