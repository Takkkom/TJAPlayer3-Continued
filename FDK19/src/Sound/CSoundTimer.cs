...
namespace FDK
{
	public class CSoundTimer : BaseTimer
	{
		public override long SystemTimeMs
		{
			get
			{
					...
					this.Device.e出力デバイス == ESoundDeviceType.ASIO )
				{
					...
					if ( this.Device.n経過時間を更新したシステム時刻ms == Timer.Unused )	// #33890 2014.5.27 yyagi
					{
					...
					else
					{
						...
						{
							return this.Device.n経過時間ms
								+ ( this.Device.tmシステムタイマ.SystemTimeMs - this.Device.n経過時間を更新したシステム時刻ms );
						}
					}
				}
		...
		internal CSoundTimer( ISoundDevice device )
		{
			...
			if ( this.Device.e出力デバイス != ESoundDeviceType.DirectSound )
			{
				...
				ctDInputTimer = new Timer( Timer.TimerType.MultiMedia );			// 1秒に1回時差を測定するようにしておく
			}
			else																// TESTCODE DirectSound時のみ、CSoundTimerでなくCTimerを使う
			{
			    ct = new Timer( Timer.TimerType.MultiMedia );
			}
		}
