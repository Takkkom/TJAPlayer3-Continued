...
namespace FDK
{
	public class FDKSoundTimer : BaseTimer
	{
		public override long SystemTimeMs
		{
			get
			{
				if( this.Device.NowSoundDevice == SoundDeviceType.ExclusiveWASAPI || 
					this.Device.NowSoundDevice == SoundDeviceType.SharedWASAPI ||
					this.Device.NowSoundDevice == SoundDeviceType.ASIO )
				{
					...
					if ( this.Device.SystemTimeWithUpdatedElapsedTime == FDKTimer.Unused )	// #33890 2014.5.27 yyagi
					{
					...
					else
					{
						...
						{
							return this.Device.ElapsedTimeMs
								+ ( this.Device.SystemTimer.SystemTimeMs - this.Device.SystemTimeWithUpdatedElapsedTime );
						}
					}
				}
				else if( this.Device.NowSoundDevice == SoundDeviceType.DirectSound )
				{
					//return this.Device.n経過時間ms;		// #24820 2013.2.3 yyagi TESTCODE DirectSoundでスクロールが滑らかにならないため、
					return ct.SystemTimeMs;				// 仮にCSoundTimerをCTimer相当の動作にしてみた
				}
				return BaseTimer.Unused;
			}
		}

		internal FDKSoundTimer( ISoundDevice device )
		{
			this.Device = device;

			if ( this.Device.NowSoundDevice != SoundDeviceType.DirectSound )
			{
				TimerCallback timerDelegate = new TimerCallback( SnapTimers );  // CSoundTimerをシステム時刻に変換するために、
                timer = new System.Threading.Timer( timerDelegate, null, 0, 1000 );				// CSoundTimerとCTimerを両方とも走らせておき、
				ctDInputTimer = new FDKTimer( FDKTimer.TimerType.MultiMedia );			// 1秒に1回時差を測定するようにしておく
			}
			else																// TESTCODE DirectSound時のみ、CSoundTimerでなくCTimerを使う
			{
			    ct = new FDKTimer( FDKTimer.TimerType.MultiMedia );
			}
		}
		...
		public long TimeStampToSoundTimeMs( long timeStamp )
		{
			return timeStamp - this.nDInputTimerCounter + this.nSoundTimerCounter;	// Timer違いによる時差を補正する
		}
