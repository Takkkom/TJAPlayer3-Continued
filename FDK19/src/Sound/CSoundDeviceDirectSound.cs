...
namespace FDK
{
	internal class CSoundDeviceDirectSound : ISoundDevice
	{
		...
		public long n経過時間ms
		{
			get
			{
				if ( ctimer != null )
				{
					...
					long n現在のシステム時刻ms = this.tmシステムタイマ.SystemTimeMs;
				...
				{
					long nRet = ctimer.SystemTimeMs - this.n前に経過時間を測定したシステム時刻ms;
					if ( nRet < 0 )	// カウンタがループしたときは
					{
						nRet = ( ctimer.nシステム時刻 - long.MinValue ) + ( long.MaxValue - this.n前に経過時間を測定したシステム時刻ms ) + 1;
					}
					this.n前に経過時間を測定したシステム時刻ms = ctimer.SystemTimeMs;
		...
		public CSoundDeviceDirectSound( IntPtr hWnd, long n遅延時間ms, bool bUseOSTimer )
		{
			...
			this.tmシステムタイマ = new Timer( Timer.TimerType.MultiMedia );
			...
			if ( !bUseOSTimer )
			{
				...
				this.n前に経過時間を測定したシステム時刻ms = this.tmシステムタイマ.SystemTimeMs;
				//-----------------
				#endregion
			}
			else
			{
				ctimer = new Timer( Timer.TimerType.MultiMedia );
			}
		...
		protected void Dispose( bool bManagedDispose )
		{
			...
			if ( bManagedDispose )
			{
				...
				if ( this.sd経過時間計測用サウンドバッファ != null )
				{
					...
					FDKCommon.Dispose( ref this.sd経過時間計測用サウンドバッファ );
				}
				//-----------------
				#endregion
				...
				FDKCommon.Dispose( this.tmシステムタイマ );
			}
			if ( ctimer != null )
			{
...
//		protected AutoResetEvent autoResetEvent = new AutoResetEvent( false );
		...
		private long n前に経過時間を測定したシステム時刻ms = Timer.Unused;
